...
- [x] H-4 环/边界检测（当前可达主链路：子路径自引用、路径成环、最大步数、值集展开上限、单位/公式失败语义；条件片段环随 P12-5 条件片段库落地接入）。
  - ✅ 测试：各非法/边界情形确定性失败；条件片段库当前无运行模型，不伪造片段环实现。
- [ ] H-5 引用与包版本一致性校验（发布门禁 + 运行期）；引用资产变更影响分析。
  - ✅ 测试：跨版本引用被拒；片段变更提示受影响资产。
...
- [ ] P12-1 自然语言预览：条件树/守卫 → 可读中文句子（`POST /authoring/preview`），与 L2/L3 并排。
  - ✅ 测试：嵌套+公式+值集语义无损渲染。
- [ ] P12-2 向导式创建 + 智能默认 + 渐进披露（高级项折叠）。
  - ✅ 测试：原型向导生成结构正确草稿；默认可提交。
- [ ] P12-3 即配即试：草稿 + 真实快照就地试运行与证据（`preview-run`），不绕门禁、不造数。
