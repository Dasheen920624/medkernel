...
- [x] P12-7 批量导入/导出（复用 `PackageOfflineImport`）+ 模板×参数表批量生成 + 批量发布（聚合影响、高危逐条）+ 批量分发（复用 `SyncTarget`）+ `batch_job` 进度审计。
...
- [x] P13-3 Outbox 产出分发到待办/通知/临床提醒治理/质控驾驶舱 + 回写诚实降级。
  - ✅ 测试：分发可重放、无重复副作用、不可达诚实降级。
- [x] P13-4 专病包端到端（CKD 示例）：路径+规则+值集+公式+片段封装，跨院分发+本地覆盖+回滚。
  - ✅ 端到端测：入径→分期→开药 BLOCK→变异→结局指标闭环。
- [x] P13-5 实时 CDS 落地（开医嘱）满足时延预算与诚实降级。
