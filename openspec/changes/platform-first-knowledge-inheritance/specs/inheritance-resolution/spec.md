...
`InheritanceResolver` SHALL 在读取/运行期按目标机构实时解析有效版本：以平台基线为起点，沿组织闭包（平台→REGION/FACILITY/…→目标，最一般到最具体）应用可适用覆盖，最具体者最终生效，不得落地中间副本。

#### Scenario: 七层链最具体优先
- **WHEN** 平台 v0、REGION INHERITABLE v1、FACILITY REPLACE v2 同时存在，目标为该机构下某科室且科室无覆盖
- **THEN** 解析返回 v2
...
#### Scenario: 过期覆盖不生效
- **WHEN** FACILITY 覆盖 effective_to 已过
