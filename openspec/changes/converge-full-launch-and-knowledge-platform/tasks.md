...
- [x] 3.4 为每个 issuer 创建独立身份和独立 keyId。依赖：3.3；主要文件/接口：IssuerRegistrationService、SigningKeyPort；失败测试：两个 issuer 复用 keyId 或密钥材料时失败；验证：BE -Dtest=IssuerRegistrationServiceTest test；证据键：authority.issuer.independent-key。
- [x] 3.5 建立外置 HSM/KMS 密钥端口。依赖：3.4；主要文件/接口：SigningKeyPort、HsmKmsSigningAdapter、测试 InMemorySigningAdapter；失败测试：私钥进入数据库实体、普通备份、包或日志时失败；验证：BE -Dtest=SigningKeyBoundaryTest test 和秘密 inventory；证据键：authority.key.external-boundary。
