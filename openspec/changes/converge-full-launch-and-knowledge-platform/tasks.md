...
- [ ] 2.1 定义确定性 FULL `.mkp` 和 13 类统一适配器。依赖：1.4；结果：规范化 manifest 绑定权威、issuer、key、发布序号、父摘要、兼容范围和逐文件 SM3，13 个 `VersionedAssetType` 全部登记 `export/validate/materialize`，相同输入跨宿主逐字节一致；验证：缺类型、重复类型、非确定字段、规则共享条件片段、路径共享子路径或循环均先红后绿；证据键：`mkp.full.manifest-adapters13`。
