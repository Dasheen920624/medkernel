...
> 🟡 **临时方案（TEMPORARY / 过渡）**：本目录是为现场 `192.168.8.191` 尽快上线而**临时搭建**的
> 单机 + Oracle 生产部署，**不是标准交付物**。**正式 / 长期生产环境请使用 `deploy/docker/` 的标准
> 容器化构建**；本套脚本仅作过渡，待 Docker 标准环境就绪后应退役。

> 适用：医院内网 **单机 + Oracle** 的现场部署（区别于 `deploy/docker/` 的容器化标准平台）。
