...
# 只在六阶段演练全部通过后执行：严格 TLS、真实重启、关系库持久化、备份与隔离恢复缺一不可。
set -euo pipefail
...
验收内容：六阶段演练证据、严格 TLS、服务重启、四职责、全知识、模型生产、沙盘、审计、
...
verify_full_system_evidence() {
  jq -e --arg source "$EXPECTED_SOURCE" '
    ...
    .coverage.providerResilienceVerified == true and
    .coverage.b0CoreVerifiedWithoutModel == true and
    .coverage.mfaDefaultEnabled == false and
    .coverage.tlsVerificationSkipped == false
  ' "$FULL_SYSTEM_EVIDENCE" >/dev/null || die "六阶段整套演练证据未完整通过"
...
verify_live_database() {
  ...
  local users role_assignments role_kinds invalid_roles platform_roles rehearsal_roles
  local identities domains current_active versions lifecycle_identity lifecycle_v1 lifecycle_v2
  local lifecycle_versions lifecycle_current jobs model_tasks gate_jobs gate_failures shadow_jobs shadow_failures
  local sandbox_rules sandbox_cases sandbox_binding provider_rows eval_rows audit_events
  ...
  role_assignments="$(database_query "$DATABASE" "select count(*) from user_role_assignment where tenant_id in ('t-1','t-rehearsal') and active_flag='Y';")"
  role_kinds="$(database_query "$DATABASE" "select count(distinct role_code) from user_role_assignment where tenant_id in ('t-1','t-rehearsal') and active_flag='Y';")"
  invalid_roles="$(database_query "$DATABASE" "select count(*) from user_role_assignment where tenant_id in ('t-1','t-rehearsal') and active_flag='Y' and role_code not in ('platform-admin','engine-operator','clinical-user','auditor');")"
  platform_roles="$(database_query "$DATABASE" "select count(*) from user_role_assignment where tenant_id='t-1' and active_flag='Y';")"
  rehearsal_roles="$(database_query "$DATABASE" "select count(*) from user_role_assignment where tenant_id='t-rehearsal' and active_flag='Y';")"
  assert_equal "九个上线身份" "9" "$users"
  assert_equal "四职责有效分配" "8" "$role_assignments"
  assert_equal "四职责种类" "4" "$role_kinds"
  assert_equal "非法旧角色分配" "0" "$invalid_roles"
  assert_equal "平台四职责" "4" "$platform_roles"
  assert_equal "演练机构四职责" "4" "$rehearsal_roles"
  ...
  shadow_jobs="$(database_query "$DATABASE" "select count(distinct job_code) from mk_knowledge_shadow_run where tenant_id='t-1' and status='PASSED' and ready_for_review=true;")"
  shadow_failures="$(database_query "$DATABASE" "select count(*) from mk_knowledge_shadow_run where tenant_id='t-1' and (status<>'PASSED' or ready_for_review=false or degradation_detected=true);")"
  ...
  assert_equal "影子评测通过任务" "12" "$shadow_jobs"
  ...
  sandbox_binding="$(database_query "$DATABASE" "select count(*) from mk_sandbox_runtime_binding where tenant_id='t-rehearsal' and status='ACTIVE';")"
  assert_equal "沙盘规则" "10" "$sandbox_rules"
  assert_equal "沙盘测试用例" "40" "$sandbox_cases"
  assert_equal "沙盘 CURRENT 运行绑定" "1" "$sandbox_binding"
...
sandbox_active_binding=$sandbox_binding
...
backup_and_restore_database() {
  local timestamp source_short dump_file restored_tables restored_roles restored_knowledge
  local restored_provider restored_rules restored_cases restored_audit
  ...
  restored_roles="$(database_query "$RESTORE_DATABASE" "select count(*) from user_role_assignment where tenant_id in ('t-1','t-rehearsal') and active_flag='Y';")"
  ...
  assert_equal "隔离恢复四职责分配" "8" "$restored_roles"
...
restore_active_role_assignments=$restored_roles
