...
grep -q '.evidenceStage != "launch-coverage"' "$SCRIPT"
grep -q '.evidencePath | type == "string" and length > 0' "$SCRIPT"
grep -q '.evidenceKey | type == "string" and length > 0' "$SCRIPT"
grep -q '.observedCode == .code' "$SCRIPT"
grep -q '.observedStatus == "PASSED"' "$SCRIPT"
grep -q '.observedAt | type == "string" and length > 0' "$SCRIPT"
grep -q '.coverage.productLayers | allPassed(6)' "$SCRIPT"
grep -q '.coverage.stakeholderViews | allPassed(12)' "$SCRIPT"
