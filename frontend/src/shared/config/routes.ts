/**
 * 页面体验元数据与产品入口合同的组合层。
 * AppLayout + router + 菜单 + 面包屑 + 权限元数据全部读取组合结果。
 */
import {
  productEntryCatalog,
  type ProductEntry,
...
const productEntryByRoute = new Map<string, ProductEntry>();
const productEntryNavigationOrder = new Map<string, number>();
productEntryCatalog.forEach((entry, index) => {
  if (productEntryByRoute.has(entry.route)) {
    throw new Error(`产品入口合同 route 重复：${entry.route}`);
  }
  productEntryByRoute.set(entry.route, entry);
  productEntryNavigationOrder.set(entry.entryCode, index + 1);
});
...
const registeredRoutePaths = new Set(routeMetaInputs.map((route) => route.path));
productEntryCatalog.forEach((entry) => {
  if (!registeredRoutePaths.has(entry.route)) {
    throw new Error(`产品入口合同缺少页面路由：${entry.entryCode}/${entry.route}`);
  }
});
...
function normalizeRouteMeta(route: RouteMetaInput): RouteMeta {
  const productEntry = productEntryByRoute.get(route.path);
  if (productEntry) {
    if (!route.requireAuth) {
      throw new Error(`产品入口必须启用认证：${productEntry.entryCode}`);
    }
    return {
      ...route,
      ...
      navigationOrder:
        productEntryNavigationOrder.get(productEntry.entryCode) ?? Number.MAX_SAFE_INTEGER,
      hidden: productEntry.placement !== "primary",
      requiredPermissions: [...productEntry.requiredPermissions],
      requiresSixStates: true,
