...
const routeMetaInputs: RouteMetaInput[] = [
  ...
  {
    path: "/qc/model-evaluations",
    title: "医学回归复核",
    breadcrumb: ["质量管理", "医学回归复核"],
    requireAuth: true,
    sectionKey: "quality-management",
    ...
    experience: {
      primaryRole: "质量与医保治理员",
      goal: "逐例核查模型医学回归证据并独立签字",
      defaultView: "待复核运行",
      defaultFilters: [
        {
          key: "status",
          ...
          options: [
            ...
            { label: "已阻断", value: "FAILED" },
          ],
        },
      ],
      expertContent: ["providerCode", "promptVersion", "toolVersion", "原始引用载荷"],
      interruptionLevel: "strong",
      evidence: "逐例输入、真实输出、来源引用、医学红线判定、复核意见与签字时间均留痕",
