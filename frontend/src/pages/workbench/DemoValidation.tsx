...
export default function DemoValidation() {
  ...
  const canQueryRuntime = Boolean(profile && canOpenDemoValidation(profile));
  ...
  if (security.isLoading) {
    return (
      <PageShell title="演示与校验" description="正在确认当前角色" state="loading">
        <></>
      </PageShell>
    );
  }
  ...
  if (!profile || !canOpenDemoValidation(profile)) {
    return (
      <PageShell
        title="演示与校验"
        ...
        stateProps={{
          title: "当前权限不足",
          description: "演示与校验仅开放给实施、信息科、医院管理员和平台管理员。",
        }}
      >
        <></>
      </PageShell>
  ...
  if (runtime.isError) {
    const parsed = parseApiError(runtime.error, "暂时无法读取演示自检");
    return (
      <PageShell
        title="演示与校验"
        description="演示自检失败"
  ...
  return (
    <PageShell title="演示与校验" description="演示前确认阻塞与去处" primary={retryButton}>
      <Space direction="vertical" size="large" className="mk-full-width">
        ...
        <Alert
          ...
          description={`${summary.ready} 可演示 / ${summary.blocked} 阻塞 / ${summary.disabled} 未启用`}
        />
        ...
        <Space wrap>
          <Card>
            <Statistic title="可演示" value={summary.ready} />
          </Card>
          <Card>
        ...
        <Card title="状态筛选" data-testid="demo-validation-default-filters">
          <Checkbox.Group
            ...
            <Space wrap>
              {STATUS_FILTERS.map((status) => (
                <Checkbox
                  key={status}
                  value={status}
                  data-testid={`demo-validation-filter-${status}`}
                >
                  {STATUS_LABEL[status]}
                </Checkbox>
              ))}
        ...
        <Card title="就绪度自检项">
          <Table<SelfCheckItem>
            ...
            onRow={(record) =>
              ({
                "data-testid": `demo-validation-item-${record.key}`,
...
function canOpenDemoValidation(profile: SecurityProfile): boolean {
  ...
  return (
    hasWorkbenchMenu &&
    permissions.has("workbench:demo:view") &&
...
function buildBackupItem(snapshot: RuntimeOperationsSnapshot): SelfCheckItem {
  return {
    ...
    item: "备份恢复",
    source: "运行底座",
    status: snapshot.backup.enabled ? "ready" : "disabled",
    reason: snapshot.backup.enabled
      ? `RPO ${snapshot.backup.rpo} / RTO ${snapshot.backup.rto}`
      : "备份恢复未启用，不计入可演示项",
    repairPath: "/system/providers",
    partial: false,
  };
...
function reasonForDependency(dependency: RuntimeDependencyStatus, status: ReadinessStatus): string {
  ...
  return `${dependency.displayName}不可演示：${dependency.detail}`;
}
...
function summarize(items: SelfCheckItem[]) {
  ...
  let conclusion = "全部自检项可演示";
  if (counts.blocked > 0) {
    conclusion = "存在阻塞项，演示前需处理";
  } else if (counts.disabled > 0) {
    conclusion = "可演示项已就绪，未启用能力不计入通过";
  }
