...
export default function AiReview() {
  ...
  return (
    <>
      <PageShell
        title="AI 知识审核"
        description="人工审核知识候选并发布权威版本"
        state={pageState}
        stateProps={pageStateProps}
        extras={
          <Button
            aria-label="刷新 AI 知识审核"
            icon={<ReloadOutlined />}
            onClick={() => {
      ...
      >
        <Space direction="vertical" size="large" className="mk-full-width">
          ...
          <Descriptions column={1} bordered size="small" title="现行权威版本">
            ...
            <Descriptions.Item label="来源锚点">
              {activeVersion?.anchors ?? "未返回来源锚点"}
            </Descriptions.Item>
          </Descriptions>
          ...
          <Descriptions column={1} bordered size="small" title="待审候选版本">
            ...
            <Descriptions.Item label="来源锚点">
              {candidateVersion?.anchors ?? "未返回来源锚点"}
            </Descriptions.Item>
