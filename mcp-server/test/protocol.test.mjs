  ...
  const client = {
    ...
    get: async (path) => {
      ...
      return [
        ...
        { name: 'fetchPublicMaterial', purpose: 'Agent 受控获取公域资料', dataLevel: 'D1', requiredPermission: 'knowledge.write' },
      ];
    },
...
test('tools/list 经后端 /tools 映射为 MCP 工具（含 inputSchema，purpose 必填）', async () => {
  ...
  const fetch = res.result.tools.find((t) => t.name === 'fetchPublicMaterial');
  assert.match(fetch.description, /公域资料/);
  assert.equal(fetch.inputSchema.properties.payload.type, 'object');
});
