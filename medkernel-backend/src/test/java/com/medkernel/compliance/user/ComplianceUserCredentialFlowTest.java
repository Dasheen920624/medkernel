...
@ActiveProfiles("test")
class ComplianceUserCredentialFlowTest {
    ...
    void createMember_generatesTempPassword_andListsWithoutHash() throws Exception {
        mvc.perform(post("/api/v1/compliance/users").with(admin())
                ...
                .content("{\"credentialManaged\":true,\"username\":\"drwang\",\"roleCode\":\"doctor\"}"))
        ...
        mvc.perform(get("/api/v1/compliance/users").with(admin()))
            ...
            .andExpect(jsonPath("$.data.items[0].username").value("drwang"))
            .andExpect(jsonPath("$.data.items[0].mustChangePwd").value(true))
            .andExpect(jsonPath("$.data.items[0].passwordHash").doesNotExist());
    }

    @Test
    void createMember_duplicateUsername_conflict() throws Exception {
        String body = "{\"credentialManaged\":true,\"username\":\"drwang\",\"roleCode\":\"doctor\"}";
        mvc.perform(post("/api/v1/compliance/users").with(admin())
    ...
    void resetPassword_returnsNewTempPassword() throws Exception {
        ...
        mvc.perform(post("/api/v1/compliance/users/{userId}:reset-password", "drwang").with(admin()))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.data.tempPassword", not(emptyOrNullString())));
    }
    ...
    void resetPasswordTokenIsOneTimeAndForcesChangePassword() throws Exception {
        ...
        var issued = mvc.perform(post("/api/v1/compliance/users/{userId}:reset-password-token", "drwang")
                .with(admin()))
            .andExpect(status().isOk())
    ...
    void setStatus_disablesAccount() throws Exception {
        ...
        mvc.perform(patch("/api/v1/compliance/users/{userId}/status", "drwang").with(admin())
    ...
    void setStatusCannotDisableSystemSuperAdmin() throws Exception {
        mvc.perform(post("/api/v1/compliance/users").with(admin())
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"credentialManaged\":true,\"username\":\"root-admin\",\"initialPassword\":\"Init@2026Pass!\"}"))
            .andExpect(status().isOk());
        Instant now = Instant.now();
        ...
        mvc.perform(patch("/api/v1/compliance/users/{userId}/status", "root-admin").with(admin())
                .contentType(MediaType.APPLICATION_JSON).content("{\"status\":\"DISABLED\"}"))
            .andExpect(status().isForbidden())
            .andExpect(jsonPath("$.code").value("SUPERADMIN_IMMUTABLE"));
    }
    ...
    void resetPasswordCannotEditSystemSuperAdmin() throws Exception {
        ...
        mvc.perform(post("/api/v1/compliance/users/{userId}:reset-password", "root-admin").with(admin()))
            .andExpect(status().isForbidden())
            .andExpect(jsonPath("$.code").value("SUPERADMIN_IMMUTABLE"));
    }
    ...
    void changePassword_wrongOldRejected_thenSuccessClearsMustChange() throws Exception {
        ...
        mvc.perform(get("/api/v1/compliance/users").with(admin()))
            .andExpect(jsonPath("$.data.items[0].mustChangePwd").value(false));
    }
