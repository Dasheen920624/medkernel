 ...
 * 领域门面组合定义。
 *
 * <p>定义只表达「复用哪些既有引擎链路」和服务包成员，不承载真实医学知识内容。
 *
 * @param code 卡 ID / 门面代码
 * @param displayName 中文展示名
 * @param kind 门面类型
 * @param scenarioIds 关联业务场景 ID
 * @param engineChain 复用的共享引擎能力
 * @param dependencyCards 依赖施工卡
 * @param b0Workflows 无模型 B0 可执行工作流摘要
 * @param memberFacadeCodes 服务包成员门面；普通领域门面为空
...
public record DomainFacadeDefinition(
    ...
    public DomainFacadeDefinition {
        ...
        dependencyCards = List.copyOf(dependencyCards);
