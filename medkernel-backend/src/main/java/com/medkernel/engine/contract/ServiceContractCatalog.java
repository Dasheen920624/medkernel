...
public final class ServiceContractCatalog {
    private static final List<ServiceContract> CONTRACTS = List.of(
        ...
        contract("knowledge-acquisition", "公域资料获取服务",
            ...
            permissions("knowledge.read", "knowledge.write", "knowledge.acquisition.approve"),
            audits(
                audit(AuditAction.CREATE, "mk_knowledge_acquisition_source",
                    "登记停用的公域资料来源草稿"),
                audit(AuditAction.UPDATE, "mk_knowledge_acquisition_source",
                    "更新草稿时撤销旧审批，或停用来源与自动调度"),
                audit(AuditAction.REVIEW, "mk_knowledge_acquisition_source",
                    "经 MFA 与职责分离审批后启用公域资料来源"),
        ...
        contract("model-evaluations", "模型医学回归评测服务",
            ...
            audits(
                ...
                audit(AuditAction.UPDATE, "mk_llm_eval_run", "专家核对逐例证据后留评语签字放行"))),
