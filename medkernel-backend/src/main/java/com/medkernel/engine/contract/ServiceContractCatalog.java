    ...
    private static final List<ServiceContract> CONTRACTS = List.of(
        ...
        contract("authoring-batch", "创作批量任务服务",
            ...
            audits(
                ...
                audit(AuditAction.EXECUTE, "mk_engine_authoring_batch_job", "执行规则生成、规则发布和运行发布批量任务"),
        ...
        contract("model-versions", "模型版本三元组治理服务",
            ...
            audits(audit(AuditAction.UPDATE, "mk_llm_model_version_bundle",
                "发布、回滚和导出 prompt/tool/model 版本包"))),
        contract("model-egress", "模型外调出域治理服务",
            ...
            audits(
                ...
                audit(AuditAction.UPDATE, "mk_llm_egress_confirmation", "确认高敏出域用途"))),
        contract("data-minimization-policies", "数据最小化策略服务",
            ...
            permissions("llm.egress.manage"),
            audits(
                audit(AuditAction.UPDATE, "mk_llm_egress_whitelist", "维护数据最小化字段白名单、脱敏规则和确认阈值"),
                audit(AuditAction.UPDATE, "mk_llm_egress_confirmation", "确认模型高敏出域用途"))),
        ...
        contract("model-evaluations", "模型医学回归评测服务",
            ...
            audits(
                ...
                audit(AuditAction.EXECUTE, "mk_llm_eval_run", "运行医学回归评测并固化逐例证据"),
        ...
        contract("runtime-release", "平台基线与医院运行修订服务",
            ...
            audits(
                audit(AuditAction.PUBLISH, "mk_platform_baseline_release", "发布不可变平台权威基线"),
                audit(AuditAction.PUBLISH, "mk_clinical_runtime_release", "激活医院完整运行修订"),
        ...
        contract("sandbox", "全真体验沙盘编排服务",
            ...
            audits(audit(
                AuditAction.EXECUTE, "sandbox_scenario", "按医院当前运行修订编排真实引擎链路并记录复演轨迹"))),
        ...
        contract("bootstrap", "首次部署引导服务",
            ...
            audits(
                ...
                audit(AuditAction.PERMISSION_CHANGE, "platform_credential", "创建初始管理员和绑定 MFA")),
            publicEndpoints(
                "GET /api/v1/bootstrap/status",
                "POST /api/v1/bootstrap/init-token",
                "POST /api/v1/bootstrap/password")),
        contract("tenant-engine", "租户开通与实施服务",
