...
/**
 * 模型外调允许范围配置的统一可执行性校验器。
 *
 * <p>上线准备检查与运行时共用同一规则，避免管理面误报可用、运行时再阻断，或因配置畸形绕过最小化护栏。
 */
    ...
    public static Validation validate(ModelEgressWhitelist whitelist) {
        ...
        try {
            ...
            if (allowed == null || !allowed.isArray()) {
                return Validation.invalid("外调允许字段必须是字段列表");
            }
            for (JsonNode item : allowed) {
                if (!item.isTextual() || item.asText().isBlank()) {
                    return Validation.invalid("外调允许字段含空值或非文本字段");
                }
                allowedFields.add(item.asText().trim());
            }
        } catch (Exception invalidAllowedFields) {
            return Validation.invalid("外调允许字段配置文本不合法");
        }
        if (allowedFields.isEmpty()) {
            return Validation.invalid("外调允许字段不能为空");
        }
        ...
        try {
            ...
            if (configuredRules == null || !configuredRules.isObject()) {
                return Validation.invalid("外调脱敏规则必须是配置对象");
            }
            ...
            while (fields.hasNext()) {
                ...
                if (field.isBlank() || !allowedFields.contains(field)) {
                    return Validation.invalid("脱敏规则字段不在外调允许字段内");
                }
                if (!entry.getValue().isTextual()) {
                    return Validation.invalid("外调脱敏算子必须是字符串");
                }
                String operator = entry.getValue().asText().trim().toUpperCase(Locale.ROOT);
                if (!OPERATORS.contains(operator)) {
                    return Validation.invalid("外调脱敏算子无效");
                }
                rules.put(field, operator);
            }
        } catch (Exception invalidRules) {
            return Validation.invalid("外调脱敏规则配置文本不合法");
        }
