...
/**
 * API-06 写入和执行类请求的标准上下文字段。
 *
 * <p>字段与 D2 统一入参保持一致；控制器在进入服务层前校验租户、用户与角色。
 * 路径运行范围由当前机构生效版本和患者快照解析，前端不再传入手工版本定位作为执行门槛。
 */
public record PathwayApiContext(
...
interface PathwayContextRequest {
    PathwayApiContext apiContext();
}
