 ...
 *
 * <p>导出完成登记环节据此校验真实产物与确认范围一致，并按真实文件字节登记 SM3 摘要。
 * 各导出来源（大列表引擎 / 引擎数据服务层等）经 {@link ExportArtifactProvider} 产出统一形态产物。
 * 置于 shared 层，使业务包（compliance 导出确认）与引擎包（list/datasvc 导出来源）均可依赖，
 * 依赖方向恒为业务/引擎 → shared（SYS-02）。
 *
 * @param jobId 导出任务对外可见 ID
 * @param resourceType 导出资源类型（与确认记录一致）
 * @param requestSnapshot 服务端保存的导出范围快照（与确认范围 JSON 一致）
 * @param idempotencyKey 导出任务幂等键（与确认记录一致）
