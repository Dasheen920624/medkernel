-- MedKernel v1.0 GA · P12-7 创作批量任务（PostgreSQL）
-- ROLLBACK：如需回滚，先导出 mk_engine_authoring_batch_job 与 mk_engine_authoring_batch_item，再删除两表。

CREATE TABLE IF NOT EXISTS mk_engine_authoring_batch_job (
...
CREATE TABLE IF NOT EXISTS mk_engine_authoring_batch_item (
    id              BIGINT       GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    job_id          VARCHAR(128) NOT NULL,
    tenant_id       VARCHAR(64)  NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_mk_engine_authoring_batch_job_status
    ON mk_engine_authoring_batch_job (tenant_id, status, created_at);

CREATE INDEX IF NOT EXISTS idx_mk_engine_authoring_batch_item_job
    ON mk_engine_authoring_batch_item (tenant_id, job_id);

CREATE INDEX IF NOT EXISTS idx_mk_engine_authoring_batch_item_status
