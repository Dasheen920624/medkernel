-- MedKernel v1.0 GA · P5-6.1 租户开通平台包引用制（PostgreSQL）
...
CREATE TABLE IF NOT EXISTS mk_pkg_tenant_package_reference (
    ...
    source_template_code VARCHAR(128) NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_pkg_tpref_tenant_status
    ON mk_pkg_tenant_package_reference (tenant_id, status, updated_at);
CREATE INDEX IF NOT EXISTS idx_pkg_tpref_package
