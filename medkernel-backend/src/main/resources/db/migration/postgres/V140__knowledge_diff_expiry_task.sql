-- MedKernel 第二阶段 P2-C · AIK-STD-08 最新知识差异检测与过期治理（PostgreSQL）
-- 新项目基线：只保留当前差异台账与过期复核任务，不做旧数据兼容回填。

CREATE TABLE IF NOT EXISTS mk_knowledge_diff (
...
CREATE INDEX IF NOT EXISTS idx_knowledge_diff_target
    ON mk_knowledge_diff (tenant_id, target_identity_id, detected_at);
CREATE INDEX IF NOT EXISTS idx_knowledge_diff_run
    ON mk_knowledge_diff (tenant_id, run_code);

CREATE TABLE IF NOT EXISTS mk_knowledge_expiry_task (
    id              BIGSERIAL    PRIMARY KEY,
    tenant_id       VARCHAR(64)  NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_expiry_task_status
    ON mk_knowledge_expiry_task (tenant_id, status, review_due_at);
CREATE INDEX IF NOT EXISTS idx_expiry_task_version
