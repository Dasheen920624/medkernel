-- MedKernel v1.0 GA · TERM-01 术语高危近似规则（PostgreSQL）
-- ROLLBACK：确认无依赖该规则生成的 PENDING 高危候选后，删除 mk_term_high_risk_rule 表。

CREATE TABLE IF NOT EXISTS mk_term_high_risk_rule (
    id             BIGSERIAL PRIMARY KEY,
    tenant_id      VARCHAR(64)   NOT NULL,
    rule_code      VARCHAR(64)   NOT NULL,
    rule_type      VARCHAR(32)   NOT NULL,
    category       VARCHAR(32)   NULL,
    left_terms     VARCHAR(1024) NULL,
    right_terms    VARCHAR(1024) NULL,
    unit_terms     VARCHAR(512)  NULL,
    scale_ratio    DOUBLE PRECISION NULL,
...
CREATE INDEX IF NOT EXISTS idx_mk_term_high_risk_rule_tenant_status ON mk_term_high_risk_rule (tenant_id, status);
CREATE INDEX IF NOT EXISTS idx_mk_term_high_risk_rule_category ON mk_term_high_risk_rule (tenant_id, category, status);
...
VALUES
    ('SYSTEM', 'MED-C1-TROPONIN-TI', 'MUTUALLY_EXCLUSIVE_TERMS', 'LAB',
     '肌钙蛋白t|ctnt|troponint|tnt', '肌钙蛋白i|ctni|troponini|tni', NULL, NULL, '肌钙蛋白 T/I 高危近似'),
    ('SYSTEM', 'MED-C1-K-NA', 'MUTUALLY_EXCLUSIVE_TERMS', 'DRUG',
     '钾|k|k+|potassium|氯化钾|kcl', '钠|na|na+|sodium|氯化钠|nacl', NULL, NULL, '钾/钠高危近似'),
    ('SYSTEM', 'MED-C1-LEFT-RIGHT', 'MUTUALLY_EXCLUSIVE_TERMS', NULL,
     '左|left', '右|right', NULL, NULL, '左/右部位高危近似'),
    ('SYSTEM', 'MED-C1-DOSE-10X', 'DOSE_MAGNITUDE', 'DRUG',
     NULL, NULL, 'mg|毫克', 10, '剂量量级 10 倍高危近似'),
    ('SYSTEM', 'MED-C1-INSULIN-UML', 'UNIT_STRENGTH', 'DRUG',
