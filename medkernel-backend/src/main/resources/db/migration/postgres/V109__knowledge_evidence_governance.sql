-- MedKernel v1.0 GA · 知识循证与复审治理（PostgreSQL）
-- ROLLBACK：删除复审到期索引、周期约束及 knowledge_asset_version 新增字段。

ALTER TABLE knowledge_asset_version ADD COLUMN IF NOT EXISTS review_cycle_months INTEGER DEFAULT 12 NOT NULL;
ALTER TABLE knowledge_asset_version ADD COLUMN IF NOT EXISTS next_review_at TIMESTAMPTZ NULL;
...
SET next_review_at = COALESCE(reviewed_at, activated_at, updated_at)
    + review_cycle_months * INTERVAL '1 month'
WHERE status = 'ACTIVE'
  AND next_review_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_knowledge_av_review_due
...
ALTER TABLE knowledge_identity DROP CONSTRAINT IF EXISTS ck_knowledge_identity_status;
...
ALTER TABLE knowledge_supersession ADD COLUMN IF NOT EXISTS successor_identity_id BIGINT NULL;
ALTER TABLE knowledge_supersession ADD COLUMN IF NOT EXISTS grace_period_end TIMESTAMPTZ NULL;
ALTER TABLE knowledge_supersession ADD COLUMN IF NOT EXISTS migration_guidance VARCHAR(1000) NULL;
ALTER TABLE knowledge_supersession DROP CONSTRAINT IF EXISTS ck_knowledge_supersession_type;
...
CREATE INDEX IF NOT EXISTS idx_supersession_successor
    ON knowledge_supersession (tenant_id, successor_identity_id);
CREATE INDEX IF NOT EXISTS idx_supersession_grace
