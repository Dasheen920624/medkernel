-- MedKernel v1.0 GA · H-6 引擎领域事件统一协同来源（PostgreSQL）
-- ROLLBACK：如需回滚，先确认无 RULE_EVENT / PATHWAY_EVENT 待办通知与新质控告警，或导出审计证据后删除相关行，再恢复旧约束。

ALTER TABLE mk_engine_workflow_todo DROP CONSTRAINT IF EXISTS ck_workflow_todo_source_type;
...
ALTER TABLE mk_quality_dashboard_alert DROP CONSTRAINT IF EXISTS ck_quality_dashboard_alert_type;
