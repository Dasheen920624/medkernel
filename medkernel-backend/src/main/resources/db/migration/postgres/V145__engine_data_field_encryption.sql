-- MedKernel 第二阶段 P6-T6.4 · DATASVC-01 D3/D4 字段级加密与分级元数据（PostgreSQL）
...
CREATE INDEX IF NOT EXISTS idx_mk_engine_data_encrypted_field_scope
    ON mk_engine_data_encrypted_field (tenant_id, scope_key);
CREATE INDEX IF NOT EXISTS idx_mk_engine_data_encrypted_field_hash
    ON mk_engine_data_encrypted_field (tenant_id, search_hash);
CREATE INDEX IF NOT EXISTS idx_mk_engine_data_field_policy_level
    ON mk_engine_data_field_policy (tenant_id, data_level, status);
CREATE INDEX IF NOT EXISTS idx_mk_engine_data_field_policy_status
