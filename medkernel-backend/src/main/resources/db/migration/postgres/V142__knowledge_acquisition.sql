-- MedKernel 第二阶段 P2-C · AIK-STD-14 公域资料获取（PostgreSQL）
...
CREATE INDEX IF NOT EXISTS idx_mk_knowledge_acquisition_source_domain
    ON mk_knowledge_acquisition_source (tenant_id, domain);
CREATE INDEX IF NOT EXISTS idx_mk_knowledge_acquisition_source_status
    ON mk_knowledge_acquisition_source (tenant_id, enabled_flag, license_policy, robots_policy);
CREATE INDEX IF NOT EXISTS idx_mk_knowledge_acquisition_run_status
    ON mk_knowledge_acquisition_run (tenant_id, status, created_at);
CREATE INDEX IF NOT EXISTS idx_mk_knowledge_acquisition_run_source
