-- MedKernel v1.0 GA · SYS-04 不可变资产版本框架（PostgreSQL）
-- ROLLBACK：确认无资产引擎接入 mk_version_asset_version 后，删除该表及索引。

CREATE TABLE IF NOT EXISTS mk_version_asset_version (
    ...
    CONSTRAINT ck_mk_version_asset_version_hash CHECK (content_hash ~ '^[0-9a-f]{64}$')
);

CREATE INDEX IF NOT EXISTS idx_mk_version_asset_version_tenant_status
    ON mk_version_asset_version (tenant_id, status, updated_at);
CREATE INDEX IF NOT EXISTS idx_mk_version_asset_version_identity
    ON mk_version_asset_version (tenant_id, asset_type, asset_identity);
CREATE INDEX IF NOT EXISTS idx_mk_version_asset_version_active_scope
