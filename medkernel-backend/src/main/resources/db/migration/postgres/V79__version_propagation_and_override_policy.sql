-- MedKernel v1.0 GA · 平台优先继承底座 P0-1.1：传播语义与覆盖策略护栏（PostgreSQL）
...
ALTER TABLE mk_version_inheritance_override
    ADD COLUMN IF NOT EXISTS propagation VARCHAR(32) NOT NULL DEFAULT 'INHERITABLE';
...
ALTER TABLE mk_version_asset_version
    ADD COLUMN IF NOT EXISTS override_policy VARCHAR(32) NOT NULL DEFAULT 'FREE';
