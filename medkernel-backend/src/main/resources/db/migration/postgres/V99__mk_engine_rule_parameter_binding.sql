-- MedKernel v1.0 GA · P12-4 参数化规则实例参数绑定（PostgreSQL）
-- ROLLBACK：如需回滚，先导出 mk_engine_rule_parameter_binding 审计证据与关联规则版本，再删除本表。

CREATE TABLE IF NOT EXISTS mk_engine_rule_parameter_binding (
...
CREATE INDEX IF NOT EXISTS idx_mk_engine_rule_parameter_binding_version
    ON mk_engine_rule_parameter_binding (tenant_id, rule_version_id);

CREATE INDEX IF NOT EXISTS idx_mk_engine_rule_parameter_binding_key
