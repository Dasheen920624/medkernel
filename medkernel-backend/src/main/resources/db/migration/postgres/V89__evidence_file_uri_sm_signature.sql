-- MedKernel v1.0 GA · EVID-01 证据真实文件与国密签名（PostgreSQL）
...
ALTER TABLE evidence_snapshot ADD COLUMN IF NOT EXISTS file_uri VARCHAR(512);
ALTER TABLE evidence_snapshot ADD COLUMN IF NOT EXISTS file_digest VARCHAR(128);
ALTER TABLE evidence_snapshot ADD COLUMN IF NOT EXISTS signature_algorithm VARCHAR(32);
ALTER TABLE evidence_snapshot ADD COLUMN IF NOT EXISTS signature_value VARCHAR(2048);
ALTER TABLE evidence_snapshot ADD COLUMN IF NOT EXISTS signer_public_key VARCHAR(2048);
