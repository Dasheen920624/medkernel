-- MedKernel v1.0 GA · 受限平台包租户授权（PostgreSQL）
-- ROLLBACK：删除 mk_pkg_package_entitlement，移除 knowledge_package.access_policy。

ALTER TABLE knowledge_package
    ADD COLUMN IF NOT EXISTS access_policy VARCHAR(32) DEFAULT 'OPEN' NOT NULL;
ALTER TABLE knowledge_package
    ADD CONSTRAINT ck_knowledge_package_access_policy
    CHECK (access_policy IN ('OPEN','ENTITLED'));

CREATE TABLE IF NOT EXISTS mk_pkg_package_entitlement (
    id                  BIGINT       GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    entitlement_id      VARCHAR(64)  NOT NULL,
    tenant_id           VARCHAR(64)  NOT NULL,
    platform_tenant_id  VARCHAR(64)  NOT NULL,
    platform_package_id VARCHAR(64)  NOT NULL,
    package_identity    VARCHAR(256) NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_package_entitlement_package_status
    ON mk_pkg_package_entitlement (platform_package_id, status, expires_at);
CREATE INDEX IF NOT EXISTS idx_package_entitlement_tenant_status
