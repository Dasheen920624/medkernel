...
CREATE TABLE mk_knowledge_customization (
    ...
    CONSTRAINT uk_knowledge_customization_scope UNIQUE
        (tenant_id, platform_identity_id, target_org_unit_id, applicable_scope),
    CONSTRAINT ck_knowledge_customization_source CHECK
        (source_type IN ('LOCAL_CUSTOMIZATION')),
    CONSTRAINT ck_knowledge_customization_status CHECK
        (status IN ('DRAFT','ACTIVE','RESTORED')),
...
CREATE TABLE mk_identity_person_appointment (
    ...
    CONSTRAINT ck_person_appointment_type CHECK
        (appointment_type IN ('INTERNAL','GROUP_SHARED','EXTERNAL_COLLABORATOR','IMPLEMENTATION')),
...
CREATE TABLE mk_identity_person_import_job (
    ...
    CONSTRAINT ck_person_import_status CHECK
        (status IN ('VALIDATING','HAS_ISSUES','READY','PROCESSING','PARTIAL','COMPLETED','CANCELLED')),
