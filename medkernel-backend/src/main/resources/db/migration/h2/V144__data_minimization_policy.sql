-- MedKernel 第二阶段 P5-T5.5 · OPT-09 数据最小化策略引擎（H2）
-- ROLLBACK：确认无出域策略依赖后删除 mk_llm_egress_whitelist 的策略扩展列与约束。

ALTER TABLE mk_llm_egress_whitelist
    ADD COLUMN IF NOT EXISTS desensitization_rules VARCHAR(2048) DEFAULT '{}' NOT NULL;

ALTER TABLE mk_llm_egress_whitelist
    ADD COLUMN IF NOT EXISTS approval_threshold_level VARCHAR(16) DEFAULT 'HIGH' NOT NULL;

ALTER TABLE mk_llm_egress_whitelist
    ADD COLUMN IF NOT EXISTS guardrail_locked_flag CHAR(1) DEFAULT 'Y' NOT NULL;
