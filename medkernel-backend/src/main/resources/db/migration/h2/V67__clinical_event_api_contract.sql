-- MedKernel v1.0 GA · API-02 临床事件客户面契约补强（H2 PostgreSQL 兼容模式）

ALTER TABLE clinical_event ADD COLUMN IF NOT EXISTS trigger_point VARCHAR(64) NULL;
ALTER TABLE clinical_event ADD COLUMN IF NOT EXISTS idempotency_key VARCHAR(128) NULL;
ALTER TABLE clinical_event ADD COLUMN IF NOT EXISTS callback_webhook_id VARCHAR(64) NULL;
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_clinical_event_idempotency
    ON clinical_event (tenant_id, idempotency_key);
CREATE INDEX IF NOT EXISTS idx_clinical_event_trigger
    ON clinical_event (tenant_id, trigger_point, received_at);
CREATE INDEX IF NOT EXISTS idx_clinical_event_callback
