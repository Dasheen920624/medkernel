-- MedKernel 全真沙盘 · CURRENT 运行绑定与不可变基线（H2）
-- 场景模板不保存固定包版本；每次运行只能从演练机构唯一 ACTIVE 绑定解析并冻结。

CREATE TABLE IF NOT EXISTS mk_sandbox_runtime_binding (
    ...
    active_scope_key         VARCHAR(96)  NULL,
...
CREATE TABLE IF NOT EXISTS mk_sandbox_run (
    id                           BIGINT        GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_mk_sandbox_runtime_binding_tenant
    ON mk_sandbox_runtime_binding (tenant_id, status, activated_at);
CREATE INDEX IF NOT EXISTS idx_mk_sandbox_run_tenant_status
    ON mk_sandbox_run (tenant_id, status, started_at);
CREATE INDEX IF NOT EXISTS idx_mk_sandbox_run_scenario
