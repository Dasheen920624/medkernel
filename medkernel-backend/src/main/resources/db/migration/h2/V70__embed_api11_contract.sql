-- MedKernel v1.0 GA · API-11 嵌入 API 契约补强（H2 PostgreSQL 兼容模式）
...
ALTER TABLE embed_launch_token ADD COLUMN IF NOT EXISTS integration_mode VARCHAR(16) NOT NULL DEFAULT 'IFRAME';
ALTER TABLE embed_launch_token ADD COLUMN IF NOT EXISTS hook VARCHAR(64) NULL;
ALTER TABLE embed_launch_token ADD COLUMN IF NOT EXISTS hook_instance VARCHAR(128) NULL;
ALTER TABLE embed_launch_token ADD COLUMN IF NOT EXISTS consumed_at TIMESTAMP NULL;

CREATE INDEX IF NOT EXISTS idx_embed_token_status_expired
    ON embed_launch_token (tenant_id, status, expired_at);

CREATE INDEX IF NOT EXISTS idx_embed_token_hook
