-- MedKernel v1.0 GA · SYS-08 知识完整适用域唯一约束（Oracle）
...
ALTER TABLE knowledge_asset_version ADD (
    organization_scope VARCHAR2(256) NULL,
    applicable_scope VARCHAR2(256) NULL,
    active_scope_key VARCHAR2(768) NULL
);
...
SET active_scope_key = CASE
    WHEN status = 'ACTIVE' THEN TO_CHAR(identity_id) || '|' || organization_scope || '|' || applicable_scope
    ELSE 'version:' || TO_CHAR(id)
END
WHERE active_scope_key IS NULL;

ALTER TABLE knowledge_asset_version MODIFY (
    organization_scope NOT NULL,
    applicable_scope NOT NULL,
    active_scope_key NOT NULL
);
...
CREATE INDEX idx_knowledge_av_effective_scope
