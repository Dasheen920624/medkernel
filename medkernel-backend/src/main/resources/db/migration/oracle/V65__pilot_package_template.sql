-- MedKernel v1.0 GA · SVC-PILOT-03 试点首发配置包模板（Oracle）

CREATE TABLE mk_pkg_pilot_package_template (
    template_id             VARCHAR2(64)   PRIMARY KEY,
    tenant_id               VARCHAR2(64)   NOT NULL,
    template_code           VARCHAR2(128)  NOT NULL,
    name                    VARCHAR2(256)  NOT NULL,
    description             VARCHAR2(1000) NULL,
    package_code_prefix     VARCHAR2(128)  NOT NULL,
    default_package_version VARCHAR2(64)   NOT NULL,
...
CREATE INDEX idx_pkg_tpl_tenant_status
    ON mk_pkg_pilot_package_template (tenant_id, status, template_code);

CREATE TABLE mk_pkg_pilot_template_item (
    ...
    dependency_note  VARCHAR2(512)  NULL,
    created_at       TIMESTAMP WITH TIME ZONE NOT NULL,
    created_by       VARCHAR2(64)   NOT NULL,
    updated_at       TIMESTAMP WITH TIME ZONE NOT NULL,
    updated_by       VARCHAR2(64)   NOT NULL,
    ...
    CONSTRAINT ck_pkg_tpli_required CHECK (required_flag IN (0,1)),
    CONSTRAINT ck_pkg_tpli_sort CHECK (sort_order >= 0)
);

CREATE INDEX idx_pkg_tpli_template
