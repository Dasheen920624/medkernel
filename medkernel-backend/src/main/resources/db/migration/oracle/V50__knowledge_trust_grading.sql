-- MedKernel v1.0 GA · KNOW-01/OPT-07 可信分级与冲突仲裁（Oracle）
-- ROLLBACK：若需回滚，先确认无 A-E 分级与 GRADE 字段被新版发布链使用，再删除新增约束 / 索引 / 字段并恢复旧 ck_source_document_authority。

ALTER TABLE source_document ADD authority_basis VARCHAR2(1024) NULL;
...
ALTER TABLE knowledge_asset_version ADD (
    authority_level VARCHAR2(32) NULL,
    grade_quality VARCHAR2(16) NULL,
    grade_strength VARCHAR2(16) NULL,
    conflict_arbitration VARCHAR2(2048) NULL
);
