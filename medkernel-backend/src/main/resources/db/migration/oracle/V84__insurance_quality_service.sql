-- MedKernel v1.0 GA · SVC-QUALITY-02 病案医保服务包（Oracle）
...
CREATE TABLE mk_quality_case_review (
    ...
    model_downgrade_reason  VARCHAR2(128)  NULL,
...
CREATE INDEX idx_quality_case_review_tenant_status
    ON mk_quality_case_review (tenant_id, review_status, created_at);
CREATE INDEX idx_quality_case_review_department
...
CREATE INDEX idx_quality_drg_grouping_tenant_status
    ON mk_quality_drg_grouping (tenant_id, grouping_status, created_at);
CREATE INDEX idx_quality_drg_grouping_department
...
CREATE TABLE mk_quality_insurance_issue (
    ...
    claim_amount            NUMBER(18,2)   NULL,
    threshold_amount        NUMBER(18,2)   NULL,
    evidence_summary        CLOB           NOT NULL,
    evaluation_run_id       VARCHAR2(64)   NULL,
    finding_id              VARCHAR2(64)   NULL,
    created_at              TIMESTAMP WITH TIME ZONE DEFAULT SYSTIMESTAMP NOT NULL,
    created_by              VARCHAR2(64)   DEFAULT 'system' NOT NULL,
    updated_at              TIMESTAMP WITH TIME ZONE DEFAULT SYSTIMESTAMP NOT NULL,
    updated_by              VARCHAR2(64)   DEFAULT 'system' NOT NULL,
...
CREATE INDEX idx_quality_insurance_issue_tenant_status
    ON mk_quality_insurance_issue (tenant_id, status, severity, created_at);
CREATE INDEX idx_quality_insurance_issue_department
    ON mk_quality_insurance_issue (tenant_id, department_id, status);
CREATE INDEX idx_quality_insurance_issue_claim
