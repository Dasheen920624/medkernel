-- MedKernel v1.0 GA · SYS-04 发布流、激活事务与历史重放（Oracle）
...
CREATE TABLE mk_version_activation_transaction (
    ...
    impact_digest      VARCHAR2(1024) NULL,
    evidence_summary   VARCHAR2(2048) NOT NULL,
    created_at         TIMESTAMP WITH TIME ZONE DEFAULT SYSTIMESTAMP NOT NULL,
    created_by         VARCHAR2(64)   NOT NULL,
    updated_at         TIMESTAMP WITH TIME ZONE DEFAULT SYSTIMESTAMP NOT NULL,
...
CREATE TABLE mk_version_replay_binding (
    ...
    CONSTRAINT ck_mk_version_replay_binding_type CHECK (asset_type IN
        ('KNOWLEDGE','TERMINOLOGY','RULE','PATHWAY','PACKAGE','EVALUATION')),
    CONSTRAINT ck_mk_version_replay_binding_hash CHECK (REGEXP_LIKE(result_hash, '^[0-9a-f]{64}$'))
);

CREATE INDEX idx_mk_version_release_plan_asset
    ON mk_version_release_plan (tenant_id, asset_type, asset_identity, status);
CREATE INDEX idx_mk_version_release_plan_version
    ON mk_version_release_plan (tenant_id, version_id, status);
CREATE INDEX idx_mk_version_activation_transaction_asset
    ON mk_version_activation_transaction (tenant_id, asset_type, asset_identity, created_at);
CREATE INDEX idx_mk_version_replay_binding_version
