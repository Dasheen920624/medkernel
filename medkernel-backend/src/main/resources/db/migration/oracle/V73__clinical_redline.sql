-- MedKernel v1.0 GA · OPT-04 临床安全红线规则库 PR1（Oracle）
...
CREATE TABLE mk_engine_clinical_redline (
    ...
    active_scope_key                VARCHAR2(512)  NULL,
    ...
    CONSTRAINT ck_clinical_redline_override CHECK (lower_tenant_override_allowed IN (0, 1))
);

CREATE INDEX idx_clinical_redline_active
    ON mk_engine_clinical_redline (tenant_id, status, category, trigger_point, updated_at);
CREATE INDEX idx_clinical_redline_category
    ON mk_engine_clinical_redline (tenant_id, category, status, redline_key);
CREATE INDEX idx_clinical_redline_source
...
COMMENT ON COLUMN mk_engine_clinical_redline.lower_tenant_override_allowed IS '下级租户是否允许关闭；安全红线默认 0/FALSE';
