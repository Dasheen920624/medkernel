-- MedKernel v1.0 GA · API-07 推荐/CDSS 客户面契约补强（Oracle）
...
ALTER TABLE recommendation_feedback ADD idempotency_key VARCHAR2(128) NULL;

CREATE UNIQUE INDEX uk_rec_feedback_idempotency
    ON recommendation_feedback (tenant_id, card_id, idempotency_key);

ALTER TABLE recommendation_fatigue_signal DROP CONSTRAINT ck_rec_fatigue_signal;
