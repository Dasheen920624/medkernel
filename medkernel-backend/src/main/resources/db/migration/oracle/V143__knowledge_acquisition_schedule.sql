-- MedKernel 第二阶段 P2-C · AIK-STD-14 公域资料自动获取调度（Oracle）
-- 新项目基线：仅声明正式调度字段，不做旧来源兼容回填。

ALTER TABLE mk_knowledge_acquisition_source
    ADD schedule_enabled_flag CHAR(1) DEFAULT 'N' NOT NULL;
ALTER TABLE mk_knowledge_acquisition_source
    ADD schedule_interval_minutes NUMBER(10) NULL;
ALTER TABLE mk_knowledge_acquisition_source
    ADD next_check_at TIMESTAMP NULL;
ALTER TABLE mk_knowledge_acquisition_source
    ADD last_check_at TIMESTAMP NULL;
ALTER TABLE mk_knowledge_acquisition_source
    ADD default_format VARCHAR2(24) NULL;
ALTER TABLE mk_knowledge_acquisition_source
    ADD generation_plan_json CLOB NULL;
