-- MedKernel v1.0 GA · SYS-06 敏感数据导出审批（Oracle）
-- ROLLBACK：确认没有服务依赖 SYS-06 导出审批框架后，删除 mk_compliance_export_approval 表。

CREATE TABLE mk_compliance_export_approval (
    ...
    request_reason             VARCHAR2(512) NOT NULL,
    requested_by               VARCHAR2(64)  NOT NULL,
    requested_at               TIMESTAMP WITH TIME ZONE DEFAULT SYSTIMESTAMP NOT NULL,
    status                     VARCHAR2(32)  DEFAULT 'REQUESTED' NOT NULL,
    reviewer_id                VARCHAR2(64)  NULL,
    review_decision            VARCHAR2(32)  NULL,
    review_comment             VARCHAR2(512) NULL,
    reviewed_at                TIMESTAMP WITH TIME ZONE NULL,
    export_uri                 VARCHAR2(512) NULL,
    export_digest              VARCHAR2(128) NULL,
    approval_evidence_id       VARCHAR2(64)  NULL,
    approval_evidence_file_uri VARCHAR2(512) NULL,
    export_evidence_id         VARCHAR2(64)  NULL,
    export_evidence_file_uri   VARCHAR2(512) NULL,
...
CREATE INDEX idx_compliance_export_approval_status
    ON mk_compliance_export_approval (tenant_id, status, requested_at);
CREATE INDEX idx_compliance_export_approval_resource
    ON mk_compliance_export_approval (tenant_id, resource_type, status);
CREATE INDEX idx_compliance_export_approval_evidence
