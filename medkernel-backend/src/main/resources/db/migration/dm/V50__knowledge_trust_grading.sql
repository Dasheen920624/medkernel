...
ALTER TABLE knowledge_asset_version ADD authority_level VARCHAR(32) NULL;
ALTER TABLE knowledge_asset_version ADD grade_quality VARCHAR(16) NULL;
ALTER TABLE knowledge_asset_version ADD grade_strength VARCHAR(16) NULL;
ALTER TABLE knowledge_asset_version ADD conflict_arbitration VARCHAR(2048) NULL;

UPDATE knowledge_asset_version kav
SET authority_level = (
    SELECT sd.authority_level
    FROM source_document sd
    WHERE sd.id = kav.source_document_id
)
WHERE kav.authority_level IS NULL
  AND kav.source_document_id IS NOT NULL
  AND EXISTS (SELECT 1 FROM source_document sd WHERE sd.id = kav.source_document_id);
