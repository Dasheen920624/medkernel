...
| 知识生产 | Knowledge Production | 承载来源、解析、模型生成、安全评测、影子验证和发布 |
...
| 证据详情 | Evidence Details | 页内按权限展开的低频证据、诊断信息和追溯字段，不形成独立角色或产品空间 |
...
| 安全门 | Safety Gate | 来源引用、结构校验、医学回归、红线测试、重复检测和影响分析 |
...
| 客户用语 | 技术用语 | 含义 |
|---|---|---|
| 服务机构 | Tenant | 客户集团或医院的数据、组织、配置和审计隔离边界 |
