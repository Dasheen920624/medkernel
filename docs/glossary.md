...
| 平台主租户（唯一内置） | 默认租户 / 集团总院 / medkernel-basic | 唯一内置平台数据租户，ID 为 `t-1`；全局医疗知识和标准包默认在这里生成，客户集团 / 医院租户从这里分配或派生 |
| 平台源副本 | 客户主源 / 二级主源 | 从平台主租户同步或离线下发到客户 / 集团租户的知识副本；客户可在本租户覆盖、调整或新增，不得反写平台主源 |
| 客户租户 | 平台租户 / 默认租户 | 客户集团、医院、院区等业务租户；从平台主源接收知识副本，可承载本院定制和覆盖，但不得回写污染平台主租户的知识源 |
