...
| BASE-06 | 七业务域菜单与前端骨架 | 路由元数据 + PageShell + 六态 + 状态机 Badge + 7 步流组件 + 命令面板 | 4d | done |
...
| SYS-03 | 关系库权威源与投影同步 | 图谱/Dify 投影可关闭可重建可审计可降级 | 4d | done |
| SYS-05 | 在线/异步/批量/离线运行框架 | 四类运行模式 + 故障重试 | 4d | done |
...
| ID | 任务 | 范围 | 工作量 | 状态 |
|---|---|---|---:|---|
| INFRA-09 | StepFlowDemo 处理 | 演示页从生产路由移除 + 保留 StepFlow 组件（供 D2 复用）+ 门禁防演示页回流 | 0.5d | done |
| WORKBENCH-01 | 工作台页面真实化（原 `D1-PAGE-工作台`）| 角色默认视图 + 租户生命周期面板（宪法 §6.4）+ 多维治理切片 + 本周建议动作 + 6 主卡片复用现有 API（不新增独立工作台接口）；每页必交见质量基线 | 4d | done |
| WORKBENCH-02 | 验收自检页真实化 | 运行验收自检（就绪/阻塞/未启用据实计数）+ 阻塞可定位去处 + 合并 Tab + 复用现有健康/引擎状态 API；每页必交见质量基线 | 2d | done |
| **D1-验收** | **工作台域级验收** | 各角色首屏真实可看 + 验收自检可跑 | — | done |
...
| ID | 任务 | 范围 | 工作量 | 状态 |
|---|---|---|---:|---|
| API-01 | 标准上下文 API | 患者/就诊/诊断/医嘱/报告/组织/包版本快照 + 字段目录/字段契约索引 | 3d | done |
| API-03 | 知识资产 API | 来源/解析/引用/版本/审核/替换/历史重放/分页/筛选/异步导出 | 4d | done |
| API-04 | 字典映射 API | 标准/院内字典/候选/冲突/发布 | 3d | done |
| API-05 | 规则引擎 API | 定义/测试/影响分析/发布/执行/解释/影子统计反馈 | 3d | done |
| API-06 | 路径引擎 API | 模板/路径知识包/患者路径/节点推进/变异/关键时钟 | 3d | done |
| API-10 | 包发布 API | 知识包/配置包/校验/灰度/全量/同步/回滚 | 3d | done |
| KNOW-01 | 知识资产引擎 | 来源登记/解析/hash/引用锚点/可信分级；图投影（关系库权威，Neo4j 仅查询投影可重建，无图降级）| 6d | done |
| KNOW-02 | 知识版本引擎 | 新旧识别/去重/冲突/待审/原子替换主链路/旧版隔离（详规 §8.13；完整影响病例任务归 SYS-08）| 5d | done |
| TERM-01 | 字典映射引擎（含 MED-C1）| ICD-10/ICD-9-CM-3/药品本位码/LOINC 映射 + 医学语义匹配 + 高危近似负样本判别器（钾/钠、肌钙蛋白T/I、左/右、剂量量级强制 HIGH，禁批量/禁自动确认）| 6d | done |
| RULE-01 | 规则引擎（后端 + 三层前端）| L1 模板 + L2 可视化条件树 + L3 DSL + 分级动作卡 + 优先级/抑制/持久化去重/静态冲突/人工继续留痕 + 人群/组织/场景/生效期/稳定灰度适用域 + 八阶段治理/高危多签/影子先行/影子统计反馈/历史回测/漂移监测/退役封存 + 仿真（病例选择器）+ 测试病例（阳/阴/边界/冲突）+ 消费字段目录/字段契约表达式 + authoring 能力开关/租户覆盖灰度 | 12d | done |
| PATH-01 | 路径引擎（后端 + 三层前端）| L1 模板 + L2 React Flow 节点画布 + L3 DSL + 多级模板继承差异合并 + 7 步流 + 单快照/队列/时光机仿真 + 关键时钟绑定质控 + RACI 角色/统一待办工作清单 + 变异分类/原因码/责任角色/再入径或终止 + 结局指标绑定 + 多路径协调提示 + 随访接续 + 守卫消费字段目录/字段契约表达式 + 路径富节点能力开关/租户覆盖灰度 | 16d | done |
...
| INTEG-02 | 第三方接口文档与契约模板 | 接入概览 + OpenAPI + `/engine/integration/data-contract` 字段契约 + 字段映射 + 鉴权 + 幂等 + 回调 + 降级 + 审计 | 3d | done |
...
| SVC-PILOT-01 | 租户与组织服务包 | TenantOnboarding + ImplementationGuide + 组织树（核心 §9：租户根/集团/医院/院区/社区服务点/科室）+ 专病横切适用域 | 4d | done |
...
| SVC-CLINICAL-03 | 临床协同服务包 | 待办/通知/护理/报告解读/床旁知识/随访触发 | 4d | done |
| PMI-01 / PPATH-01 / REMIND-01 / RULECHK-01 / TODO-01 / NOTIFY-01 / FUP-01（+ 规则库/路径配置已于 D2 含页）| 7 页面真实化 | 每页：六态 + 1 主按钮 ≤3 默认筛选 + 五维 RBAC + 低打扰 + B0（确定性 CDSS）+ E2E；每页必交见质量基线 | — | done |
| **D3-验收** | **临床运行域级验收** | 建患者 → 入径 → 节点推进 → 确定性提醒 → 采纳/拒绝带原因 → 待办闭环，全 B0 真实可跑；证据见 [D3 临床运行域级验收报告](audit/D3-domain-acceptance.md) | — | done |
...
> 🗂️ 本域 **11 张施工卡已建**（5 ID + 6 页面，全建）：[D5 域简报](cards/D5/_brief.md) + [卡索引 · D5 全卡目录](cards/_index.md)；已建卡 ID = 卡名（`cards/D5/<ID>.md`），合同级功能/契约/11 视角/验收以卡为准。页面卡 ID 由占位 `D5-PAGE-*` 实化（USERS-01 用户管理 / IDBIND-01 身份绑定 / AUDITLOG-01 审计日志 / SECBASE-01 安全基线与系统配置 / PROVIDER-01 运行状态 / NOTIFSET-01 通知设置）。下表 ID/范围以卡为准。
...
| SVC-COMPLIANCE-01 | 身份安全服务包 | 用户 + 身份绑定 + 数据权限 + 租户隔离 + 安全基线 | 5d | done |
| SVC-COMPLIANCE-02 | 审计运维服务包 | 审计 + 证据包 + 依赖运行状态 + 备份恢复就绪与演练证据 + 离线运行就绪 | 4d | done |
| USERS-01 / IDBIND-01 / AUDITLOG-01 / SECBASE-01 / PROVIDER-01 / NOTIFSET-01 | 6 页面真实化 | 每页：五维权限管理粒度 + 审计可查 + 六态 + Provider 无连接诚实显示（NOT_CONNECTED）；**「安全基线与系统配置」承载系统配置中心**（[CONFIG-01](cards/D0/CONFIG-01.md) 前台：功能开关/认证/备份/国产化/Provider/日志级别，高危护栏置灰不可关，不净增二级菜单）；每页必交见质量基线 | — | done |
| **D5-验收** | **合规运维域级验收** | 管权限 → 改身份绑定 → 查审计 → 导出证据包，全 B0 真实可跑；Provider 无连接诚实降级 | — | done |

> 2026-06-07 收口：线1统一承接全部任务，线2/线3/线4不再独立施工；历史 `done` 仅表示曾交付，统一主流程复验合格后才恢复完成状态。
...
> 目标：专业能力 B0 真实，按普通功能归入知识治理或系统运维。AI 工作流编排留第二波。
...
| ID | 任务 | 范围 | 工作量 | 状态 |
|---|---|---|---:|---|
| OPT-10 | 插件和生态安全边界 | 插件只读/受控写入 + 权限/审计/数据主权/临床安全门禁 | 3d | done |
| DOMCHK-01 / DEVCON-01 | 国产化自检 / 开发者控制台 | 真实自检报告 + API目录/Trace/插件管理；桌面与移动端人工可用 | — | done |
| PROV-01 | 来源追溯 | 关系库权威身份、版本、来源文献、精确锚点统一追溯；支持 `identityId` 直达 | 3d | done |
| GRAPH-01 | 图谱查询 | 关系库权威源的可重建投影；服务端分页、租户隔离、一致性核查与权限下钻 | 3d | done |
| AIFLOW-01 | AI工作流 | 统一真实能力目录、路由策略与运行状态；编排执行仍归wave2 | 3d | done |
| **D6-验收** | **跨域专业能力验收** | 来源可追溯 + 图谱投影可查 + AI能力状态诚实 + 国产化自检可跑 + 诊断工具可用 | — | done |
...
| AIK-STD-14 | 第三方 Agent 生产接入协议（Claude Code/Codex 跑生产中心，经 MCP/CLI，沙箱无患者数据，人在环）| 5d | pending |

### 引擎数据服务层 + CLI + MCP（DATASVC，接入底座）🗂️ 1 卡已建（第二阶段回收 2026-05-26 规范）

| ID | 任务 | 工作量 | 状态 |
|---|---|---:|---|
| DATASVC-01 | 引擎数据服务层 + 产品级 CLI + MCP 服务（四入口共用受控合同 + 数据分级 D0-D5 + 字段级加密 + 不绕治理 + 降级矩阵；Agent 生产底座）| 12d | pending |

### 首发知识资产（X-KNOWGEN，28 项）🗂️ 28 卡已建
...
| KNOWGEN-15 | 首发资产总验收（**稳定基础发行版 + 全景分类全覆盖**：01~14、16~28 共 27 类合并“试点医院首发知识包 v1.0”；基础官方发行版全量对账；A1-A9 通过；OPT-04 红线生效；PKG-01 同步）| 5d | pending |
...
| KNOWGEN-19 | 特殊人群剂量 + 药物基因组 PGx（肾/肝/儿童/老年/妊娠 + CPIC，高危剂量双签）| 8d | pending |
| KNOWGEN-20 | 18 项医疗核心制度核查（三查七对/手术安全核查/输血/危急值报告…，关键制度红线提醒）| 6d | pending |
| KNOWGEN-21 | 罕见病知识库（国家目录识别线索 + 诊断标准 + 转诊，防漏诊候选）| 6d | pending |
| KNOWGEN-22 | 急救与生命支持（CPR/ACLS/休克/过敏/中毒解毒 + 剂量，确定性断网可用）| 6d | pending |
...
**第二波小计**：68 项（原 55 + AIK-STD-13/14 + DATASVC-01 + KNOWGEN-16~25）
...
3. **AI 工厂**：无模型可运行组（AIK-STD-01~14 + DATASVC-01）通过；审核台真实可审可发；CLI/MCP 不绕治理
...
13. **集团化**：平台→租户→集团→医院→院区→社区→科室 多层继承覆盖与冲突仲裁可运行，专病作为横切适用域参与规则、路径、知识包与发布
...
| 第二波 · AI 加深 | 68 | — |
| GA 总验收 | 12 | — |
| **合计** | **166**（153 基础上，第二阶段设计落卡新增 13 张 wave2 卡；INFRA-06 炸开为各域页面、MED-C1 并入 TERM-01；v8.1 D0 新增 SUPERADMIN-01 + CONFIG-01；v8.2 D0 新增 AUTH-01/02/03 双模认证登录）| **33** |
