...
| API-05 | 规则引擎 API | 定义/测试/影响分析/发布/执行/解释 | 3d | done |
...
| RULE-01 | 规则引擎（后端 + 三层前端）| L1 模板 + L2 可视化条件树 + L3 DSL + 分级动作卡 + 优先级/抑制/持久化去重/静态冲突/人工继续留痕 + 人群/组织/场景/生效期/稳定灰度适用域 + 八阶段治理/高危多签/影子先行/退役封存 + 仿真（病例选择器）+ 测试病例（阳/阴/边界/冲突）+ 消费字段目录/字段契约表达式 | 12d | 复验中 |
