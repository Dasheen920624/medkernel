...
| RULE-01 | 规则引擎（后端 + 三层前端）| L1 模板 + L2 可视化条件树 + L3 DSL + 分级动作卡 + 优先级/抑制/持久化去重/静态冲突/人工继续留痕 + 人群/组织/场景/生效期/稳定灰度适用域 + 八阶段治理/高危多签/影子先行/影子统计反馈/历史回测/漂移监测/退役封存 + 仿真（病例选择器）+ 测试病例（阳/阴/边界/冲突）+ 消费字段目录/字段契约表达式 + authoring 能力开关/租户覆盖灰度 | 12d | 复验中 |
| PATH-01 | 路径引擎（后端 + 三层前端）| L1 模板 + L2 React Flow 节点画布 + L3 DSL + 多级模板继承差异合并 + 7 步流 + 单快照/队列/时光机仿真 + 关键时钟绑定质控 + RACI 角色/统一待办工作清单 + 变异分类/原因码/责任角色/再入径或终止 + 结局指标绑定 + 多路径协调提示 + 随访接续 + 守卫消费字段目录/字段契约表达式 + 路径富节点能力开关/租户覆盖灰度 | 16d | 复验中 |
