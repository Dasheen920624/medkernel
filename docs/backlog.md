...
| RULE-01 | 规则引擎（后端 + 三层前端）| L1 模板 + L2 可视化条件树 + L3 DSL + 分级动作卡 + 优先级/抑制/持久化去重/静态冲突/人工继续留痕 + 7 步流 + 仿真（病例选择器）+ 测试病例（阳/阴/边界/冲突）+ 消费字段目录/字段契约表达式 | 12d | done |
...
> 2026-06-07 收口：线1统一承接全部任务，线2/线3/线4不再独立施工；D5及D6前承接项已验收，D6继续按单卡证据推进。
