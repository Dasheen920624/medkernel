...
1. ~~映射包灰度/全量发布依赖健康发布适配器；当前机构发布适配器 0（`releaseAdapterCount=0`），须由集成运维员先完成系统接入（幕9 前置），届时回收「灰度（机构知识治理员）→ 全量（机构管理员，配置包页）」跨角色发布链。~~ **已回收（2026-06-13）**：幕9 前置完成系统接入后，发布链「灰度 → 全量」全链真实前台走通，`TERM.P5.MAPPING 2026.06.1` 已 ACTIVE，投递留痕见 [幕9 README](../幕9-系统接入与发布链/README.md)。
...
闭环（2026-06-13）：修复经 PR #578 合并为 `7f69c946` 并部署 134（发布前备份 `p5-7f69c946-predeploy-20260613-091455` 隔离恢复全过，post-deploy manifest/jar 精确匹配），重跑发布链灰度发布成功（`DRAFT → PUBLISHED`），机构管理员全量激活（`PUBLISHED → ACTIVE`），接收端收到两条 `MEDKERNEL_PACKAGE_RELEASE` 真实投递。P5-ACT2-04 双路径（重复版本/窄范围 409）部署复验同轮通过。证据：[幕9 README](../幕9-系统接入与发布链/README.md)。
