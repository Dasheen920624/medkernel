...
### 线 1 · SYS-05 重试死信与离线运行 PR2 🚧

- 类型：软件开发 / 后端运行任务框架
- 分支：`codex/sys-05-retry-dead-letter-pr2`（基于 `origin/main` 2747c4f）
- 目标：按 [SYS-05](cards/D0/SYS-05.md) 大卡工序交付 PR2：`OFFLINE` 离线运行模式、`NOT_CONNECTED` 诚实断连终态、失败任务人工重试、重试耗尽入 `sys_task_dead_letter`、死信人工回放，收口 FR-4/5/6 与 AC-3/4/5。
- 状态：已写计划 `docs/superpowers/plans/2026-06-01-sys-05-retry-dead-letter-pr2.md`；已完成红测并确认缺口为 `RuntimeTaskDeadLetterRepository` / `RuntimeTaskDeadLetterRecord` / `OFFLINE` / `NOT_CONNECTED` / `DEAD_LETTER` / `retryTask` / `replayDeadLetter` / V42 迁移；已实现运行任务 PR2 代码、V42 五方言迁移、服务契约、领域 owner 与文档更新；`mvn -B -q -Dtest=RuntimeTaskServiceTest,RuntimeTaskMigrationContractTest test` 已通过；`mvn -B -q -Dtest=RuntimeTaskServiceTest,RuntimeTaskMigrationContractTest,MigrationBaselineContractTest,H2BaselineMigrationTest,DomainOwnershipContractTest,ServiceContractGovernanceTest,OpenApiContractConfigurationTest test` 已通过，H2 已应用 V1–V42 且重复 migrate 为 0；`mvn -B -q -Dtest=FlywayMultiDialectSmokeTest,MigrationBaselineContractTest,RuntimeTaskMigrationContractTest test` 已通过，PostgreSQL / H2 / Oracle 均迁移至 V42 且重复 migrate 为 0；`mvn -B -q test` 已通过，Surefire `tests=760 failures=0 errors=0 skipped=0`，含 PostgreSQL 15 + Oracle 21 Testcontainers 迁移至 V42；本地 T-GATE 已通过：真实性 20/20 + changed 12 文件、配置边界 2/2 + changed 12 Java 文件、迁移规约 6/6 + changed 5 个 V42 文件、中文注释 0 fail/0 warn、空白检查通过。首次全量暴露 Oracle `ORA-01408` 重复索引问题，已按根因将五方言 `idx_sys_task_dead_task` 改为 `(task_id, tenant_id)`；首次 T-GATE 暴露 `DROP CONSTRAINT IF EXISTS` 被误判为约束名 `IF`，已补门禁测试并修正。
- 下一步（精确到动作/命令）：1. 推送 `codex/sys-05-retry-dead-letter-pr2` 并创建 PR；2. 等远端 CI 全绿后合并；3. 清理分支 / worktree 后从 backlog 领取下一张 D0 闸门卡。
- 相关文件 / 测试 / 坑：`sys_task` 继续是任务权威表，V42 增加 `retry_count` / `max_retries` / `next_attempt_at` / `dead_letter_id` / `replayed_from_task_id`，`sys_task_dead_letter` 保存失败证据并只由人工回放创建新任务；`NOT_CONNECTED` 不能被写成成功。平台级离线许可服务不在本卡实现，按 backlog 归 D5 `SVC-COMPLIANCE-02`；达梦 / 人大金仓真实运行环境仍为 [DEFER-001](audit/deferred-issues.md)，不阻塞当前 PostgreSQL + Oracle 主线。外部环境类阻塞写入 [待处理问题清单](audit/deferred-issues.md) 后继续主线；登录可用、权限隔离、真实性门禁、医疗安全、当前卡主链路缺陷不得延期。
...
> 末次更新：2026-06-01 · SYS-05 PR1 已通过 #226 合入 `origin/main`（merge `2747c4f`）并清理分支 / worktree；当前在 `codex/sys-05-retry-dead-letter-pr2` 推进 SYS-05 PR2，离线运行、重试、死信、回放、`NOT_CONNECTED` 与 V42 五方言迁移已完成红绿实现；聚焦行为测试、迁移合同、H2 V1–V42、领域 owner 和服务契约已通过，下一步跑后端全量、PostgreSQL/Oracle Testcontainers、changed T-GATE 后推送 PR。达梦 / 人大金仓真实环境适配保持 `DEFER-001`，前端依赖审计与非阻断输出噪声登记 `DEFER-002` / `DEFER-003`，本机浏览器截图超时登记 `DEFER-004`；长期目标保持 active，open 待处理问题只登记不阻断主线。
