...
- OpenSpec `tasks.md` 已收敛为 205 个可供后续 AI 顺序执行的原子任务（19 完成、186 待执行）；覆盖缺口总账、首次信任根、签发者密钥隔离、自包含 `.mkp`、16 语义族医疗资源工厂、离线依赖仓、openEuler 空机部署、目标工具本地实现、134 一次确认部署、全量资源生产、医院复制和知识源迁移。
...
1. 在 `codex/launch-ledger` 按 TDD 执行 3.1：先创建失败的 `AuthorityRepositoryContractTest`，再在 `com.medkernel.engine.knowledge.authority` 定义 Authority、IssuerInstance、TrustRoot、SigningKey、Handover、Revocation、PackageRegistration 及 repository 端口；只建权威领域合同，不提前创建 3.2 的表或迁移。
2. 3.1 独立提交并同步本文件后进入 3.2，从唯一 `medkernel.schema.json` 扩展平台知识权威事实并生成五方言 V1；不得手写平行迁移或跳过中文 COMMENT、租户、版本、审计和唯一约束。
3. 按 3.1 起的依赖顺序继续平台知识权威、首次信任根、密钥围栏、`.mkp`、16 语义族资源工厂、离线空机安装、openEuler/容量验证和医院复制；不得绕过依赖跳到 134。
