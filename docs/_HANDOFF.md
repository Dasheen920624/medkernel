...
- 类型：软件开发 / 后端投影同步
- 分支：`codex/sys-03-projection-sync`（基于 `origin/main` 1ac1a9d）
- 目标：按 [SYS-03](cards/D0/SYS-03.md) 大卡工序交付 PR1：关系库权威源 + 图投影重建 + 一致性校验，收口 AC-1 / AC-5；不冒领 PR2 的 Dify 执行器解耦、投影关闭降级和同步审计。
- 状态：已完成 TDD 红绿主实现：新增 `ClinicalGraphProjectionSource` 从 SYS-01 的 12 类 `mk_clinical_*` 关系库权威对象生成临床图节点 / 边投影事实，规范载荷排除患者密文字段；新增 `ProjectionSyncService` / `ProjectionSnapshot` / `ProjectionSync` / V40 `mk_projection_sync` + `mk_projection_snapshot` 五方言迁移，重建时清空当前租户投影快照后由关系库重放，一致性报告可检测 missing / extra / changed；新增 `/api/v1/projections/clinical-graph/rebuild` 与 `/consistency` 端点、`projection.read` / `projection.rebuild` 权限和 SYS-02 服务契约登记；`DomainOwnershipCatalog` 已登记 `engine-projection`，避免新增表无人负责或跨 owner 直写。已通过聚焦测试、H2 迁移 smoke、五方言静态迁移合同、服务契约测试、`DomainOwnershipContractTest` 和后端全量 `mvn -B -q test`（744 tests，PostgreSQL 15 + Oracle 21 Testcontainers 迁移至 V40）；official changed T-GATE 已通过（真实性 30 文件、配置边界 30 文件、迁移 5 文件、中文注释 0 fail / 0 warn、diff check 0）。待推送 PR、CI、合并。
- 下一步（精确到动作/命令）：1. 推送分支并创建 PR；2. 远端 CI 全绿后合并到 `origin/main`，清理分支 / worktree，再领取 SYS-03 PR2。
- 相关文件 / 测试 / 坑：计划见 `docs/superpowers/plans/2026-06-01-sys-03-projection-sync-pr1.md`。核心实现为 `medkernel-backend/src/main/java/com/medkernel/engine/projection/*`、`medkernel-backend/src/test/java/com/medkernel/engine/projection/*`、V40 五方言迁移、`PermissionCode`、`DefaultPermissionPolicy`、`ServiceContractCatalog`、`MigrationBaselineContractTest`、`H2BaselineMigrationTest`。当前真实运行环境只保障 PostgreSQL + Oracle；达梦 / 人大金仓真实环境证据继续登记 `DEFER-001`，不阻塞 PR1。新增外部阻塞必须登记 [待处理问题清单](audit/deferred-issues.md) 后继续主线；若触及当前卡主链路、登录可用、权限隔离、真实性门禁或医疗安全红线，则即时处理。
