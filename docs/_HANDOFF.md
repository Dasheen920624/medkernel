...
- **新主线「全流程演练 · 使用指南 · 体验重构」已进入幕3收口**：总体计划见 [docs/superpowers/plans/2026-06-10-full-flow-drill-usability-program.md](superpowers/plans/2026-06-10-full-flow-drill-usability-program.md)（幕0–10 演练剧本 + 线B指南 + 线C体验重构 + 第二阶段准入六判据）。**状态：计划已合并（#526）；DOC-SYNC 第一阶段已合并（#528，`fbaa012e`）；幕0已合入（#529，`d7cc0e9c`）；幕1已合入（#530，`97a3b217`）；幕2已合入（#531，`882182f6`）；当前分支 `codex/demo-drill-act3-knowledge-governance` 承载幕3「知识治理」前端修复、134 证据、试点准备手册第 4 章、词条和待处理问题登记；本分支验证/PR/CI/合入后继续幕4。**
- **幕3真实执行事实**：134 已在租户 `drill-hospital-20260611` 登记 CAP 指南、国家卫健委临床实验室管理办法、演练医院危急值目录等来源；已发布 `CAP 经验性抗感染推荐`（当前版本 ID 4）和 `血钾危急值阈值与报告时限`（当前版本 ID 6）。
- **幕3护栏事实**：高风险缺电子签名发布被拒绝、无来源引用发布被拒绝、跨来源引用被拒绝、重复内容跳过、引用片段偏移被校验；来源追溯最终按租户级上下文解析，血钾旧租户级版本 `5` 已被新版本 `6` 替代。
- **幕3证据目录**：`docs/release/evidence/v1.0-drill-20260611/幕3-知识治理/`，结构化汇总见 `32-summary.json`；页面证据为 `50-ui-knowledge-governance.png` 和 `51-ui-provenance-potassium.png`；截图链路摘要见 `52-ui-screenshot-check.json`。
- **幕3本分支代码修复**：前端知识治理页会签提交改用 `CandidateClassification.id`，并补 `KnowledgeGovernance.test.tsx` 场景证明候选版本 ID 与分类审核 ID 分叉时不会误传。真实后端端点 `/candidates/{candidateId}/review` 的 path id 是分类审核记录 ID。
- **幕3未通过项已如实登记**：多来源证据链当前不能把不同来源版本的锚点合并到同一知识版本，已登记 `docs/audit/deferred-issues.md#DEFER-022`；客户租户未执行平台级知识身份退役，本幕以版本替代关系证明旧版本不再作为当前权威版本。
...
- **幕2已合并代码修复**：新增 API-04 标准术语 / 院内术语登记接口；新增 V114 五方言迁移，把 SYSTEM 级 `MED-C1-K-NA` 从 `DRUG` 放宽为跨分类安全底线，覆盖 LAB 血钠/血清钾；确认候选后将 `local_term.status` 从 `UNMAPPED` 推进到 `MAPPED`。
...
- **演练凭据事实**：幕1凭据仍只存服务器 `/zoesoft/medkernel/conf/drill-act1-credentials-20260611.json`（`medkernel:medkernel 600`），仓库只提交路径、权限和页面证据；幕2 API 证据由信息科账号执行，幕3会签证据由医院管理员与医务处账号执行，UI 截图由已完成首登的医院管理员账号执行。
...
- PR #524 验证：后端 **2188 项 0 失败 0 错误**（5 项本机无 Docker 跳过：2 既有方言 smoke + 3 新空库 smoke，CI 真跑）；T-GATE（真实性 / 配置边界 / 迁移规约 / 中文注释）changed 全绿。当前 `codex/demo-drill-act3-knowledge-governance` 的验证以本分支提交前 fresh run 为准，不复用历史结果。

## 下一步

1. 完成 `codex/demo-drill-act3-knowledge-governance` 本地验证、提交、PR、CI、合入；本分支包含幕3前端会签 ID 修复、134 API/UI 证据、试点准备手册第 4 章、词条、`_HANDOFF` 接力和 `DEFER-022`。
2. 合入后从最新 `origin/main` 继续幕4「规则配置与模拟」：先复核 `DEFER-021` 是否阻塞规则覆盖；若规则启用必须依赖已发布术语快照，则先关闭发布适配器缺口或调整幕4顺序，不得把幕2草稿包写成运行态有效。
3. 幕4继续沿用幕1凭据位置和脱敏纪律；所有规则、模拟、覆盖、发布事实都必须带 traceId / 页面截图 / T-GATE 证据。
