...
- OpenSpec `tasks.md` 已收敛为 205 个可供后续 AI 顺序执行的原子任务（17 完成、188 待执行）；覆盖缺口总账、首次信任根、签发者密钥隔离、自包含 `.mkp`、16 语义族医疗资源工厂、离线依赖仓、openEuler 空机部署、目标工具本地实现、134 一次确认部署、全量资源生产、医院复制和知识源迁移。
...
1. 在 `codex/launch-ledger` 按 TDD 执行 2.11：仅允许真实不可控的目标现场资源缺口归入 `ENVIRONMENT`，仓内可实现、可测试或可生产的数据缺口必须拒绝伪装成环境问题。
2. 执行 2.12 的严格归零总账；任何失败、未知、跳过、缺证、未解释失败或人工豁免非零时均禁止 `PASSED`，完成后独立提交并同步本文件。
