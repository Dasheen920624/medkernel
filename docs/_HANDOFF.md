...
- **新主线「全流程演练 · 使用指南 · 体验重构」已立**：客户反馈第一版功能多但名词/用法看不懂、规则/路径配置不可读、推荐引擎找不到入口。总体计划见 [docs/superpowers/plans/2026-06-10-full-flow-drill-usability-program.md](superpowers/plans/2026-06-10-full-flow-drill-usability-program.md)（幕0–10 演练剧本 + 线B指南 + 线C体验重构 + 第二阶段准入六判据）。**状态：计划已合并（#526）；DOC-SYNC 第一阶段在 `codex/demo-refactor-doc-sync-phase1` 本地完成并通过轻量验证；合入后下一步＝幕0 部署接管与首次登录（134 已是干净基线可直接开演）。**
...
- **DOC-SYNC 第一阶段（本分支完成，待 PR 合入）**：已按代码现实对齐权威层（宪法 / 质量基线 / 体验契约 / 范围核查 / backlog / README）：二级菜单数量不再是静态文档硬锁，菜单维以 `MenuPermissionCatalog` + 前端 `routes.ts` + 页面卡/合并说明为权威；质量基线补入「全流程演练 · 使用指南 · 体验重构」总闸门和体验专项验收；体验契约补入三层诊断、页面四问、五项重构原则和能力可见性闭环。发布流程代码实测仍为 7 步（`StepFlow.contract.ts`）；用户提及的额外发布步骤在仓库查无决策落点，仍不得写入权威事实，需定义实锤后建卡随代码同 PR 落地。
...
1. 提交并推动 `codex/demo-refactor-doc-sync-phase1` 走 PR / CI / 合入；合入后从最新 `origin/main` 继续，不恢复旧分线。
2. 开始幕0「部署接管与首次登录」：浏览器首访 134 → 读接管码 → 创建平台管理员 → 首登改密 → 绑定 MFA → 进入 `/workbench/readiness-validation`，证据归档 `docs/release/evidence/v1.0-drill-2026MMDD/幕0-部署接管与首次登录/`。
3. 幕0 同步产出《合规运维手册》第 1 章、词条（接管码 / MFA / 恢复码 / 就绪态）、`/bootstrap`、`/login`、验收自检页四问审计；不新增重复 handoff 文档。
