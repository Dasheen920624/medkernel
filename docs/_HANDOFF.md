...
- 当前最新验证：后端受影响回归 74/74，新增专项分类定向回归 7/7 且零跳过；前端 `npm run verify` 为 116 文件、2210 测试全绿，生产构建成功；Playwright 结构修复的 RC runner 测试 11/11 且能从 r5 原始报告独立重算 114/114；本次制品导出字节修复先红后绿，制品构建器与清单相关回归 61/61，完整 RC/release 工具 112/112；OpenSpec strict、RC JS Prettier 与 `git diff --check` 均通过。以上不能替代下一个候选的完整 clean RC0。
...
- 当前工作树基于已作废的 `88a166377`，只允许提交本次候选 `git archive` 导出字节校验修复、回归测试和同步文档；其干净 `git rev-parse HEAD` 才是下一次待重跑候选，提交后任何漂移都必须形成新候选。不得把构建残留或 r6/r7 证据纳入候选。
