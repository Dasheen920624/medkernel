...
- 基线：最新权威为 PR #651「合并 MedKernel 上线级主线交接版本」，`origin/main` 已更新到
  `0ebb80ab22abd3d6b6b9ca85707c6f857862e48b`。更早 PR、偏离设计、旧执行计划和旧接力事实只留在
  Git 历史，不作为当前事实入口。
- 当前分支：远端主线已通过 PR #651 合入；原临时远端工作分支 `origin/codex/engine-core-golive` 已清理。
  此前“只本地提交、不推送远程、不合并 `main`”约束已被用户最新指令“合并到远程 main”覆盖并完成；后续没有新的明确指令时，
  仍按 AGENTS 规则通过分支 + PR 进入受保护 `main`，不得直推或绕过保护。
- 134 运行候选：`930745d5eb2a9516b8f1e43fa7e00259ce22f2ca`（`按可见业务文本选择随访下拉项`）。
- 下列“本地最新……”体验优化条目已随 PR #651 合入 `main`；此处保留它们是为了标明 134 运行制品仍未更新，
  后续不得把 `main` 已合入误读为 134 已部署。
- 远端交接说明：`main` 已更新到 PR #651，但不代表 134 manifest 已更新，不代表 134 已重新部署，
  也不代表全角色真实演练与全局产品优化已经闭环；后续 AI 续接必须继续从本文件和权威文档核查，
  不得因 `main` 已合入而跳过 134 清库重部署、全功能/全知识/全流程演练和全视角优化。
...
- 本地最新生产前校验策略表达优化：`d117106b`（`统一生产前校验策略表达`）已完成本地验证，尚未同步到 134；
  134 manifest 仍为 `930745d5eb2a9516b8f1e43fa7e00259ce22f2ca`。
- 本地最新随访模板方案身份表达优化：`425d619d`（`统一随访模板方案身份表达`）已完成本地验证，尚未同步到 134；
  134 manifest 仍为 `930745d5eb2a9516b8f1e43fa7e00259ce22f2ca`。
- 本地最新知识审核摘要字段默认视图优化：`4320c6d7`（`收敛知识审核摘要字段默认视图`）已完成本地验证，尚未同步到 134；
  134 manifest 仍为 `930745d5eb2a9516b8f1e43fa7e00259ce22f2ca`。
- 本地最新知识生产任务默认证据视图优化：`ba1d373c`（`收敛知识生产任务默认证据视图`）已完成本地验证，尚未同步到 134；
  134 manifest 仍为 `930745d5eb2a9516b8f1e43fa7e00259ce22f2ca`。
- 本地最新临床快照选择器默认视图优化：`ece5ca3c`（`收敛临床快照选择器默认视图`）已完成本地验证，尚未同步到 134；
  134 manifest 仍为 `930745d5eb2a9516b8f1e43fa7e00259ce22f2ca`。
- 本地最新协同任务转交体验优化：`6c93d921`（`优化协同任务转交体验`）已完成本地验证，尚未同步到 134；
  134 manifest 仍为 `930745d5eb2a9516b8f1e43fa7e00259ce22f2ca`。
- 本地最新路径模板详情业务视图优化：`5b242d75`（`优化路径模板详情业务视图`）已完成本地验证，尚未同步到 134；
  134 manifest 仍为 `930745d5eb2a9516b8f1e43fa7e00259ce22f2ca`。
- 本地最新规则试运行风险等级表达优化：`c3efe393`（`收敛规则试运行风险等级表达`）已完成本地验证，尚未同步到 134；
  134 manifest 仍为 `930745d5eb2a9516b8f1e43fa7e00259ce22f2ca`。
- 本地最新沙盘重放默认业务视图优化：`df805103`（`收敛沙盘重放默认业务视图`）已完成本地验证，尚未同步到 134；
  134 manifest 仍为 `930745d5eb2a9516b8f1e43fa7e00259ce22f2ca`。
- 本地最新沙盘编排决策业务表达优化：`23bbfdec`（`统一沙盘编排决策业务表达`）已完成本地验证，尚未同步到 134；
  134 manifest 仍为 `930745d5eb2a9516b8f1e43fa7e00259ce22f2ca`。
- 本地最新规则抑制选择业务表达优化：`726ada64`（`优化规则抑制选择业务表达`）已完成本地验证，尚未同步到 134；
  134 manifest 仍为 `930745d5eb2a9516b8f1e43fa7e00259ce22f2ca`。
- 本地最新知识生成入口业务表达优化：`28f5bf7c`（`优化知识生成入口业务表达`）已完成本地验证，尚未同步到 134；
  134 manifest 仍为 `930745d5eb2a9516b8f1e43fa7e00259ce22f2ca`。
- 本地最新知识迁移后继身份表达优化：`93f79601`（`收敛知识迁移后继身份表达`）已完成本地验证，尚未同步到 134；
  134 manifest 仍为 `930745d5eb2a9516b8f1e43fa7e00259ce22f2ca`。
- 本地最新模型外调确认业务视图优化：`7cea2a38`（`优化模型外调确认业务视图`）已完成本地验证，尚未同步到 134；
  134 manifest 仍为 `930745d5eb2a9516b8f1e43fa7e00259ce22f2ca`。
- 本地最新临床提示卡操作配置体验优化：`f2ebd780`（`优化提示卡操作配置体验`）已完成本地验证，尚未同步到 134；
  134 manifest 仍为 `930745d5eb2a9516b8f1e43fa7e00259ce22f2ca`。
- 本地最新规则动作建议配置体验优化：`479f6c7f`（`优化规则动作建议配置体验`）已完成本地验证，尚未同步到 134；
  134 manifest 仍为 `930745d5eb2a9516b8f1e43fa7e00259ce22f2ca`。
- 本地最新回调签名预览业务输入优化：`b99136c1`（`优化回调签名预览业务输入`）已完成本地验证，尚未同步到 134；
  134 manifest 仍为 `930745d5eb2a9516b8f1e43fa7e00259ce22f2ca`。
- 本地最新沙盘运行产物默认视图优化：`9006beb5`（`收敛沙盘运行产物默认视图`）已完成本地验证，尚未同步到 134；
  134 manifest 仍为 `930745d5eb2a9516b8f1e43fa7e00259ce22f2ca`。
- 本地最新模型外调用途确认摘要生成优化：`72df9163`（`优化模型外调用途确认摘要生成`）已完成本地验证，尚未同步到 134；
  134 manifest 仍为 `930745d5eb2a9516b8f1e43fa7e00259ce22f2ca`。
- 本地最新批量规则草稿表业务列名优化：`92657628`（`优化批量规则草稿表业务列名`）已完成本地验证，尚未同步到 134；
  134 manifest 仍为 `930745d5eb2a9516b8f1e43fa7e00259ce22f2ca`。
- 本地最新路径配置下拉身份展示优化：`e6397713`（`收敛路径配置下拉身份展示`）已完成本地验证，尚未同步到 134；
  134 manifest 仍为 `930745d5eb2a9516b8f1e43fa7e00259ce22f2ca`。
...
- 当前阶段结论：现有模式八段全系统演练已在 134 清库部署后通过；基础真实前台数据路线已按前台操作跑通，
  ...
  第七十三刀继续回到真实前台随访模板创建流程，模板创建表单不再要求医生、护士、随访专员或实施人员理解“院内方案编号”、
  “标准编码”或 `FUP.COPD...` 示例，统一为“院内随访方案身份”和业务化示例；底层 `templateCode`、真实前台 E2E
  生成身份、随访模板创建和发布契约保持不变。
  第七十四刀继续回到知识审核真实流程，候选审核对照抽屉默认不再把 `contentHash` 和“编码”作为字段名展示，
  改为“现行摘要 / 候选摘要 / 知识身份”；打开证据详情后仍显示 `contentHash` 和知识身份编码。
  第七十五刀继续回到路径模板详情与真实快照试运行，节点画布、路径流转表、时钟指标绑定、结局指标绑定和回放轨迹默认
  使用路径节点名称、第 N 个节点/流转、业务节点类型、已关联指标等表达，避免医生、护士、临床路径负责人、信息科和实施人员
  默认看到 `ASSESS`、`FOLLOWUP`、`EDGE...`、`PATH.TIME...` 等技术身份；打开证据详情后仍可追溯节点 code、边 code、
  条件 JSON、受控配置文本和回放原始轨迹。
  第七十六刀继续回到临床规则试运行真实前台，最高严重警示和规则命中表默认展示“高风险 / 极高风险”等中文风险等级，
  不再让医生在红线命中时看到 `HIGH`、`CRITICAL` 这类枚举；证据详情打开后仍附带原始严重度枚举用于实施和审计追溯。
  第七十七刀继续回到全真体验沙盘历史重放与版本差异真实流程，历史规则结果默认展示中文历史状态、中文风险等级，
  版本差异默认展示平台标准/机构版本和第 N 版业务摘要，不再让医生、信息科、实施工程师或院内管理者默认看到
  `RETIRED`、`CRITICAL`、`PLATFORM`、`ORG` 或规则编码；证据详情打开后仍可追溯原始状态、严重度、来源层级和规则编码。
  第七十八刀继续回到全真体验沙盘当前运行与临床嵌入决策回传流程，编排入口默认展示“推荐综合卡编排 / 智能推荐”等业务表达，
  不再展示 `RECOMMENDATION_COMPOSITE`、`recommendation`；嵌入终端回传默认展示采纳建议、建议已采纳、卡片证据已记录和
  追踪证据已保留，不再展示 `ADOPT`、`ADOPTED`、卡片号和追踪号；证据详情打开后仍可追溯原始 action、状态、卡片号和追踪证据。
  第七十九刀继续回到规则配置真实编辑流程，抑制规则选择不再默认展示 `RULE... · P100 · 规则名称` 这类技术拼接，
  统一为“规则名称 · 优先级 N”的业务选项；仍支持按稳定规则身份搜索和提交 `suppressedBy`，证据详情打开后可追溯原始规则身份。
  第八十刀继续回到医疗引擎运营员的知识生产真实流程，大模型生成正式知识候选弹窗默认展示“正式知识候选生成”、
  “统一模型服务”和知识主题，不再要求操作者填写模型能力代码、模型服务代码或重复的资产身份；目标知识身份自动派生
  `assetIdentity`，来源锚点提示改为指南章节、制度条款或文献段落定位，证据详情打开后仍可追溯原始能力、服务和知识身份。
  第八十一刀继续回到平台治理安排知识弃用/迁移流程，后继知识身份下拉默认只展示业务主题，不再拼接
  `plat:drug...` 等身份编码；底层仍按后继身份 ID 调用弃用接口，证据详情打开后可追溯原始身份。
  第八十二刀继续回到模型能力和外调安全真实流程，公网模型患者上下文用途确认不再让实施人员手填
  `sha256:payload-001` 这类摘要样例，统一由系统基于当前字段预览、脱敏策略和用途说明生成审计摘要；前台只确认业务用途，
  避免人工伪填或误填审计凭证。
  第八十三刀继续回到医疗引擎运营员和实施人员的批量规则创作流程，批量生成草稿不再要求粘贴
  `ruleCode,name,threshold` 这类内部列表头；前台接受“规则身份 / 规则名称 / 阈值 / 启用”等业务列名并映射回原生成契约。
  第八十四刀继续回到临床路径负责人、医生、护士和实施人员的路径配置流程，节点、阶段、里程碑和结局指标下拉不再默认拼接
  `ASSESS`、`FOLLOWUP` 或 `PATH.OUTCOME.LOS` 等身份，改用节点序号、阶段名、里程碑业务描述和指标版本区分。
...
- 最新生产前校验策略表达切片：
  ...
  `npm --prefix frontend run typecheck`、`npm --prefix frontend run lint`、`git diff --check`。
- 最新随访模板方案身份表达切片：
  `425d619d` 将真实前台随访模板创建表单从“院内方案编号 / 标准编码 / FUP.COPD 示例”收敛为“院内随访方案身份 /
  标准身份 / 慢阻肺出院随访-2026 示例”，并同步 `real-frontdesk-rehearsal` 的前台操作选择器；底层 `templateCode`、
  随访模板创建、发布和真实前台生成身份契约保持不变。本地验证通过
  `npm --prefix frontend test -- --run src/pages/clinical/Followup.test.tsx`、
  `npm --prefix frontend test -- --run src/pages/clinical/Followup.test.tsx src/pages/clinical/WorkflowTodos.test.tsx src/pages/clinical/Notifications.test.tsx src/pages/clinical/Mpi.test.tsx src/pages/clinical/PatientPathways.test.tsx src/pages/pages.smoke.test.tsx src/shared/ui/PageExperienceShell.test.tsx src/shared/config/routes.test.ts src/shared/config/menu.test.ts`、
  `npm --prefix frontend run typecheck`、`npm --prefix frontend run lint`、`git diff --check`。
- 最新知识审核摘要字段默认视图切片：
  `4320c6d7` 将知识候选审核对照抽屉里的 `contentHash` 默认标签收敛为“现行摘要 / 候选摘要”，并把知识身份字段从
  默认“编码”改成“知识身份”；证据详情打开后仍可追溯 `contentHash`、知识身份编码和来源证据。本地验证通过
  `npm --prefix frontend test -- --run src/pages/quality/KnowledgeGovernance.test.tsx`、
  `npm --prefix frontend test -- --run src/pages/quality/KnowledgeGovernance.test.tsx src/pages/knowledge-production/KnowledgeProductionPage.test.tsx src/pages/knowledge-production/ProductionReadinessPanel.test.tsx src/pages/knowledge-production/ProviderSetupPanel.test.tsx src/pages/knowledge-production/MedicalEvaluationPanel.test.tsx src/pages/pages.smoke.test.tsx src/shared/ui/PageExperienceShell.test.tsx src/shared/config/routes.test.ts src/shared/config/menu.test.ts`、
  `npm --prefix frontend run typecheck`、`npm --prefix frontend run lint`、`git diff --check`。
- 最新知识生产任务默认证据视图切片：
  `ba1d373c` 将知识生产任务表格、中止确认和生成候选弹窗中的生产任务号、模型策略标识等默认暴露，收敛为
  “生产任务已登记 / 模型生产策略已配置 / 当前生产任务已选定 / 确认中止生产任务”等业务表达；未显式填写模型生产策略时
  不再预填 `gpt-pipeline`，避免医疗引擎运营员、信息科和实施人员把内部策略标识当作必填业务内容。底层 `jobCode`、
  `modelStrategy`、任务取消、模型生成候选和证据详情追溯契约保持不变。本地验证通过
  `npm --prefix frontend test -- --run src/pages/quality/KnowledgeGovernance.test.tsx`、
  `npm --prefix frontend test -- --run src/pages/quality/KnowledgeGovernance.test.tsx src/pages/knowledge-production/KnowledgeProductionPage.test.tsx src/pages/knowledge-production/ProductionReadinessPanel.test.tsx src/pages/knowledge-production/ProviderSetupPanel.test.tsx src/pages/knowledge-production/MedicalEvaluationPanel.test.tsx src/pages/pages.smoke.test.tsx src/shared/ui/PageExperienceShell.test.tsx src/shared/config/routes.test.ts src/shared/config/menu.test.ts`、
  `npm --prefix frontend run typecheck`、`npm --prefix frontend run lint`、`git diff --check`。
- 最新临床快照选择器默认视图切片：
  `ece5ca3c` 将共享 `ContextSnapshotSelector` 的默认视图从自动展开患者号、就诊号和快照号，改为默认展示
  “已关联患者与就诊 / 第 N 个临床快照”业务视图；只有调用方显式传入证据详情开启态时才显示原始患者/就诊/快照标识。
  同步随访计划、质控仿真和医保审核测试选择器，保证内部 `contextSnapshotId` 仍提交给后端，医生、护士、质控、
  医保审核员和患者代理默认不再因调用方漏传开关看到敏感标识。本地验证通过
  `npm --prefix frontend test -- --run src/shared/ui/ContextSnapshotSelector.test.tsx`、
  `npm --prefix frontend test -- --run src/shared/ui/ContextSnapshotSelector.test.tsx src/pages/clinical/Followup.test.tsx src/pages/clinical/PatientPathways.test.tsx src/pages/clinical/RuleValidate.test.tsx src/pages/clinical/CdssFatigue.test.tsx src/pages/quality/QcEvalSets.test.tsx src/pages/quality/InsuranceAudit.test.tsx src/pages/pages.smoke.test.tsx src/shared/ui/PageExperienceShell.test.tsx`、
  `npm --prefix frontend run typecheck`、`npm --prefix frontend run lint`、`git diff --check`。
- 最新协同任务转交体验切片：
  `6c93d921` 将协同任务“转交待办”从医生/护士手填接收人编号和 `NURSING` 等角色枚举，优化为按姓名或院内人员身份
  搜索接收人员，并用中文接收岗位下拉选择；前台仍提交原 `transferTo`、`transferRole`、`transferReason` 审计契约，
  不改变后端流转接口。默认列表继续只展示责任岗位、患者已关联和来源证据已保留，证据详情权限边界保持不变。本地验证通过
  `npm --prefix frontend test -- --run src/pages/clinical/WorkflowTodos.test.tsx`、
  `npm --prefix frontend test -- --run src/pages/clinical/WorkflowTodos.test.tsx src/pages/clinical/Notifications.test.tsx src/pages/clinical/CdssFatigue.test.tsx src/pages/clinical/PatientPathways.test.tsx src/pages/clinical/Followup.test.tsx src/pages/quality/QcAlerts.test.tsx src/pages/quality/QcEvalResults.test.tsx src/pages/pages.smoke.test.tsx src/shared/ui/PageExperienceShell.test.tsx`、
  `npm --prefix frontend run typecheck`、`npm --prefix frontend run lint`、`git diff --check`。
- 最新路径模板详情业务视图切片：
  `5b242d75` 将路径模板详情的节点画布、路径流转表、时钟指标绑定、结局指标绑定和真实快照回放轨迹从默认展示
  `ASSESS`、`FOLLOWUP`、`EDGE.ASSESS.FOLLOWUP`、`ASSESSMENT`、`CONDITION`、`PATH.TIME.ASSESS` 等技术身份，
  收敛为入径评估、出径随访、第 N 个路径节点、第 N 条路径流转、业务节点类型和已关联指标等默认业务表达；证据详情打开后
  仍显示节点 code、边 code、条件 JSON、受控配置文本和原始回放轨迹。编辑态仍保留稳定身份输入和原始提交契约，保证
  发布治理、机构生效版本和后端路径仿真精确追溯不变。本地验证通过
  `npm --prefix frontend test -- --run src/pages/tenant/PathwayTemplates.test.tsx`、
  `npm --prefix frontend test -- --run src/pages/tenant/PathwayTemplates.test.tsx src/pages/tenant/PathwayGraphEditor.test.tsx src/pages/tenant/RuleDefinitions.test.tsx src/pages/pages.smoke.test.tsx src/shared/ui/PageExperienceShell.test.tsx`、
  `npm --prefix frontend run typecheck`、`npm --prefix frontend run lint`、`git diff --check`。
- 最新规则试运行风险等级表达切片：
  `c3efe393` 将临床规则试运行页最高严重警示和规则命中表从默认展示 `HIGH`、`CRITICAL` 等枚举，收敛为
  “高风险 / 极高风险”等统一中文风险等级；证据详情打开后仍附带原始严重度枚举，规则评估、历史执行解释回放和人工继续留痕
  接口契约保持不变。本地验证通过
  `npm --prefix frontend test -- --run src/pages/clinical/RuleValidate.test.tsx`、
  `npm --prefix frontend test -- --run src/pages/clinical/RuleValidate.test.tsx src/pages/clinical/CdssFatigue.test.tsx src/pages/clinical/PatientPathways.test.tsx src/pages/clinical/WorkflowTodos.test.tsx src/pages/clinical/Notifications.test.tsx src/pages/clinical/Followup.test.tsx src/pages/pages.smoke.test.tsx src/shared/ui/PageExperienceShell.test.tsx`、
  `npm --prefix frontend run typecheck`、`npm --prefix frontend run lint`、`git diff --check`。
- 最新沙盘重放默认业务视图切片：
  `df805103` 将全真体验沙盘历史重放和版本差异结果从默认展示 `RETIRED`、`CRITICAL`、`PLATFORM`、`ORG` 与规则编码，
  收敛为历史已退役、危急风险、平台标准、机构版本和第 N 版等业务表达；证据详情打开后仍可追溯原始状态、严重度、来源层级和
  规则编码，历史清单重放与当前版本差异评估提交契约保持不变。本地验证通过
  `npm --prefix frontend test -- --run src/pages/sandbox/SandboxHost.test.tsx`、
  `npm --prefix frontend test -- --run src/pages/sandbox/SandboxHost.test.tsx src/features/sandbox/SandboxEmbedFrame.test.tsx src/features/sandbox/SandboxPathInspector.test.tsx src/features/sandbox/SandboxDataEntry.test.tsx src/features/sandbox/sandboxScenarios.test.ts src/pages/pages.smoke.test.tsx src/shared/ui/PageExperienceShell.test.tsx`、
  `npm --prefix frontend run typecheck`、`npm --prefix frontend run lint`、`git diff --check`。
- 最新沙盘编排决策业务表达切片：
  `23bbfdec` 将全真体验沙盘当前运行的编排入口从默认展示 `RECOMMENDATION_COMPOSITE`、`recommendation` 等技术字段，
  收敛为“推荐综合卡编排 / 智能推荐”等业务表达；嵌入终端决策回传默认展示采纳建议、建议已采纳、卡片证据已记录和追踪证据已保留，
  原始 `ADOPT`、`ADOPTED`、卡片号和追踪号只在证据详情中展开。沙盘运行、postMessage 来源校验和嵌入终端回调契约保持不变。
  本地验证通过
  `npm --prefix frontend test -- --run src/pages/sandbox/SandboxHost.test.tsx src/features/sandbox/SandboxEmbedFrame.test.tsx src/features/sandbox/SandboxPathInspector.test.tsx src/features/sandbox/SandboxDataEntry.test.tsx src/features/sandbox/sandboxScenarios.test.ts src/pages/pages.smoke.test.tsx src/shared/ui/PageExperienceShell.test.tsx`、
  `npm --prefix frontend run typecheck`、`npm --prefix frontend run lint`、`git diff --check`。
- 最新规则抑制选择业务表达切片：
  `726ada64` 将规则配置创建/编辑表单中的抑制规则选择从默认展示 `RULE.QC.REVIEW · P100 · 规则发布校验核查`，
  收敛为“规则发布校验核查 · 优先级 100”等业务选项；表单仍可按稳定规则身份搜索并提交 `suppressedBy`，证据详情打开后
  可追溯原始规则身份。本地验证通过
  `npm --prefix frontend test -- --run src/pages/tenant/RuleDefinitions.test.tsx`、
  `npm --prefix frontend test -- --run src/pages/tenant/RuleDefinitions.test.tsx src/pages/tenant/ReleaseGovernance.test.tsx src/pages/tenant/PathwayTemplates.test.tsx src/pages/tenant/PathwayGraphEditor.test.tsx src/pages/clinical/RuleValidate.test.tsx src/pages/pages.smoke.test.tsx src/shared/ui/PageExperienceShell.test.tsx`、
  `npm --prefix frontend run typecheck`、`npm --prefix frontend run lint`、`git diff --check`。
- 最新知识生成入口业务表达切片：
  `28f5bf7c` 将知识生产中心的大模型生成候选弹窗从默认展示模型能力代码、模型服务代码、知识身份编码和重复资产身份，
  收敛为正式知识候选生成、统一模型服务、知识主题和现有知识身份业务选择；`assetIdentity` 改由目标知识身份派生，
  来源锚点提示改为指南章节、制度条款或文献段落定位，后端提交契约仍保留能力代码、模型服务、资产身份和来源锚点。
  本地验证通过
  `npm --prefix frontend test -- --run src/pages/quality/KnowledgeGovernance.test.tsx -t "uses business choices in model generation"`、
  `npm --prefix frontend test -- --run src/pages/quality/KnowledgeGovernance.test.tsx`、
  `npm --prefix frontend test -- --run src/pages/quality/KnowledgeGovernance.test.tsx src/pages/quality/AcquisitionSourceGovernancePanel.test.tsx src/pages/pages.smoke.test.tsx src/shared/ui/PageExperienceShell.test.tsx`、
  `npm --prefix frontend run typecheck`、`npm --prefix frontend run lint`、`git diff --check`。
- 最新知识迁移后继身份表达切片：
  `93f79601` 将知识审核与发布页安排知识身份弃用时的后继知识身份选项从默认展示“主题 · 身份编码”，收敛为只展示
  业务主题；底层弃用接口仍按 `successorIdentityId` 调用，证据详情打开后仍可追溯原始知识身份。本地验证通过
  `npm --prefix frontend test -- --run src/pages/quality/KnowledgeGovernance.test.tsx -t "lets platform governance schedule a successor"`、
  `npm --prefix frontend test -- --run src/pages/quality/KnowledgeGovernance.test.tsx src/pages/quality/AcquisitionSourceGovernancePanel.test.tsx src/pages/pages.smoke.test.tsx src/shared/ui/PageExperienceShell.test.tsx`、
  `npm --prefix frontend run typecheck`、`npm --prefix frontend run lint`、`git diff --check`。
- 最新模型外调确认业务视图切片：
  `7cea2a38` 将知识生产中心的模型外调用途确认弹窗从默认展示能力代码、载荷 hash、字段名和模型服务 code，收敛为
  正式知识候选生成、统一模型服务、脱敏摘要已登记和拟供模型使用信息；打开证据详情后仍可追溯原始
  `capabilityCode`、`payloadHash`、`egressFields` 和 `providerCode`，确认接口仍提交原始能力码、载荷摘要和用途说明。
  本地验证通过
  `npm --prefix frontend test -- --run src/pages/quality/KnowledgeGovernance.test.tsx -t "confirms actionable egress purpose"`、
  `npm --prefix frontend test -- --run src/pages/quality/KnowledgeGovernance.test.tsx src/pages/quality/AcquisitionSourceGovernancePanel.test.tsx src/pages/pages.smoke.test.tsx src/shared/ui/PageExperienceShell.test.tsx`、
  `npm --prefix frontend run typecheck`、`npm --prefix frontend run lint`、`git diff --check`。
- 最新临床提示卡操作配置体验切片：
  `f2ebd780` 将声明式资产工作台的临床提示卡可选操作从默认填写 JSON“操作参数”，收敛为“关联业务对象”业务字段；
  建议医嘱会生成后端认可的 `payload.orderSetRef`，打开记录表单和相关页面分别保留结构化映射到 `formRef` 与 `route`。
  证据详情打开后仍可查看/编辑扩展 JSON 参数，保存时结构化业务对象覆盖同名扩展键，避免实施人员必须手写 JSON 才能完成
  医嘱建议配置；建议医嘱仍自动要求医师逐条确认。本地验证通过
  `npm --prefix frontend test -- --run src/pages/tenant/DeclarativeAssetWorkbench.test.tsx -t "business target field"`、
  `npm --prefix frontend test -- --run src/pages/tenant/DeclarativeAssetWorkbench.test.tsx src/pages/tenant/AuthoringAssets.test.tsx src/shared/config/declarativeAssetAuthoring.test.ts src/pages/pages.smoke.test.tsx src/shared/ui/PageExperienceShell.test.tsx`、
  `npm --prefix frontend run typecheck`、`npm --prefix frontend run lint`、`git diff --check`。
- 最新规则动作建议配置体验切片：
  `479f6c7f` 将规则定义 L2 条件树里的医生可选操作从默认手写“操作参数/参数键/参数值”，收敛为“可选操作类型”
  业务下拉与“关联业务对象”字段；新增动作默认使用后端认可的 `ACKNOWLEDGE`，建议医嘱、打开记录表单和打开相关页面分别
  结构化生成 `payload.orderSetRef`、`payload.formRef` 与 `payload.route`。受控配置文本模式仍保留高级 DSL 编辑入口，
  但主路径不再要求实施人员手写技术键值。本地验证通过
  `npm --prefix frontend test -- --run src/pages/tenant/RuleDefinitions.test.tsx -t "规则动作建议使用业务对象"`、
  `npm --prefix frontend test -- --run src/pages/tenant/RuleDefinitions.test.tsx src/pages/tenant/AuthoringBatchDrawer.test.tsx src/pages/tenant/DeclarativeAssetWorkbench.test.tsx src/pages/tenant/PathwayTemplates.test.tsx src/pages/tenant/ReleaseGovernance.test.tsx src/pages/pages.smoke.test.tsx src/shared/ui/PageExperienceShell.test.tsx`、
  `npm --prefix frontend run typecheck`、`npm --prefix frontend run lint`、`git diff --check`。
- 最新回调签名预览业务输入切片：
  `b99136c1` 将系统接入页回调通道的签名预览从默认手写 JSON 载荷，收敛为“预览事件、样例患者、样例就诊、事件摘要”
  业务字段；默认使用非真实样例患者/就诊信息生成后端仍认可的 `payload` 字符串，不向外部地址发起请求。打开证据详情后
  才提供原始“签名预览载荷”覆盖入口，用于核查服务契约，避免实施工程师和信息科长在主路径手写事件 JSON。本地验证通过
  `npm --prefix frontend test -- --run src/pages/tenant/AdapterHub.test.tsx -t "previews signatures"`、
  `npm --prefix frontend test -- --run src/pages/tenant/AdapterHub.test.tsx src/pages/pages.smoke.test.tsx src/shared/ui/PageExperienceShell.test.tsx`、
  `npm --prefix frontend run typecheck`、`npm --prefix frontend run lint`、`git diff --check`。
- 最新沙盘运行产物默认视图切片：
  `9006beb5` 将全真体验沙盘运行摘要中的患者路径实例、随访计划和评估运行从默认展示内部 ID，收敛为“路径实例已生成、
  随访计划已登记、评估运行已记录”；证据详情打开后仍可追溯 `patientPathwayId`、`followupPlanId` 与 `evaluationRunId`。
  这避免院长、信息科长和实施工程师在验收演练摘要中把内部运行号误读为业务结论。本地验证通过
  `npm --prefix frontend test -- --run src/pages/sandbox/SandboxHost.test.tsx -t "runs the selected scenario"`、
  `npm --prefix frontend test -- --run src/pages/sandbox/SandboxHost.test.tsx src/pages/pages.smoke.test.tsx src/shared/ui/PageExperienceShell.test.tsx`、
  `npm --prefix frontend run typecheck`、`npm --prefix frontend run lint`、`git diff --check`。
- 最新模型外调用途确认摘要生成切片：
  `72df9163` 将模型能力页公网模型使用患者上下文的用途确认从默认手填“脱敏载荷摘要”收敛为系统生成 SHA-256 审计摘要；
  摘要口径基于当前能力、路由、脱敏策略、字段处理结果和用途说明，前台仅要求填写业务用途，仍向后端提交 `payloadHash`
  与用途说明，避免实施人员、信息科或审计员手工伪填样例哈希。本地验证通过
  `npm --prefix frontend test -- --run src/pages/advanced/AiWorkflows.test.tsx -t "允许具备权限的实施人员"`、
  `npm --prefix frontend test -- --run src/pages/advanced/AiWorkflows.test.tsx src/pages/pages.smoke.test.tsx src/shared/ui/PageExperienceShell.test.tsx`、
  `npm --prefix frontend run typecheck`、`npm --prefix frontend run lint`、`git diff --check`。
- 最新批量规则草稿表业务列名切片：
  `92657628` 将批量规则生成表单从默认“参数表”和 `ruleCode,name,threshold` 示例，收敛为“批量规则草稿表”和
  “规则身份、规则名称、阈值、启用”等业务列名；前台将业务列名映射为后端 `ruleCode`、`name`、模板参数和可选变更说明，
  批量生成、影响分析、发布推进和高危逐条确认契约不变。本地验证通过
  `npm --prefix frontend test -- --run src/pages/tenant/AuthoringBatchDrawer.test.tsx -t "generates one rule draft"`、
  ...
  `npm --prefix frontend run typecheck`、`npm --prefix frontend run lint`、`git diff --check`。
- 最新路径配置下拉身份展示切片：
  `e6397713` 将路径模板创建/编辑中的起始节点、源节点、目标节点、阶段、里程碑和结局指标下拉默认展示，从“业务名（身份 code）”
  收敛为“业务名 · 节点序号 / 阶段名 / 里程碑业务描述 / 指标第 N 版”；底层仍提交 `startNodeCode`、`fromNodeCode`、
  `toNodeCode`、`phaseCode`、`milestoneCode` 和 `indicatorCode`，详情证据仍可追溯原始身份。本地验证通过
  `npm --prefix frontend test -- --run src/pages/tenant/PathwayTemplates.test.tsx -t "路径建模表单以稳定业务身份"`、
  `npm --prefix frontend test -- --run src/pages/tenant/PathwayTemplates.test.tsx`、
  `npm --prefix frontend test -- --run src/pages/tenant/PathwayTemplates.test.tsx src/pages/tenant/PathwayGraphEditor.test.tsx src/pages/tenant/RuleDefinitions.test.tsx src/pages/tenant/ReleaseGovernance.test.tsx src/pages/pages.smoke.test.tsx src/shared/ui/PageExperienceShell.test.tsx`、
  `npm --prefix frontend run typecheck`、`npm --prefix frontend run lint`、`git diff --check`。
- 最新质量问题整改体验切片：
...
1. 进入真实前台全角色体验：平台管理员看系统接入与安全基线，医疗引擎运营员看知识生产和版本发布，临床使用者拆分医生、
   ...
   权限、全院指标和故障降级。
2. 已完成沙盘、沙盘运行产物默认视图、沙盘重放/版本差异默认业务视图、沙盘编排决策业务表达、MPI、患者路径、患者路径入径提示、患者路径结局指标身份表达、消息通知、临床快照选择器与默认敏感标识收敛、临床嵌入启动、规则试运行与二次默认表头/留痕标签/风险等级表达、规则配置与规则资产身份表达、规则配置操作身份表达、规则抑制选择业务表达、规则动作建议配置体验、路径配置与身份表达、路径配置下拉身份展示、路径模板详情业务视图、批量规则创作、批量规则草稿表业务列名、临床提示卡操作配置体验、协同任务与转交体验、CDSS 提醒推荐与频次治理技术标识收敛、随访协同与快照患者信息表达、随访模板方案身份表达、质量/医保快照就诊信息表达、人员账号与身份来源院内人员身份表达、系统配置服务机构身份表达、系统接入回调签名预览业务输入、审计操作人筛选表达、上线运营默认检索登记表达、运维演练默认身份表达、工作台审计操作人默认视图、验收自检错误留痕契约纠偏、知识生产策略业务表达、生产前校验策略表达、知识生产任务默认证据视图、知识生成入口业务表达、知识迁移后继身份表达、模型外调确认业务视图、模型外调用途确认摘要生成、质量管理概览、质量问题来源与问题身份表达、质量责任归属业务表达、质量问题与整改、医保审核与输入、评价指标与评价指标身份表达、知识审核与发布、知识审核摘要字段默认视图、知识身份业务表达、公域来源治理、诊断知识维护与发现项身份表达、术语与字典、全局权限范围、全局入口业务反馈、通知偏好、工作台错误留痕、异步导出留痕、编排预览留痕、接口错误留痕、共享错误状态留痕、验收证据配置、服务机构品牌配置、模型能力、模型服务配置与身份配置表达、来源血缘与知识身份搜索表达、图谱查询、审计证据、安全基线、运行诊断、验收自检和国产化自检默认视图/证据详情多轮本地优化；
   继续优先扫描关键临床/患者真实流程：
...
4. 下一阶段仍需在 134 执行全角色、全知识、全流程复演；本轮只证明基础真实前台数据路线已跑通，从 `cd44d8ab`
   到 `e6397713` 的全角色体验优化提交均尚未部署 134。
5. PR #651 已按最新用户指令合并远程 `main`；后续没有新的明确指令时恢复常规协作边界：
   分支开发、PR 合并、不得直推受保护 `main`；不要提交未跟踪的 `.codex/config.toml`。
