...
- OpenSpec `tasks.md` 已收敛为 205 个可供后续 AI 顺序执行的原子任务（12 完成、193 待执行）；覆盖缺口总账、首次信任根、签发者密钥隔离、自包含 `.mkp`、16 语义族医疗资源工厂、离线依赖仓、openEuler 空机部署、目标工具本地实现、134 一次确认部署、全量资源生产、医院复制和知识源迁移。
...
1. 在 `codex/launch-ledger` 按 TDD 执行 2.6：强制前置证据不可由最终审计自证，并绑定白名单路径、同一 run-id、判定前时间和内容摘要。
2. 2.6 归零后依次执行 2.7-2.12 的四类缺口和严格归零总账；每个 checkbox 独立验证、提交并同步本文件。
