...
- **新主线「全流程演练 · 使用指南 · 体验重构」已进入幕0收口**：客户反馈第一版功能多但名词/用法看不懂、规则/路径配置不可读、推荐引擎找不到入口。总体计划见 [docs/superpowers/plans/2026-06-10-full-flow-drill-usability-program.md](superpowers/plans/2026-06-10-full-flow-drill-usability-program.md)（幕0–10 演练剧本 + 线B指南 + 线C体验重构 + 第二阶段准入六判据）。**状态：计划已合并（#526）；DOC-SYNC 第一阶段已合并（#528，`fbaa012e`）；当前分支 `codex/demo-drill-act0-bootstrap` 正在收口幕0「部署接管与首次登录」证据、指南、术语和发布脚本修复，待验证/PR/CI/合入后继续幕1。**
...
- **幕0执行事实（本分支待 PR）**：134 已完成真实接管；首发账号 `drill-platform-admin-20260611` 具备 `system-superadmin`，租户 `t-1`，已完成首登改密与 MFA 绑定。凭据只存服务器 `/zoesoft/medkernel/conf/drill-admin-credentials-20260611.txt`，仓库只提交路径/权限证据。幕0证据目录：`docs/release/evidence/v1.0-drill-20260611/幕0-部署接管与首次登录/`。
- **幕0暴露并修复的运维缺陷**：134 上旧前端资源导致 `system-superadmin` 访问验收自检页误显权限不足，API `/security/me` 证明确有 `menu.workbench` 与 `workbench:readiness:view`；已重新构建并前端-only 发布，复验页展示 9 就绪 / 5 阻塞 / 7 未启用。发布过程中发现 `medkernel-deploy --frontend` 会自动捡 incoming 残留 jar，误触后端部署并回滚；本分支修复脚本为「只有未显式指定 jar/frontend 时才自动发现包」，远端脚本已备份后热修，服务健康恢复。
...
- 知识生产服务器（193.112.107.134，govcloud + PostgreSQL 15）已据 `b2e7329e` 清库重发，并在幕0完成接管与前端刷新：**Flyway 113/113 真应用**（此前实际停在 V99，本次清库重迁补齐）、`pref_value=text`、每日 02:30 备份至 COS 盘 30 天保留、nginx 已封堵 swagger/api-docs/actuator（health 除外）、java.io.tmpdir 已固化（PrivateTmp 防证据文件丢失）。
...
- PR #524 验证：后端 **2188 项 0 失败 0 错误**（5 项本机无 Docker 跳过：2 既有方言 smoke + 3 新空库 smoke，CI 真跑）；T-GATE（真实性 / 配置边界 / 迁移规约 / 中文注释）changed 全绿。当前 `codex/demo-drill-act0-bootstrap` 的验证以本分支提交前 fresh run 为准，不复用历史结果。

## 下一步

1. 完成 `codex/demo-drill-act0-bootstrap` 验证、提交、PR、CI、合入；本分支包含幕0证据、合规运维手册首章、词条、`_HANDOFF` 接力和 `medkernel-deploy --frontend` 防误捡 jar 修复。
2. 合入后从最新 `origin/main` 继续幕1「租户、组织与用户」：使用服务器凭据文件中的首发管理员登录，开通「演练总医院」，创建科室树与 9 个演练账号，交叉验证五维权限。
3. 幕1同步产出《试点准备手册》第 1-2 章、词条（租户 / 组织范围 / 五维权限 / 角色）、实施向导四问审计和角色覆盖矩阵证据；不新增重复 handoff 文档。
