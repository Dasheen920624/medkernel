...
- **主线「全流程演练·使用指南·体验重构」：幕0–10 L1/L2 已全部合入 main；当前分支正在做总验收收口与第二阶段准入核验**（计划 #526；DOC-SYNC #528；幕0–8 = #529–#537；幕9 = #538 `3e4ba441`；实现路径纠偏 #539 `0b0d4cb2`；幕10 L1 = #540 `efbfd9ab`；幕8.5 第一批 = #541 `6aaba08b`；幕8.5 第二批 = #542 `859477f`；幕8.5 第三批 = #543 `516ad753`；幕10 L2 = #544 `dde82115`；当前分支 `codex/demo-drill-total-acceptance`）。总体计划见 [docs/superpowers/plans/2026-06-10-full-flow-drill-usability-program.md](superpowers/plans/2026-06-10-full-flow-drill-usability-program.md)。
...
- **总验收当前结论（当前分支）**：已补齐 3 本角色培训材料，并新增 [全流程演练总验收](release/evidence/v1.0-drill-20260611/00-total-acceptance.md)。真实状态是：幕0–10 证据、指南、培训、四问审计和能力可见性矩阵已补齐；第二阶段准入尚未全绿，仍需实现 P1 体验项（`OPT-IA-01` / `OPT-TRACE-01` / `OPT-VIS-01` / `OPT-VIS-02` / `UI-ACT10-AUDIT-01`）、完成独立指南验收，再执行 134 清库重置。

## 当前状态

- main：`dde82115`（幕10 L2 #544 已合）；当前分支 `codex/demo-drill-total-acceptance` 承载总验收报告、3 本角色培训补齐和第二阶段准入缺口清单。
...
1. 收口当前分支：验证 Markdown 链接 / JSON / diff check / T-GATE，提交 `codex/demo-drill-total-acceptance`，推送 PR，CI 通过后合并。
2. 合并后继续体验实现线，优先级：`OPT-IA-01` + `OPT-TRACE-01`（提醒与推荐中枢 / 链路一张图）、`OPT-VIS-01`（规则自然语言与只读流程图）、`OPT-VIS-02` + `OPT-PATH-UI-01`（医生只读路径图）、`UI-ACT10-AUDIT-01`（traceId 搜索与诊断链跳转）。
