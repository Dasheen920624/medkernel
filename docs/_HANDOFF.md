...
### 线 1 · SYS-05 运行模式框架 PR1 🚧

- 类型：软件开发 / 后端运行任务框架
- 分支：`codex/sys-05-runtime-modes-pr1`（基于 `origin/main` 8e6063a）
- 目标：按 [SYS-05](cards/D0/SYS-05.md) 大卡工序交付 PR1：在线 / 异步 / 批量模式 + 待办状态机 + 批量部分成功，收口 FR-1/2/3 与 AC-1/2；离线、重试、死信、回放保留给 PR2。
- 状态：已写计划 `docs/superpowers/plans/2026-06-01-sys-05-runtime-modes-pr1.md`；已完成红测并确认失败点为新框架类型缺失；已实现 `com.medkernel.shared.runtime.task`、`sys_task` V41 五方言迁移、`/api/v1/system/tasks` 提交/查询端点、服务契约与领域 owner；`RuntimeTaskServiceTest` 覆盖在线超时升级、异步入队轮询、批量部分成功、批量完成计数归一化；`RuntimeTaskMigrationContractTest` 覆盖 V41 五方言注释与索引；聚焦验证 `mvn -B -q -Dtest=RuntimeTaskServiceTest,RuntimeTaskMigrationContractTest,MigrationBaselineContractTest,DomainOwnershipContractTest,ServiceContractGovernanceTest,OpenApiContractConfigurationTest test` 已通过；后端全量 `mvn -B -q test` 已通过（Surefire：`tests=756 failures=0 errors=0 skipped=0`，含 PostgreSQL 15 + Oracle 21 Testcontainers 迁移至 V41）；提交后 changed T-GATE（真实性 / 配置边界 / 迁移规约 / 中文注释 / 空白）已通过。待推送 PR、远端 CI、合并。
- 下一步（精确到动作/命令）：1. 推送分支并创建 PR；2. 等远端 CI 全绿后合并；3. 清理分支 / worktree 后继续 SYS-05 PR2。
- 相关文件 / 测试 / 坑：当前 PR 不处理离线许可、重试、死信、回放和 `sys_task_dead_letter`，不得提前勾选 SYS-05 FR-4/5/6 或 AC-3/4/5。`sys_task` 是本卡权威表名，`scripts/migration-convention-guard.mjs` 已加入精确 allowlist（只允许 `sys_task` / `sys_task_dead_letter`，不放宽通用表名规则）。外部环境类阻塞写入 [待处理问题清单](audit/deferred-issues.md) 后继续主线；登录可用、权限隔离、真实性门禁、医疗安全、当前卡主链路缺陷不得延期。
...
> 末次更新：2026-06-01 · SYS-03 PR2 已通过 #225 合入 `origin/main`（merge `8e6063a`）并清理分支 / worktree；当前在 `codex/sys-05-runtime-modes-pr1` 推进 SYS-05 PR1，在线 / 异步 / 批量任务框架、`sys_task` V41 五方言迁移、任务提交/查询端点、服务契约和领域 owner 已完成红绿实现；聚焦契约验证、后端全量 `mvn -B -q test`（756 tests，含 PostgreSQL 15 + Oracle 21 Testcontainers V41）与提交后 changed T-GATE 已通过，下一步推送 PR。达梦 / 人大金仓真实环境适配保持 `DEFER-001`，前端依赖审计与非阻断输出噪声登记 `DEFER-002` / `DEFER-003`，本机浏览器截图超时登记 `DEFER-004`；长期目标保持 active，open 待处理问题只登记不阻断主线。
