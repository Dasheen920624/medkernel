...
- OpenSpec `tasks.md` 已收敛为 205 个可供后续 AI 顺序执行的原子任务（15 完成、190 待执行）；覆盖缺口总账、首次信任根、签发者密钥隔离、自包含 `.mkp`、16 语义族医疗资源工厂、离线依赖仓、openEuler 空机部署、目标工具本地实现、134 一次确认部署、全量资源生产、医院复制和知识源迁移。
...
1. 在 `codex/launch-ledger` 按 TDD 执行 2.9：为 `TEST` 缺口强制真实可执行观察、观察码和非静态断言，补齐受影响契约/E2E 后重跑归零。
2. 依次执行 2.10-2.12 的数据、环境缺口闭环和严格归零总账；每个 checkbox 独立验证、提交并同步本文件。
