...
- **主线「全流程演练·使用指南·体验重构」：幕0–10 L1/L2 与总验收报告已全部合入 main；当前分支正在做 P1 体验实现线第一项「提醒与推荐中枢 / 推荐链路一张图」**（计划 #526；DOC-SYNC #528；幕0–8 = #529–#537；幕9 = #538 `3e4ba441`；实现路径纠偏 #539 `0b0d4cb2`；幕10 L1 = #540 `efbfd9ab`；幕8.5 第一批 = #541 `6aaba08b`；幕8.5 第二批 = #542 `859477f`；幕8.5 第三批 = #543 `516ad753`；幕10 L2 = #544 `dde82115`；总验收与培训材料 #545 `cd2268af`；当前分支 `codex/demo-drill-recommendation-hub`）。总体计划见 [docs/superpowers/plans/2026-06-10-full-flow-drill-usability-program.md](superpowers/plans/2026-06-10-full-flow-drill-usability-program.md)。
...
- **P1 体验实现进展（当前分支）**：`/cdss/fatigue` 已本地升级为「提醒与推荐中枢」，新增推荐链路总览、患者 / traceId / 来源对象本页检索、抽屉内「这条推荐是怎么来的」七段链路（触发事件→命中规则→知识来源→路径上下文→待办 / 通知→医生反馈→药师复核），并修正移动端筛选表单布局。本地验证已通过：`npm run verify`（84 文件 / 619 测试）、`npm run build`、changed-mode T-GATE、真实 dev 后端 + 浏览器桌面 / 390px 移动可视检查；仍需 PR / CI / 合并后部署 134 复验，才能把 `OPT-IA-01` / `OPT-TRACE-01` 记为现场销项。

## 当前状态

- main：`cd2268af`（总验收与 3 本角色培训 #545 已合）；当前分支 `codex/demo-drill-recommendation-hub` 承载 `OPT-IA-01` / `OPT-TRACE-01` 第一批前端体验实现。
...
1. 收口当前分支：提交 `codex/demo-drill-recommendation-hub`，推送 PR，CI 通过后合并；合并后部署 134 并用医生账号复验 `/cdss/fatigue` 桌面 / 移动可见性。
2. 继续体验实现线，优先级：`OPT-VIS-01`（规则自然语言与只读流程图）、`OPT-VIS-02` + `OPT-PATH-UI-01`（医生只读路径图）、`UI-ACT10-AUDIT-01`（traceId 搜索与诊断链跳转）。
3. P1 体验项合入并在 134 复验后，发起独立指南验收；六项全绿后再按总体计划 §8 备份、清库、重迁和 health 校验，最后在 `_HANDOFF.md` 宣告进入第二阶段。
