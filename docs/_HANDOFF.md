...
- OpenSpec `tasks.md` 已收敛为 205 个可供后续 AI 顺序执行的原子任务（22 完成、183 待执行）；覆盖缺口总账、首次信任根、签发者密钥隔离、自包含 `.mkp`、16 语义族医疗资源工厂、离线依赖仓、openEuler 空机部署、目标工具本地实现、134 一次确认部署、全量资源生产、医院复制和知识源迁移。
...
1. 在 `codex/launch-ledger` 按 TDD 执行 3.4：为每个发布实例创建独立、不可变的 `issuerInstanceId` 与独立 `keyId`，禁止两个实例复用密钥身份。
2. 3.4 独立提交后按依赖顺序完成外置密钥边界、首次信任根、3.8 唯一活动签发者 CAS、密钥围栏、不可变注册表和可信迁移。
