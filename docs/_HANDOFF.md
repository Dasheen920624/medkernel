...
1. 完成 D0–D6 卡片到功能、页面、菜单、API、数据与演练证据的完整覆盖矩阵，缺口逐项登记并能当场补齐的直接补齐。
2. 先修复嵌入宿主链路、沙盘业务系统模拟器、术语冲突处置、院内主数据同步接口和未闭环整改任务，再处理覆盖矩阵发现的其他功能与体验问题。
3. 完成 134 统一真实复演、备份恢复、降级、全量测试、五方言和 T-GATE 后，才可重新形成第一阶段最终验收；正式知识生产继续阻断，直到文献资料库受管根地址完成真实配置与独立验收。
...
5. P5 与第一阶段正式验收通过、可追溯基线确认后，才可在系统配置页维护正式文献资料库受管 URI 并进入 P6。
