...
- **主线「全流程演练·使用指南·体验重构」：幕0–10 L1 已全部合入 main，幕8.5 第一批（幕0–2）已合入 main，当前分支正在收口幕8.5 第二批（幕3–5）前台重演**（计划 #526；DOC-SYNC #528；幕0–8 = #529–#537；幕9 = #538 `3e4ba441`；实现路径纠偏 #539 `0b0d4cb2`；幕10 L1 = #540 `efbfd9ab`；幕8.5 第一批 = #541 `6aaba08b`；当前分支 `codex/demo-drill-act85-ui-replay-acts3-5`）。总体计划见 [docs/superpowers/plans/2026-06-10-full-flow-drill-usability-program.md](superpowers/plans/2026-06-10-full-flow-drill-usability-program.md)。
...
- **幕8.5 第二批实证（当前分支）**：`node scripts/drill/act85-ui-replay-acts3-5.mjs` 已在 134 真实前台完成幕3–5 复演，截图落幕3–5 `ui-replay/`；幕3 3 步、幕4 4 步 / 8 图、幕5 4 步 / 7 图，全部带 URL 标头。新增/细化缺口：`OPT-KNOW-UI-01`（知识治理页不生成候选，不能前台登记知识源/版本/退役）、`OPT-VIS-01`（规则可读预览仍有字段路径/技术 ID，医生无规则库只读入口）、`OPT-VIS-02`（医生端无整条 CAP 路径只读图）、`OPT-PATH-UI-01`（已发布模板缺“复制为新版本”入口）。手册、计划 §6.5 和能力可见性矩阵已按当前分支回填，待验证、提交、PR、CI、合并。

## 当前状态

- main：`6aaba08b`（幕8.5 第一批 #541 已合）；当前分支 `codex/demo-drill-act85-ui-replay-acts3-5` 已承载幕8.5 第二批（幕3–5）前台客户走查、截图、四问审计、指南配图和能力可见性矩阵增量，下一步是验证、提交、PR、CI、合并。
...
- 指南现状：4 本手册 + 第三方对接案例集有实质内容且章节结构合规；合规运维手册已补幕0 UI 复演图和幕10 L1 审计/权限/脱敏/审批/降级章；试点准备手册已补幕1–4 UI 复演图和幕2/3/4缺口；临床运行手册已补幕5 UI 复演图和路径可读性缺口；幕6–10 对应章节仍需按幕8.5第三批与幕10 L2 补页面复演截图；3 本角色培训手册仍为空骨架。

## 下一步

1. **先收口当前分支**：运行验证（JSON 可解析、脚本语法、截图尺寸、T-GATE、diff check、中文注释），提交 `codex/demo-drill-act85-ui-replay-acts3-5`，推送 PR，等 CI 通过并合并。
2. **幕8.5 第三批（幕6–9）**：基于最新 `origin/main` 新建分支；医生必须在前台待办/通知/推荐卡页面真实接收→处理→闭环；随访/质控要前台处理预警；配置包发布治理要复测 `OPT-PKG-01`；适配器健康状态页必须能让客户读懂。
3. 幕10 L2「合规、审计与降级」按 §2.5 执行契约补做前台走查、截图、四问审计和指南配图；幕8.5 + 幕10 L2 都收口后才进入 §1 六判据核验。
