...
- 当前最新验证：后端受影响回归 74/74；前端 `npm run verify` 为 116 文件、2210 测试全绿，生产构建成功；RC/release 工具全套 111/111（含短连接、瞬断重试、身份错配立即阻断和错误因果用例）；真实性、配置、迁移、性能、中文 Javadoc/迁移注释门禁通过；本次 OpenSpec strict、Prettier 与 `git diff --check` 通过。以上不能替代下一个候选的完整 clean RC0。
...
- 当前工作树基于已作废的 `c369a4be2`，仅允许提交本次候选探针可靠性修复和同步文档形成全新候选；不得把构建残留或旧 RC 证据纳入候选。

## 下一执行序列

1. 完成本次候选探针可靠性复核，运行 RC 工具全套、OpenSpec strict 和格式检查，清除 `target/dist/test-results/tsbuildinfo` 等残留，只暂存本逻辑单元文件并提交形成全新候选；提交后任何源码或合同漂移都必须形成新候选并废止旧证据。
