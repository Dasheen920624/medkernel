...
- OpenSpec `tasks.md` 已收敛为 205 个可供后续 AI 顺序执行的原子任务（13 完成、192 待执行）；覆盖缺口总账、首次信任根、签发者密钥隔离、自包含 `.mkp`、16 语义族医疗资源工厂、离线依赖仓、openEuler 空机部署、目标工具本地实现、134 一次确认部署、全量资源生产、医院复制和知识源迁移。
...
1. 在 `codex/launch-ledger` 按 TDD 执行 2.7：建立唯一缺口分类器，让每项缺口只归 `IMPLEMENTATION`、`TEST`、`DATA`、`ENVIRONMENT` 之一并绑定可执行 ownerPath。
2. 2.7 归零后依次执行 2.8-2.12 的四类缺口闭环和严格归零总账；每个 checkbox 独立验证、提交并同步本文件。
