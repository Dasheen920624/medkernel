...
- 当前阶段结论：134 当前仍运行上一轮已验收候选
  `2de9ebf069292aa2331c649eee8dcdb95b3ddba4`；本地分支已继续完成多轮全局体验与契约清理，最新
  本地候选尚未清库部署到 134。下一步必须先用最新本地 HEAD 重新构建候选交付物并在 134 清库部署，
  按既有八段全系统演练走通数据路线；通过后再进入全角色、全流程、全视角真实前台操作和产品优化。
  不能把上一轮 134 证据、本地薄切片或脚本式演练误记为最新候选的完整前台真实演练完成。
...
- 本地分支最新候选必须用 `git rev-parse HEAD` 在构建前读取 40 位提交哈希；若更新本文并再次提交，
  以新的 HEAD 为准，不要沿用上一条提交或旧 134 manifest。
- 最新候选尚未重新部署 134；构建后端 jar、前端 `dist.tar.gz` 和部署脚本时必须绑定同一个本地 HEAD。
- 后端全量测试必须优先使用 `mvn -q -f medkernel-backend/pom.xml clean test` 生成干净 Surefire 报告。
  曾发现 `mvn test` 返回 0 但 `target/surefire-reports` 残留已删除旧类
  `DeveloperConsoleControllerTest` 的 21:33 失败报告；根因是旧报告残留，不是当前运行失败。后续 AI 不得用
  未清理的 `target` 报告判断最新候选。
...
- 客户面旧态、未来态和模拟态口径已继续按全局产品体验收束：首次接管、模型服务、MPI 合并、沙盘历史重放、
  ...
  `bash scripts/check-comment-zh.sh --mode=full`、`git diff --check`；尚未重新部署 134。
- 知识生产入口已继续按 MedKernel 智能中枢口径收束：`/knowledge/production`、前端页面、后端菜单目录和
  可复现功能目录统一命名为“知识生产”，不再以“模型生产控制台”把该链路误导成独立模型平台；页面文件同步
  重命名为 `KnowledgeProductionPage`，路由契约改为配置型知识生产工作流，真实性门禁新增模型平台化旧名称
  拦截。已验证：
  `npm test -- --run src/shared/config/routes.test.ts src/shared/config/menu.test.ts src/shared/config/productCatalog.test.ts src/pages/knowledge-production/KnowledgeProductionPage.test.tsx src/pages/quality/KnowledgeGovernance.test.tsx`、
  `node --test scripts/authenticity-guard.test.mjs`、
  `mvn -q -f medkernel-backend/pom.xml -Dtest=MenuPermissionCatalogTest,D0DomainAcceptanceTest,DefaultPermissionPolicyTest test`、
  `npm run typecheck`、`npm run lint`、`npm run stylelint`、`npm run format:check`、`npm run test:lint-rules`、
...
- 部署演练权威文档、后端生产注释和上线覆盖矩阵已继续同步为“管理与生产工作区”口径，
  ...
  `git diff --check`；尚未重新部署 134。
- 最新候选上线前全量门禁已在本地通过且报告已清理：release/迁移/配置/沙盘/知识/韧性脚本测试
  `98 passed`，`node scripts/db/generate-migrations.mjs --check`、迁移与配置 inventory、Shell 断言校验通过；
  `npm run verify` 通过，前端 `109 files / 803 tests passed`；`mvn -q -f medkernel-backend/pom.xml clean test`
  通过，干净 Surefire 汇总 `3042 tests / 0 failures / 0 errors / 7 skipped`，7 个 skipped 来自本机
  Docker/Testcontainers 不可用条件用例；工作区只剩未跟踪 `.codex/config.toml`，不要提交。
  最新候选尚未重新部署 134。
...
1. 先完成最新本地候选的交付物构建与 134 清库重部署：后端 jar、前端 `dist.tar.gz`、部署脚本、nginx 配置、
   service unit 和 `LAUNCH_SOURCE` 必须绑定同一个最新 HEAD；业务表数量从
   `medkernel-backend/src/main/resources/db/schema/medkernel.schema.json` 读取，Flyway 版本从当前迁移读取，
   不硬编码旧证据；服务器模型信息从 `/zoesoft/mimoModel` 获取配置但不得输出密钥。
2. 按现有模式完成最新候选八段全系统演练和发布后独立验收，先把数据路线走通，避免基础问题在真实前台体验阶段
   反复干扰；演练访问 134 自签 SAN 证书只追加 `NODE_EXTRA_CA_CERTS`，不设置 `SSL_CERT_FILE`。
3. 现有模式演练通过后，进入全角色、全流程、全视角真实前台体验与产品优化：医生、护士、药师、医技、
   患者/随访、平台管理员、医疗引擎运营员、审计员、医疗实施工程师、信息科长、院长、医疗产品经理等都要体验。
   发现功能分类不合理、流程过长、理解困难、六态不足、页面空壳、权限误导、患者信息安全表达不清或医疗安全
   边界不清时，直接按最符合真实医疗产品的方案优化。
4. 将脚本式全链路已通的路线转为更多真实前台操作：尽可能通过前台创建机构、账号、来源、患者资源、字典映射、
   知识候选、版本发布、机构生效版本、沙盘运行、临床调用、审计与恢复证据；脚本只作为辅助校验，不再作为
   唯一数据来源。
5. 继续扩展公网/外网患者信息使用体验：字段出域预览、手工用途确认、审计回看和知识生产真实任务阻断确认闭环
   已完成本地薄片并随 134 清库演练验证；还要补强不同部署模式默认策略和更多前台真实操作证据，使其可理解、
   可检查、可追溯。
6. 按“证据详情新定义”继续回扫历史页面：知识审核候选来源溯源和全局证据详情偏好已完成；其它旧页面如仍存在
   生硬身份化开关、孤立技术入口或把关键安全/审计/业务判断证据藏起来的设计，后续都要改成上下文里的渐进证据、
   诊断信息或变更明细。
7. 本地临床协同任务、随访协同体验薄片、沙盘场景目录语义清理、退役工具主域文本清理、性能压测契约、
   ...
   134 已验收。
8. 继续清理旧兼容、冗余设计和误导性历史事实；`.codex/config.toml` 是本地未跟踪文件，不要纳入提交。
