...
- OpenSpec `tasks.md` 已收敛为 205 个可供后续 AI 顺序执行的原子任务（20 完成、185 待执行）；覆盖缺口总账、首次信任根、签发者密钥隔离、自包含 `.mkp`、16 语义族医疗资源工厂、离线依赖仓、openEuler 空机部署、目标工具本地实现、134 一次确认部署、全量资源生产、医院复制和知识源迁移。
...
1. 在 `codex/launch-ledger` 按 TDD 执行 3.2：先让数据库合同因七张权威表缺失而失败，再只扩展唯一 `medkernel.schema.json` 并由生成器同步五方言 V1；不得手写平行迁移或跳过中文 COMMENT、租户、版本、审计和唯一约束。
2. 3.2 独立提交后继续 3.3 单活动签发者唯一约束与并发争抢测试，再按依赖顺序完成首次信任根、密钥围栏、不可变注册表和可信迁移。
