...
- **2026-06-16 最新状态（覆盖下条长任务旧尾句）**：`codex/wave2-knowledge-model-readiness` 已完成 AIK-STD-13/14 模型/Agent 生产器接入、前端生产中心只读证据面与 Task 22 最终工程验证：`ModelKnowledgeProducer` 只经模型网关生成候选，配置齐全时可产 AI 候选进入同一候选池/门禁/8 态分流/影子评测/审核链，readiness 缺模型、文献根、评测、白名单、三元组或 P6 验收时均结构化阻断且不调用模型、不造候选；DATASVC 新增 `submitProductionCandidate` 受控回写工具，CLI `agent submit-candidate` 与 MCP `payload` schema 已接线；前端 `知识生产` tab 已展示 readiness、生产 job、候选血缘、门禁、分流、影子评测、共存替换提醒。已合并 `origin/main=a3c132de`（#632 B0 后续全系统深查整改与门禁补强）入本分支：文本冲突已解（hooks 导入两边并存、生产中心安全测试两边并存、本接力文档以本条为准），语义冲突已修（#632 把 `listCandidates` 改为分页，`ControlledToolService.existingCandidates` 已改为按分页 API 翻页取全量候选）。**验证**：后端全量 `mvn test` 通过、前端全量 95 文件/740 用例通过、CLI 28/MCP 16 通过、typecheck/真实性/配置/迁移/中文注释/diff check 通过。**下一步**：经 PR [#633](https://github.com/Dasheen920624/medkernel/pull/633) 合并入 main；仍未宣称 P6 真实外部 provider 现场放行、Agent 中止/纠偏等协同控制完整闭环或 134 部署。
...
- **当前主线**：P5 **第一阶段 B0 主链路已收官**（PR #600 + 复核 #603）。B0 第一阶段全系统核查与完美化整改 **#629 已合并入 main `9cd3a4f4`**；最新 `origin/main` 已到 **`a3c132de`**（#630 AIK-STD-04 候选生成 PR1、#631 AIK-STD-05 候选安全门禁 PR1、#632 B0 后续全系统深查整改与门禁补强均已合并），本分支已并入其全部成果。**2026-06-16 用户已明确指令：恢复 wave2 P2-C 内容管线推进**（覆盖此前 B0 暂停）——续 `AIK-STD-03 术语`（核查＝TERM-01 + #629 `TerminologyCandidateGenerationJob` 已实质建成，第三次「别建重复表」命中）/ `AIK-STD-04 候选生成`（**PR1 已合并**）/ 续 `AIK-STD-05` 11 项门禁 / `AIK-STD-10` 8 态去重。后续 B0 深查与 P2-C 并行推进，按卡 TDD。
