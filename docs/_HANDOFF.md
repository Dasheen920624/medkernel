...
- OpenSpec `tasks.md` 已收敛为 205 个可供后续 AI 顺序执行的原子任务（18 完成、187 待执行）；覆盖缺口总账、首次信任根、签发者密钥隔离、自包含 `.mkp`、16 语义族医疗资源工厂、离线依赖仓、openEuler 空机部署、目标工具本地实现、134 一次确认部署、全量资源生产、医院复制和知识源迁移。
...
1. 在 `codex/launch-ledger` 按 TDD 执行 2.12 的严格归零总账：任何失败、未知、跳过、缺证、未解释失败或人工豁免非零时均禁止 `PASSED`，并把 2.8-2.11 四类结果绑定到同一候选与运行标识。
2. 完成 2.12 后独立提交并同步本文件，再进入 3.1 平台知识权威领域模型；不得在总账未严格归零时提前创建发布权威或跳到资源工厂。
