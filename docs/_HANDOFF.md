...
- P9-1 PR #470 已创建：路径模板新增 `entryMode`，入径/出径纳入和排除条件复用真实递归条件树；自动建议入径严格校验纳入/排除，人工确认入径允许纳入未命中但仍阻断排除命中；出径按真实快照校验条件树。
...
- 前端覆盖率：首轮远端 CI #27099367283 的 `frontend-build-test / Unit tests with coverage` 因 `PathwayTemplates` 两个高交互用例 30s 超时失败；已将测试改为稳定受控输入，本地 `npm run test:coverage` 退出 0，`78` 文件 / `542` tests 通过。
...
1. 推送 PR #470 的覆盖率超时修复，等待远端 CI 全绿并合入 `main`。
