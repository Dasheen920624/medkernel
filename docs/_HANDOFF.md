...
- 全角色核查已过（剧本引用实证：路由 21/21、Controller 48/48、契约文档全实存；71 Controller/44 页面/14 内置角色实测校准）。**实锤新缺口 OPT-ROLE-01**：内置角色无「检验技师/临床药师」（`RoleCode` 枚举锁死），演练以专科专家代配，幕1 验证后定方案；幕1 账号扩为 9 个（补医保办/护士戏份），幕4 R3 归医保办、幕7 随访填报归护士。
