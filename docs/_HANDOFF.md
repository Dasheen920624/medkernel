...
- 真实前台继续演练补充（`2026-06-29T22:43:13+08:00`，已按当前账号契约校正）：
  ...
  - 已本地修复：运行字段目录解析与草稿生成会把空白字段说明规范化为
...
- 模型能力双模式患者上下文边界优化（`2026-06-30T10:31:00+08:00`）：
  - 背景：用户补充院外模型配置位于 134 的 `/zoesoft/mimoModel`，该文件为 `root:root`、`600`；
    后续不得读取、展示或写入密钥内容。产品口径已确认：公网/外部模型可在授权用途内使用患者上下文，
    但必须屏蔽核心敏感标识、字段预览、责任确认并审计；院内/本地模型可按授权使用必要患者信息，
    仍需留处理边界、调用摘要和审计，日志/证据不留患者明文。
  - 产品决策：不恢复旧“专家模式 / 查看技术细节”表达；`/advanced/ai-workflows` 默认表格新增
    “数据边界”列，用业务语言直接说明公网、院内或双路径模型的数据使用边界。能力代码、策略作用域、
    输出格式明细仍放在受控“追溯证据”内。
  - 路由体验契约同步：
    `/knowledge/production` 与 `/advanced/ai-workflows` 的模型安全负责人职责统一为“确认/核查院内/公网模型患者上下文使用、
    字段预览和用途确认”；边界统一为“公网模型屏蔽核心敏感标识；院内模型按授权使用必要信息并保留处理边界”。
  - TDD 红灯：
    - `npm --prefix frontend test -- AiWorkflows.test.tsx -t 本地模型能力说明授权使用必要患者信息但日志证据不留明文`
      初次失败，页面缺少本地模型行级数据边界说明。
    - `npm --prefix frontend test -- routes.test.ts -t '高风险治理与运维|上线配置与知识建模'`
      初次失败，路由契约仍是旧“公网模型”单一路径表述。
  - 绿色验证：
  ...
  - 直接访问 `https://193.112.107.134/medkernel` 的 8 段真实前台操作均通过，
    每段 `browserErrors=0`、`serverErrors=0`、`networkFailures=0`。
- 前端全量 verify 红绿闭环（`2026-06-30T10:52:34+08:00`）：
  - 红灯：`npm --prefix frontend run verify` 先后暴露
    `Mpi.tsx` 页面硬编码病种示例 / 默认诊断编码、`Followup.tsx` 嵌套三元表达式、
