...
### 线 1 · BASE-09 后端包回滚目标状态净化 PR10 🚧

- 类型：软件开发
- 分支：codex/base-09-rollback-offline-target
- 目标：完成 BASE-09 第十批后端净化：配置包回滚目标只允许曾经执行并已下线的 `OFFLINE` 历史版本，禁止 `PUBLISHED` 预发布包绕过正式发布流程被直接激活。
- 状态：已基于 `origin/main@02f18cf` 完成实现与完整本地验证：`PackageEngineService.rollbackPackage` 将回滚目标状态从 `PUBLISHED 或 OFFLINE` 收紧为仅 `OFFLINE`；新增 `rollbackPackageRejectsPublishedTargetAndKeepsStatus` 红绿测试；`ConfigPackages` 回滚弹窗只展示已下线历史版本，并清理 `PUBLISHED` 可快速回退的误导文案；新增审计记录 `docs/audit/BASE-09-backend-package-rollback-target-pr10.md`，并更新 BASE-09 执行记录。当前已通过后端目标红绿测试、`PackageEngineServiceTest` 全量、前端 typecheck/verify/build、真实性 inventory、三类脚本门禁测试、`mvn -B -q test`（含 Docker Testcontainers PostgreSQL / Oracle 迁移烟测）、`git diff --check` 与生产路径旧回滚目标 grep。
...
- 相关文件 / 测试 / 坑：本 PR 仍不宣称 BASE-09 / PKG-01 完成，不勾选全部 FR/AC；`PUBLISHED` 代表已投影但不等于“曾经在用”，不能作为历史回滚点；后续 AI 必须继续遵守纯净代码原则：发现假成功、旧占位、死代码或临时兼容层，能删则删，不能删必须写清风险和删除点。
...
> 末次更新：2026-05-31 · BASE-09 后端包回滚目标状态净化 PR10 已完成完整本地验证，待提交 / PR / 远端 CI；D0 域级验收前不得启动 D1 新功能 PR
