...
### 线 1 · SYS-03 投影降级与审计 PR2 🚧

- 类型：软件开发 / 后端投影同步
- 分支：`codex/sys-03-degrade-audit-pr2`（基于 `origin/main` 409eda5）
- 目标：按 [SYS-03](cards/D0/SYS-03.md) 大卡工序交付 PR2：投影可关闭 + 诚实降级 + Dify 执行器边界 + 同步审计，收口 FR-3/4/5 与 AC-2/3/4。
- 状态：已完成 TDD 红绿主实现：新增 `ProjectionRuntimePolicy` 统一读取配置中心 `graph-projection` / `dify-workflow` 运行开关；新增 `/api/v1/projections/clinical-graph/status`、`ProjectionRuntimeStatusResponse` 与 `ProjectionClinicalStatusPort`，图投影关闭时临床主链路诚实返回 `NOT_SYNCED`；新增 `ProjectionExecutionPort` / `ProjectionExecutionCommand` / `NoopProjectionExecutionPort`，Dify 只接收 syncId/sourceHash/count/traceId 等元数据，未接真实执行器时返回 `NOT_SYNCED`；`ProjectionSyncService` 在成功、失败、关闭等终态调用 `AuditRecorder` 记录 `mk_projection_sync` 真实结果。红测 `ProjectionRuntimeDegradeTest` / `ProjectionDifyExecutorBoundaryTest` / `ProjectionAuditTest` 已先失败后转绿；聚焦回归 `ProjectionSyncServiceTest`、`ClinicalGraphProjectionSourceTest`、`StandardClinicalAuthorityServiceTest`、`ServiceContractGovernanceTest`、`OpenApiContractConfigurationTest` 已通过；后端全量 `mvn -B -q test` 通过 751 tests，PostgreSQL 15 + Oracle 21 Testcontainers 迁移至 V40；提交前真实性 / 配置边界 / 迁移规约 / 中文注释 / 空白门禁通过。待提交、PR、CI、合并。
- 下一步（精确到动作/命令）：1. 提交 `完成 SYS-03 投影降级与审计 PR2`；2. 推送分支并创建 PR；3. 远端 CI 全绿后合并到 `origin/main`；4. 清理分支 / worktree，再领取下一卡。
- 相关文件 / 测试 / 坑：计划见 `docs/superpowers/plans/2026-06-01-sys-03-degrade-audit-pr2.md`。核心触点为 `medkernel-backend/src/main/java/com/medkernel/engine/projection/*`、`NoopClinicalProjectionStatusPort`、`RuntimeOperationsService` 现有 `graph-projection` / `dify-workflow` 口径、`AuditRecorder` / `AuditRecordCommand`。Dify 没有真实外部环境时只能返回 `NOT_SYNCED`，不得伪造执行成功；达梦 / 人大金仓真实环境证据继续登记 `DEFER-001`，不阻塞 PR2。新增外部阻塞必须登记 [待处理问题清单](audit/deferred-issues.md) 后继续主线；若触及当前卡主链路、登录可用、权限隔离、真实性门禁或医疗安全红线，则即时处理。
...
> 末次更新：2026-06-01 · SYS-03 PR1 已通过 #224 合入 `origin/main`（merge `409eda5`）并清理分支 / worktree；当前在 `codex/sys-03-degrade-audit-pr2` 推进 SYS-03 PR2，投影关闭诚实降级、Dify 执行器边界、投影同步审计和状态端点已完成红绿实现，后端全量与提交前 T-GATE 已通过，下一步提交、推送 PR、等远端 CI 后合并。达梦 / 人大金仓真实环境适配保持 `DEFER-001`，前端依赖审计与非阻断输出噪声登记 `DEFER-002` / `DEFER-003`，本机浏览器截图超时登记 `DEFER-004`；长期目标保持 active，open 待处理问题只登记不阻断主线。
