...
- 用户已暂停“持续推进”并要求校准是否走偏。当前判断：方向仍是全角色真实前台、真实服务链路和上线门禁，
  但继续按单个代表切片或单个附件扩门禁会有过度局部化风险。下一步必须改为“上线总账驱动”：
...
- 第二百五十三批本地推进：继续按“上线总账驱动”推进 `PRODUCT_SCOPE.md` §15 第 6 项 S0-S40 五态演练矩阵。
  ...
  `LIS_MONITORING_CRITICAL` 接入保持 `NOT_CONNECTED` 诚实断连，但本地 ICU 生命支持上下文、机械通气、升压药运行、
  当前机构生效版本推荐、医生人工升级确认和待办闭环继续成立。明确不声明完整 S27、完整 ICU 系统、完整生命支持系统、
  完整 LIS 系统、完整监护设备平台、真实外部成功联通、自动开嘱、自动转 ICU、设备控制、自动调整呼吸机、完整 S0-S40、
  134 清库、目标环境部署或完整上线验收。
- 第二百五十三批实现细节：`frontend/e2e/critical-emergency-icu-frontdesk.spec.ts` 的附件新增
  `criticalCareDegradationEvidence`，固定
  `source=CRITICAL_ICU_LIFE_SUPPORT_NOT_CONNECTED_LOCAL_ESCALATION_CONTINUES`，并绑定本轮
  `onboardingHealthStatus/systemFamilyCode/runtimeReleaseId/contextSnapshotId/ventilatorMode/vasopressorRunning/procedureCode/`
  `recommendationCardId/feedbackId/todoId`，其中 `procedureCode` 必须绑定机械通气 `5A1955Z`；同时要求
  `requiresPhysicianConfirmation=true`、`noAutoOrder=true`、`noAutoTransfer=true`、`noDeviceControl=true`、
  `noAutoVentilatorChange=true`、`noExternalSuccessClaim=true`、`auditVerified=true`、`permissionVerified=true`、
  `sixStateBoundaryVerified=true` 和明确 scope 边界。`frontend/e2e/support/launchCoverageEvidence.ts` 将
  `S27__DEGRADATION` 纳入 `criticalEmergencyIcuScenarioConditionRows`，新增
  `hasCompleteCriticalEmergencyIcuS27DegradationEvidence()` 与 `hasCriticalEmergencyIcuS27DegradationScopeBoundary()`：
  只有显式条件行、结构化 S27 降级证据、接入断连、本地 ICU 生命支持上下文、机械通气 Procedure、当前 runtime 推荐、
  医生人工确认、升级待办、LIS 监护消费者切片和审计 / 权限 / 六态安全边界同时成立，才声明该行。
  `frontend/src/test/e2eLaunchCoverageEvidence.test.ts` 先红后绿覆盖：正例声明
  `[S19__HIGH_RISK,S24__HIGH_RISK,S27__HIGH_RISK,S27__DEGRADATION]`；缺显式 S27 降级条件附件、source 错、证据为空、
  缺结构化证据、结构化 source 错、接入健康状态不是 `NOT_CONNECTED`、系统族错、缺机械通气模式、缺升压药运行证据、
  缺机械通气 Procedure、结构化 `procedureCode` 错绑到非 `5A1955Z` Procedure、推荐未绑定当前 runtime、结构化推荐卡错绑、
  反馈不是医生人工确认、待办未完成、允许自动开嘱 / 自动转 ICU / 设备控制 / 自动调整呼吸机、伪造外部成功、
  缺审计 / 权限 / 六态、scope 冒领完整 S27，均不得声明 `S27__DEGRADATION`。
...
- 第二百五十三批验证证据：已先让
  ...
  `S27__DEGRADATION`，`scenarioConditionRows` 为 `undefined`）；实现后通过
  `npm --prefix frontend run test -- e2eLaunchCoverageEvidence -- --run`（1064 tests）、
  `npm --prefix frontend run test -- e2eLaunchCoverageEvidence e2eAuthCredentialContract -- --run`（1132 tests）、
  `npm --prefix frontend run typecheck -- --pretty false`、`npm --prefix frontend run format:check`、
  `node --test scripts/release/full-system-rehearsal.test.mjs scripts/release/launch-coverage-audit.test.mjs`（15 tests）。
  并行只读 explorer 发现结构化 `procedureCode` 需强制绑定 `5A1955Z`，已补负例和 parser 强校验后重跑上述门禁通过。
  真实 E2E 临时启动本地后端 18102（dev/H2，既有 jar）和前端 5175（本批已停止；复核 18102/5175 无监听），最终执行
  `E2E_EXTERNAL_DEPLOYMENT=1 E2E_BASE_URL=http://localhost:5175 E2E_API_BASE_URL=http://localhost:18102/medkernel/api/v1 MEDKERNEL_API_PROXY_TARGET=http://localhost:18102 E2E_EXPECT_MFA_DISABLED=1 E2E_EVIDENCE_DIR=/tmp/medkernel-e2e-s27-degradation-condition-row-20260710-r2 npm --prefix frontend run e2e -- --project=chromium critical-emergency-icu-frontdesk.spec.ts`
  通过；`/tmp/medkernel-e2e-s27-degradation-condition-row-20260710-r2/report/results.json` 读回 `status=PASSED`、
  `expected=1`、`unexpected=0`、`flaky=0`、`skipped=0`，`launchCoverage.scenarioConditionRows` 为
  `[S19__HIGH_RISK,S19__DEGRADATION,S24__HIGH_RISK,S24__DEGRADATION,S27__HIGH_RISK,S27__DEGRADATION]`。
  附件抽查 `criticalCareDegradationEvidence.source=CRITICAL_ICU_LIFE_SUPPORT_NOT_CONNECTED_LOCAL_ESCALATION_CONTINUES`、
  `onboardingHealthStatus=NOT_CONNECTED`、`systemFamilyCode=LIS_MONITORING_CRITICAL`、`ventilatorMode=SIMV`、
  `vasopressorRunning=true`、`procedureCode=5A1955Z`、`requiresPhysicianConfirmation=true`、
  `noAutoOrder/noAutoTransfer/noDeviceControl/noAutoVentilatorChange/noExternalSuccessClaim=true`、
...
- 第二百二十九批并行只读审计结论：本批复用并关闭 6 个已完成只读子代理，均未编辑、未暂存、未提交、未启动服务、未外调。
  ...
  `docs/DEPLOYMENT_AND_REHEARSAL.md` 与 `test-results/` 仍不要回滚、不要暂存；`/tmp` E2E 产物不要提交。
- 第二百三十批本地推进：继续按“上线总账驱动”减少 `PRODUCT_SCOPE.md` §15 第 6 项 S0-S40 五态总账缺口，
  本批只把科研、真实世界与数据服务在第三方系统族接入中的“科研伦理数据平台已登记，但真实消费者和标准资源缺失”
  升级为显式 `S34__MISSING_DATA`。`third-party-system-families-rehearsal.spec.ts` 仍由平台管理员真实前台逐类登记
  13 类第三方系统族、创建接入申请、执行健康诊断并生成数据质量报告；本批仅为 `RESEARCH_ETHICS_DATA` 追加
  `researchEthicsDataMissingEvidence` 和显式 `scenarioConditionEvidence`，来源固定为
  `RESEARCH_ETHICS_DATA_CONSUMER_AND_STANDARD_RESOURCE_MISSING`。只声明 `S34__MISSING_DATA`；不声明
  `S34__NORMAL/ABNORMAL/HIGH_RISK/DEGRADATION`，不冒领完整科研数据服务、真实世界研究、脱敏队列生成、伦理授权审批回传、
  数据集导出、使用审计闭环、完整第三方系统族覆盖或完整上线验收第 12 项。
- 第二百三十批实现细节：`frontend/e2e/support/launchCoverageEvidence.ts` 新增
  `thirdPartySystemFamilyScenarioConditionRows` 和 `collectThirdPartySystemFamilyScenarioConditionClaims()`；
  只接受 `third-party-system-families-rehearsal.spec.ts` 的 `third-party-system-family-codes` 附件，且测试标题必须是
  “逐类登记第三方系统族接入并验证断连诚实降级”。parser 要求 `systemFamilyCodes` 含 `RESEARCH_ETHICS_DATA`；
  `scopeStatement` 必须否定真实消费者、标准资源、闭环回传、完整科研数据服务和完整 S34；`registrationEvidence`
  必须证明 13 类适配器、断连 / 缺口摘要和非空质量报告；`researchEthicsDataMissingEvidence` 必须绑定同一
  ...
  `DE_IDENTIFIED_COHORT/ETHICS_AUTHORIZATION/DATASET_EXPORT/USAGE_AUDIT`。`frontend/src/test/e2eLaunchCoverageEvidence.test.ts`
  先红后绿覆盖正例和负例：缺显式条件行、source/condition/evidence 错配、系统族缺科研伦理数据、消费者或标准资源已完成、
  健康状态为 `HEALTHY`、缺脱敏队列缺失能力、scope 冒领完整科研数据服务，均不声明 `S34__MISSING_DATA`。
- 第二百三十批真实 E2E：临时启动后端 18102（dev/H2，既有 jar）和前端 5175（本批启动，已停止；复核
  18102/5175 无监听）。执行
  `E2E_EXTERNAL_DEPLOYMENT=1 E2E_BASE_URL=http://localhost:5175 E2E_API_BASE_URL=http://localhost:18102/medkernel/api/v1 MEDKERNEL_API_PROXY_TARGET=http://localhost:18102 E2E_EXPECT_MFA_DISABLED=1 E2E_EVIDENCE_DIR=/tmp/medkernel-e2e-s34-missing-data-condition-row-20260710-r1 npm --prefix frontend run e2e -- --project=chromium third-party-system-families-rehearsal.spec.ts`
  通过；`/tmp/medkernel-e2e-s34-missing-data-condition-row-20260710-r1/report/results.json` 读回 `status=PASSED`、
  `expected=1`、`unexpected=0`、`flaky=0`、`skipped=0`，`launchCoverage.scenarioConditionRows` 为
  `[S34__MISSING_DATA]`，同时保留既有 13 类 `thirdPartySystemFamilyDegradationRows`。后端日志确认本轮创建
  `family-research-ethics-data-*` 适配器、`onb-research-ethics-data-*` 接入申请，并生成真实数据质量报告。
- 第二百三十批验证证据：已先让
  `npm --prefix frontend run test -- e2eLaunchCoverageEvidence -- --run` 红在新增 S34 缺数态正例缺
  `S34__MISSING_DATA`；随后实现并复跑通过 `npm --prefix frontend run test -- e2eLaunchCoverageEvidence -- --run`
  （748 tests）、`npm --prefix frontend run typecheck -- --pretty false`、`npm --prefix frontend run format:check`、
  `node --test scripts/release/full-system-rehearsal.test.mjs scripts/release/launch-coverage-audit.test.mjs`（15 tests）、
  `git diff --check`。
- 第二百三十批并行只读审计结论：本批使用 1 个只读 explorer 审计剩余本地可推进候选，子代理未编辑、未暂存、未提交、
  未启动服务、未外调。审计推荐 `S34__MISSING_DATA` 为最稳候选；`S25/S28/S29/S38/S39` 未找到足够贴合强附件，
...
- 第二百二十四批本地推进：继续按“上线总账驱动”减少 `PRODUCT_SCOPE.md` §15 第 6 项 S0-S40 五态总账缺口，
  ...
  已签发 `DiagnosticReport`、报告解读推荐卡、人工完成 `REPORT_INTERPRETATION` 协同待办和已完成筛选回读；
  附件显式写出 `scenarioConditionEvidence`，来源固定为
  `REPORT_INTERPRETATION_RUNTIME_KNOWLEDGE_TODO_CLOSED`。`ReportInterpretationService.ADVISORY_PRESENT`
  同步补足医疗安全边界：报告解读不改写已签发报告、不自动开嘱、不替代医师判断。只声明 `S17__NORMAL`，
  不声明 `S17__ABNORMAL/MISSING_DATA/HIGH_RISK/DEGRADATION`，不冒领完整 S17、完整医技系统、危急值 S36、
  区域互认 S40、全部医技报告族或完整上线验收。
- 第二百二十四批实现细节：`frontend/e2e/support/launchCoverageEvidence.ts` 新增 `S17__NORMAL` 白名单、普通
  `scenarios:S17` 强附件门禁和条件行 collector；必须通过 `report-interpretation-scenario-codes` 完整结构后才声明，
  且要求 scope 否定完整范围、服务状态均为 2xx、`runtimeReleaseId/contextSnapshotId/reportId/reportType` 与上下文一致、
  `sourceVersionId` 为正数、`versionNo` 非空、推荐卡非空、`advisoryNote` 同时包含不改写和不自动开嘱、
  待办 `sourceId` 绑定推荐卡、待办已完成、`noAutoOrder=true`、已完成筛选回读同一待办。`frontend/src/test/e2eLaunchCoverageEvidence.test.ts`
  先红后绿覆盖正例和负例：缺显式条件附件、未知行、来源错配、空证据、scope 过度宣称完整 S17、报告解读非 2xx、
  runtime 不一致、缺知识来源版本身份 / 版本号 / 推荐卡、报告解读允许改写、待办未完成 / 未绑定推荐卡 /
  完成说明缺不改写边界 / 允许自动开嘱、已完成筛选未回读或 `sourceId` 不一致，均不声明 `S17__NORMAL`。
- 第二百二十四批真实 E2E：首次使用旧 jar 运行时 `stakeholder-view-rehearsal.spec.ts` 通过但 parser 只产出
  `[S23__ABNORMAL]`，根因是服务端报告解读 `advisoryNote` 缺少“不自动开嘱”，严格 collector 正确拒绝声明 S17。
  修复后重新构建后端 jar，临时启动后端 18102（dev/H2）和前端 5175（本批启动，已停止；复核 18102/5175 无监听），执行
  `E2E_EXTERNAL_DEPLOYMENT=1 E2E_BASE_URL=http://localhost:5175 E2E_API_BASE_URL=http://localhost:18102/medkernel/api/v1 MEDKERNEL_API_PROXY_TARGET=http://localhost:18102 E2E_EXPECT_MFA_DISABLED=1 E2E_EVIDENCE_DIR=/tmp/medkernel-e2e-s17-normal-condition-row-20260709-r2 npm --prefix frontend run e2e -- --project=chromium stakeholder-view-rehearsal.spec.ts`
  通过；`/tmp/medkernel-e2e-s17-normal-condition-row-20260709-r2/report/results.json` 读回 `status=PASSED`、
  `expected=1`、`unexpected=0`、`flaky=0`、`skipped=0`，`launchCoverage.scenarioConditionRows` 为
  `[S23__ABNORMAL,S17__NORMAL]`，`launchCoverage.scenarios` 为 `[S17]`；附件抽查 `advisoryNote` 为
  “报告解读仅用于辅助阅读，不改写已签发报告，不自动开嘱，不替代医师判断。”，待办完成和已完成筛选回读均为 200。
- 第二百二十四批验证证据：已先让
  `npm --prefix frontend run test -- e2eLaunchCoverageEvidence -- --run` 红在 S17 正例缺条件行；随后实现并复跑通过
  `npm --prefix frontend run test -- e2eLaunchCoverageEvidence hooks CdssFatigue -- --run`（816 tests）、
  `npm --prefix frontend run typecheck -- --pretty false`、`npm --prefix frontend run format:check`、
  `node --test scripts/release/full-system-rehearsal.test.mjs scripts/release/launch-coverage-audit.test.mjs`（15 tests）、
  `mvn -f medkernel-backend/pom.xml -Dtest=ReportInterpretationServiceTest test`（7 tests）和
  `mvn -f medkernel-backend/pom.xml -DskipTests package`。提交前仍需跑 `git diff --check`。
- 第二百二十四批并行只读审计结论：子代理未编辑、未暂存、未提交。下一批沿 `LAUNCH-06` 最低冲突候选为：
  1）`real-frontdesk-rehearsal.spec.ts` 的 `S12__ABNORMAL`，来源可固定
  `FOLLOWUP_TEMPLATE_PLAN_QUESTIONNAIRE_ABNORMAL_RETURN`，但必须绑定 `abnormalReturn.operation=REGISTER_ABNORMAL_RETURN`、
  `riskLevel=HIGH`、`registered=true`、`noAutoOrder=true`，不得冒领 S30 慢病基层双向转诊；2）
  `runtime-release-frontdesk.spec.ts` 的 `S13__DEGRADATION`，来源可固定
  `RUNTIME_RELEASE_ROLLBACK_DEGRADATION_RECOVERY`，但必须绑定 rollback posted/current readback/consumer readback、
  回滚版本递增和候选资产缺席，不得冒领目标环境灾备、134 清库重部署或 S15 运维恢复正常态；3）S33 暂不建议直接上，
  需先补 SPD/UDI 器械耗材真实消费者字段。当前无关 `docs/DEPLOYMENT_AND_REHEARSAL.md` 与 `test-results/`
  仍不要回滚、不要暂存；`/tmp` E2E 产物不要提交。
...
- 第二百零三批并行只读审计结论：诊断危急值 S36 当前两行可安全保留；其余已有强链路中，下一刀优先建议
  ...
  仍不要回滚、不要暂存；`/tmp` E2E 产物不要提交。
- 第二百零四批本地推进：继续按“上线总账驱动”推进 `PRODUCT_SCOPE.md` §15 第 6 项 S0-S40 五态矩阵，
  本批只收口围手术 / 麻醉 / 输血 S26 代表切片已有真实前台强链路，新增 3 条显式背书行：`S26__HIGH_RISK`
  （SAFETY 红线和 CDSS_RISK 风险矩阵均为 `CRITICAL`，困难气道、ASA III 和用血风险进入推荐卡，医生人工确认，
  系统不自动开嘱、不自动输血、不自动手术）、`S26__DEGRADATION`（外部手麻手术室输血核查回传收敛到
  `NOT_CONNECTED`，断连补偿不阻断本地推荐和人工确认主链路）、`S26__ABNORMAL`（围手术期时序质控形成 P1/P0
  整改任务，固定职责账号提交并复核关闭）。不声明 `S26__NORMAL` 或 `S26__MISSING_DATA`，不冒领完整 S26、
  完整围手术期 / 手麻 / 手术室 / 输血系统、完整第三方系统族覆盖、完整 S0-S40 或完整上线。
- 第二百零四批实现细节：`frontend/e2e/surgery-anesthesia-transfusion-frontdesk.spec.ts` 的
  `surgery-anesthesia-transfusion-frontdesk-codes` 附件新增 `scenarioConditionEvidence` 三行；
  `frontend/e2e/support/launchCoverageEvidence.ts` 新增 S26 条件行白名单和 collector，必须先通过完整
  `hasRequiredSurgeryAnesthesiaTransfusionFrontdeskAttachment()`，再严格校验 `code/scenarioCode/condition/source/evidence`，
  并分别绑定完整 API 证据、SAFETY 红线、风险矩阵、临床上下文、推荐卡、人工确认、出站补偿和质量整改闭环。
  `frontend/src/test/e2eLaunchCoverageEvidence.test.ts` 先红后绿覆盖正例和负例：缺显式条件附件、未知行、来源错配、
  空证据、风险矩阵允许自动执行、红线非 `CRITICAL`、推荐卡不要求医师确认、人工确认未接受、出站未 `NOT_CONNECTED`、
  出站阻断主链路、整改未复核通过或未绑定本轮推荐卡，均不声明 S26 条件行；普通 S26 主链路负例仍继续阻断普通场景覆盖。
- 第二百零四批真实 E2E：临时启动后端 18102（dev/H2，既有 jar）和前端 5175（本批启动，已停止；复核
  18102/5175 无监听）。执行
  `E2E_EXTERNAL_DEPLOYMENT=1 E2E_BASE_URL=http://localhost:5175 E2E_API_BASE_URL=http://localhost:18102/medkernel/api/v1 MEDKERNEL_API_PROXY_TARGET=http://localhost:18102 E2E_EVIDENCE_DIR=/tmp/medkernel-e2e-s26-condition-rows-20260709-r1 E2E_EXPECT_MFA_DISABLED=1 npm --prefix frontend run e2e -- --project=chromium surgery-anesthesia-transfusion-frontdesk.spec.ts`
  通过；`/tmp/medkernel-e2e-s26-condition-rows-20260709-r1/report/results.json` 读回 `status=PASSED`、
  `expected=1`、`unexpected=0`、`flaky=0`、`skipped=0`，`launchCoverage.scenarioConditionRows` 为
  `[S26__HIGH_RISK,S26__DEGRADATION,S26__ABNORMAL]`，`launchCoverage.scenarios` 为 `[S26]`，
  `thirdPartySystemFamilyConsumerSlices` 为 `[NURSING_ANESTHESIA_TRANSFUSION_ICU]`。
- 第二百零四批验证证据：已先让 `npm --prefix frontend run test -- e2eLaunchCoverageEvidence -- --run`
  红在 S26 正例缺条件行，随后实现并复跑通过 `npm --prefix frontend run test -- e2eLaunchCoverageEvidence -- --run`
  （419 tests）、`npm --prefix frontend run typecheck -- --pretty false`、`npm --prefix frontend run format:check`、
  `node --test scripts/release/full-system-rehearsal.test.mjs scripts/release/launch-coverage-audit.test.mjs`（15 tests）。
  提交前仍需跑 `git diff --check`。
- 第二百零四批边界与下一步：本批只是把围手术 S26 已有强链路接入 3 条五态总账行，仍不是完整上线完成、
  不是完整 205 行五态矩阵完成、不是 34 入口全部业务深度完成、不是全医学知识生产完成、不是 134 清库重部署。
  下一批建议继续沿 `LAUNCH-06` 选择同一附件多行的药学审方 `S18__HIGH_RISK/S31__DEGRADATION/S31__ABNORMAL`，
...
- 第二百一十批边界与下一步：本批只是把服务机构 S1 已有强链路接入 1 条五态总账行，仍不是完整上线完成、
  ...
  不要暂存；`/tmp` E2E 产物不要提交。
- 第二百一十一批本地推进：继续按“上线总账驱动”推进 `PRODUCT_SCOPE.md` §15 第 6 项 S0-S40 五态矩阵，
  本批只收口诊断知识维护 S3 代表切片已有真实前台强链路，新增 1 条显式背书行：`S3__NORMAL`
  （医疗引擎运营员真实前台登记标准发现项术语，创建证据完整诊断资产草稿，登记诊断标准，并登记验证病例；
  四个服务状态均为 2xx，诊断资产返回正数 `identityId/versionId`，诊断标准与验证病例均绑定同一
  `standardTerm.termCode`）。不声明 `S3__ABNORMAL/MISSING_DATA/HIGH_RISK/DEGRADATION`，不声明 S16 诊断支持，
  不把诊断知识维护代表切片冒领为完整医学知识生产、全知识上线或完整 S0-S40。
- 第二百一十一批实现细节：`frontend/e2e/diagnosis-knowledge-maintenance.spec.ts` 的
  `diagnosis-knowledge-scenario-codes` 附件从阶段名扩为结构化证据：`apiEvidence`、`standardTerm`、
  `diagnosisAsset`、`diagnosisCriterion`、`validationCase` 和 `scenarioConditionEvidence`；
  附件写入真实响应状态、知识身份 / 版本 ID、稳定身份编码、证据片段、标准术语编码和验证病例身份。
  `frontend/e2e/support/launchCoverageEvidence.ts` 新增 S3 条件行白名单和 collector，必须先通过完整
  `hasRequiredDiagnosisKnowledgeScenarioAttachment()`，再严格校验 `code/scenarioCode/condition/source/evidence`，
  并逐项绑定 4 个服务状态 2xx、`identityId/versionId` 正数、关键字符串非空、诊断标准和验证病例发现项与标准术语一致。
  `frontend/src/test/e2eLaunchCoverageEvidence.test.ts` 先红后绿覆盖正例和负例：缺显式条件附件、未知行、来源错配、
  空证据、术语服务非 2xx、缺身份 / 版本、诊断标准发现项不一致、验证病例发现项不一致、验证病例身份为空，
  均不声明 `S3__NORMAL`。
- 第二百一十一批真实 E2E：临时启动后端 18102（dev/H2，既有 jar）和前端 5175（本批启动，已停止；复核
  18102/5175 无监听）。执行
  `E2E_EXTERNAL_DEPLOYMENT=1 E2E_BASE_URL=http://localhost:5175 E2E_API_BASE_URL=http://localhost:18102/medkernel/api/v1 MEDKERNEL_API_PROXY_TARGET=http://localhost:18102 E2E_EVIDENCE_DIR=/tmp/medkernel-e2e-s3-condition-row-20260709-r1 E2E_EXPECT_MFA_DISABLED=1 npm --prefix frontend run e2e -- --project=chromium diagnosis-knowledge-maintenance.spec.ts`
  通过；`/tmp/medkernel-e2e-s3-condition-row-20260709-r1/report/results.json` 读回 `status=PASSED`、
  `expected=1`、`unexpected=0`、`flaky=0`、`skipped=0`，`launchCoverage.scenarioConditionRows` 为
  `[S3__NORMAL]`，`launchCoverage.scenarios` 为 `[S3]`。
- 第二百一十一批验证证据：已先让 `npm --prefix frontend run test -- e2eLaunchCoverageEvidence -- --run`
  红在新增正例缺 `S3__NORMAL`，随后实现并复跑通过
  `npm --prefix frontend run test -- e2eLaunchCoverageEvidence -- --run`（506 tests）、
  `npm --prefix frontend run typecheck -- --pretty false`、`npm --prefix frontend run format:check`、
  `node --test scripts/release/full-system-rehearsal.test.mjs scripts/release/launch-coverage-audit.test.mjs`（15 tests）。
  提交前仍需在更新本文件后复跑 `git diff --check`。
- 第二百一十一批边界与下一步：本批只是把诊断知识维护 S3 已有强链路接入 1 条五态总账行，仍不是完整上线完成、
  不是完整 205 行五态矩阵完成、不是 34 入口全部业务深度完成、不是全医学知识生产完成、不是 S16 诊断支持完成、
...
- 用户最新补充：知识生产不能只覆盖少量代表样例，必须面向整个系统运行所需的全部医疗知识与术语资源，
  ...
  探索全网公域最新、最权威来源，完成来源分级、许可 / 可用性记录、原文或原包留存、结构化解析、候选生成、
  人工审核、发布生效、运行消费、影响分析、持续更新、替换、撤回和回滚。134 服务器可将原始需求文档和原文资料
  暂存在 `/zoesoft/medkernel-data` 或同级 `medkernel-data` 大容量目录；但该目录只作为受控原始资料与中间产物存储，
  不得替代关系库权威、正式资产版本、审计、机构生效版本和运行时消费证据。
- 第一百九十一批本地推进：接用户“知识要扩展需要哪些、不是只字典和指南、要制定可持续迭代规范”的校准要求，
  本批只做非破坏、可本地验证且能防止上线总账冒领的一刀：把知识范围从“11 个分类名 + V1/V2 回滚”升级为
  “全医学资源生产与持续迭代规范 + 全知识演练强证据门禁”。范围是 PRODUCT_SCOPE §15 第 4 / 10 / 14 / 15 项的
  口径校准和防冒领门禁，不是实际完成全网资料收集、134 原文包建设、全部资产入库、全角色前台复演或清库重部署。
- 第一百九十一批产品范围：`docs/PRODUCT_SCOPE.md` 在 §8 新增“全医学资源生产与持续迭代规范”，明确全知识不是
  11 个分类名或一次性上传指南 / 字典，而是从来源发现与授权核验、原文 / 原包受控留存、结构化解析与引用锚点、
  术语 / 值集 / 公式 / 规则 / 路径 / 安全 / 动作依赖拆解、候选生成、类型校验、责任审核、发布生效、运行消费、
  监控更新、替换、撤回和回滚的完整闭环。资源对象扩到国家 / 行业 / 学协会 / 监管 / 院内制度，疾病诊断，
  症状体征风险，药品与用药安全，检验检查影像病理内镜心电监测，手术操作麻醉输血介入器械耗材，护理康复营养心理疼痛安宁，
  感控公卫监管上报，病历病案医保 DRG/DIP 质控评级，中医药，专病专科区域协同，患者宣教随访健康管理，
  模型提示模板测试集影子评测材料等，并规定每类要落到 13 类版本化资产或运行证据。134 的 `medkernel-data`
  只能作为受控原始资料库和加工暂存区，不替代关系库权威、正式版本、审计、机构生效清单、离线交付或运行消费证据。
- 第一百九十一批门禁实现：`scripts/release/full-system-rehearsal-lib.mjs` 的 `full-knowledge` 阶段新增强校验：
  `sourceVerification` 必须覆盖 11 域且每域为 VERIFIED、2xx、带 `sourceUrl`、`contentSha256` 和 `matchedTerms`；
  `knowledge` 行必须覆盖 11 域且具备稳定身份、来源版本 / 哈希、作业与模型任务、模型 / 提示 / 工具版本、候选、
  分类、正式版本、ACTIVE 状态、质量门、责任审核动作、影子评测、质量门记录、运行 activeVersion、引用数和来源证据数；
  `observability` 必须 completedDomains=11、remainingDomains=0、modelTasks>=12；`safety` 必须证明不含凭证、
  不含患者数据、不生成临床动作、不自动开嘱。`scripts/release/full-system-rehearsal.test.mjs` 与
  `scripts/release/launch-coverage-audit.test.mjs` 改用完整 `fullKnowledgeEvidence()` 夹具，并新增缺来源核验、
  缺运行引用 / 来源证据时拒绝放行的测试。
- 第一百九十一批验证证据：已复跑
  `node --test scripts/release/full-system-rehearsal.test.mjs scripts/release/launch-coverage-audit.test.mjs`
  通过（13 tests，0 fail）；`git diff --check` 退出码 0，仅提示无关 `docs/DEPLOYMENT_AND_REHEARSAL.md`
  与三个脚本文件 CRLF 将被 LF 替换，无 whitespace error。当前无关 `docs/DEPLOYMENT_AND_REHEARSAL.md`
  仍为工作树脏文件，不要回滚、不要暂存；`test-results/` 不要暂存。
- 第一百九十一批边界与下一步：本批仍不是完整知识库、不是全网权威来源实际采集完成、不是全部医学资源资产化完成、
  不是 34 入口全部真实业务动作闭环、不是 S0-S40 全异常 / 缺数 / 高风险 / 降级矩阵完成，也不是 134 清库 / 重部署复演。
  下一批若继续知识主线，应从 PRODUCT_SCOPE §8.1 建实际生产链：来源目录与权威 / 许可分级、134 原文包目录索引和校验和、
  结构化解析批次、资产类型映射、覆盖矩阵、缺口队列、人工审核、运行消费者证据、持续更新探测、替换 / 撤回 / 回滚审计；
  同时主线仍要并行推进 34 入口真实业务动作、第三方系统族真实消费者、S0-S40 降级矩阵和最终 134 破坏性复演。
