...
## 2026-06-14 P5 第一阶段**重新打开完整验收**：补齐功能、页面、菜单、嵌入业务系统与沙盘链路后再收官
...
- **重新打开原因**：第二轮演练遗漏院内业务系统嵌入链路；134 对跨站嵌入预检返回 401，响应同时存在 `X-Frame-Options: DENY` 与 `SAMEORIGIN`，真实 HIS/EMR iframe 无法加载。进一步核查还发现术语一对多冲突缺前台处置、失败演练遗留整改任务未闭环，以及 D0–D6 页面/菜单/功能/证据需要统一复核。
- **当前执行目标**：不限定五个一级主域，按客户任务重新裁决信息架构；专家能力按普通功能归类，不单独拆“专家入口”；功能缺失直接补齐，页面和菜单不合理直接优化；院内用户、院内组织、院内字典、角色与身份绑定须提供对接业务系统的同步/更新接口；D0–D6 B0 功能、权限、数据、页面与真实证据一致后再形成第一阶段最终验收。
- **沙盘纳入范围**：`/sandbox` 以 HIS/EMR/LIS 宿主业务系统视角模拟真实数据录入、调用真实引擎、拉起嵌入式终端并留完整证据链；规则卡来自已发布规则动作定义，经 `RecommendationDeterministicMatcher` 产卡；数据三入口复用临床事件、上下文快照与适配器 inbound；新增 `sandbox.run` 权限与 `menu.sandbox` 时必须通过同一菜单/权限治理和演练证据。
- **本轮长任务计划**：总控计划已建 `docs/superpowers/plans/2026-06-14-p5-first-phase-final-closeout.md`，复用沙盘 A/A2/B/C/D 子计划，把嵌入宿主、沙盘、术语冲突、院内主数据同步、遗留整改、覆盖矩阵、本地门禁和 134 统一复演串成一条收官线。
- **本轮本地收敛事实（2026-06-14）**：新增 `GET /api/v1/engine/sandbox/scenarios`，场景目录由后端 `SandboxScenarioCatalog` 提供，前端 `useSandboxScenarios` 消费目录并只保留“后端目录不可用”的诚实占位，不再在前端伪造临床场景阈值；沙盘数值录入改为目录驱动，仍调用真实引擎编排。主数据同步 traceId 回退改为请求上下文 traceId，避免 UUID 冒充 hash；`master_data_sync_*`、`followup_template` 迁移表名收敛为 `mk_integration_*` / `mk_followup_*`，并同步领域 owner 目录。
...
- **仍未完成，不得冒领**：本轮尚未部署 134，未跑 134 统一真实复演、发布前备份与隔离恢复、真实降级探针、沙盘全真目标环境脚本、整改闭环目标环境脚本，也未形成最终验收结论。
- **主线事实**：PR #596「P5幕10审计导出审批真实演练闭环」已于 2026-06-14（Asia/Shanghai）squash 合并，`main=5e788e4d` 的幕0–幕10演练证据保持有效，但不再作为第一阶段最终验收结论。CI 8/8 通过：后端构建测试、前端构建测试、前端 lint、中文注释、guard-rules、Corretto/Temurin/Zulu JDK smoke 全绿。
...
1. 继续完成 D0–D6 卡片到功能、页面、菜单、API、数据与演练证据的完整覆盖矩阵，缺口逐项登记；本轮已本地补齐沙盘目录驱动、主数据同步验证、迁移命名和 owner 归属。
2. 目标环境下一步按顺序执行：发布前备份与隔离恢复校验 → 部署当前分支构建到 134 → post-deploy manifest/jar/Flyway/表计数/xattr → 沙盘全真链路、嵌入宿主、主数据同步、术语一对多处置、遗留整改、降级与备份恢复复演。
3. 134 统一真实复演、敏感扫描、证据归档、CI/PR 全绿前，不得重新形成第一阶段最终验收；正式知识生产继续阻断，直到文献资料库受管根地址完成真实配置与独立验收。
