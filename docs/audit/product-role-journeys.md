...
| 质量与医保治理员 | 质量与医保治理员工作台 | `处理质量问题与整改` → `/qc/alerts` | 模型生产控制台；医保审核；评价指标 | 指标下钻到责任问题、整改和复核；模型评测与独立复核在统一控制台留痕 |
<!-- role:compliance-auditor -->
| 合规审计员 | 合规审计员工作台 | `查看审计与证据` → `/admin/audit` | 无 | 只读检索人员、对象、动作和时间，导出受控并记录证据 |
<!-- role:integration-operator -->
| 集成运维员 | 集成运维员工作台 | `维护系统接入` → `/adapter/hub` | 模型生产控制台；运行保障；身份来源 | 在统一控制台维护 Provider 与加密 Key；区分正常、降级、未连接和停服 |
<!-- role:implementation-operator -->
| 实施运维员 | 实施运维员工作台 | `继续实施与验收` → `/onboarding/guide` | 服务机构；模型生产控制台；系统接入 | 阻断项回到统一生产控制台的对应步骤，交付证据进入验收自检和审计 |
