...
| DEFER-003 | 134 生产环境缺少首次接管令牌配置 | 2026-06-24 远程运行当前 `medkernel-fresh-deploy.sh --validate-environment-only` 失败于 `MEDKERNEL_BOOTSTRAP_INIT_TOKEN` 未配置；未读取或输出密钥值 | 在 `/zoesoft/medkernel/conf/medkernel.env` 配置唯一真实接管令牌并通过环境预检 |
