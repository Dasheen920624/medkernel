...
| DEFER-015 | `PathwayTemplates` 路径配置页仍有历史硬编码节点 / 边 JSON 和粘贴式快照试运行入口，尚未形成真实 API-01 快照选择与三层配置体验 | 不影响当前 PATH-01 PR1 后端契约与真实推进实现；但 PATH-01 PR2 页面三层配置时必须关闭，不能把当前路径配置页写成体验已达标或仿真已真实闭环 | 否 | D2 `PATH-01` PR2 三层前端与页面净化 | open | 删除 `DEFAULT_NODES_JSON` / `DEFAULT_EDGES_JSON` 等前端硬编码路径模板，接入真实专病包 / 路径模板 / API-01 快照列表与详情选择，仿真请求发送 `snapshotId` 并展示后端返回的快照质量、缺失字段、映射状态和决策证据；提交页面目标测试、`npm run verify`、`npm run build`、T-GATE 和浏览器验收记录 |
| DEFER-016 | 历史迁移全仓规约 inventory 存在旧表名、租户索引、回滚说明和国产方言历史债务（`migration-convention-guard --mode=inventory` 扫描 265 个文件暴露存量问题） | 当前 PATH-01 PR1 未新增迁移，`changed` 迁移门禁扫描 0 文件且无阻断；不得宣称历史迁移规约 inventory 已清零，也不得直接改旧迁移破坏 Flyway checksum | 否 | BASE-05 迁移治理专项；GA `INFRA-10` 总验收前 | open | 制定不破坏已部署 checksum 的修复方案：通过新补偿迁移、经批准的基线重建或明确豁免治理历史表名/索引/回滚说明；提交 `node scripts/migration-convention-guard.mjs --mode=inventory` 无阻断或有批准豁免、H2/PostgreSQL/Oracle 迁移验证、CI 和审计记录 |
