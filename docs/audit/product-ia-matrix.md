...
| 七业务域导航 + 角色工作台 + 页内渐进详情 | 5 | 5 | 5 | 5 | 5 | 4 | 4 | 5 | 38 | **采用**：稳定业务域承载长期对象，工作台承载角色高频任务，专业能力按普通功能归类 |
| 阶段型分组 + 独立技术工具域 | 3 | 3 | 2 | 3 | 2 | 3 | 2 | 2 | 20 | **移除**：阶段名会过期，知识/人员错位，独立技术工具域暴露内部结构 |

## 2. 唯一架构裁决

1. 客户左侧主导航现行为：工作台、机构与人员、知识治理、临床协同、质量管理、合规安全、系统运维。
2. 一级域按长期业务对象划分，不按实施阶段、部门名称或技术组件划分。
3. 每个职责角色登录后先到角色工作台；工作台只给一个主动作和不超过三个高频入口。
4. 消息通知走页头，个人通知偏好走个人菜单；它们不形成侧栏业务域。
5. 来源血缘、知识关系、智能工作流、国产化核验和诊断工具与普通功能一样进入所属业务域，由权限控制；技术字段在页内渐进展示。
...
| 2.6 | 来源与血缘 | `/advanced/provenance` | 平台/机构知识治理员 | 实施运维员 | `MENU_PROVENANCE` |
| 2.7 | 知识关系 | `/advanced/graph` | 平台/机构知识治理员 | 集成运维员 | `MENU_GRAPH_EXPLORE` |
| 2.8 | 智能工作流 | `/advanced/ai-workflows` | 平台/机构知识治理员 | 实施运维员 | `MENU_AI_WORKFLOWS` |
...
| 3.6 | 全真体验沙盘 | `/sandbox` | 临床决策使用者、实施运维员 | 临床治理负责人、集成运维员 | `MENU_SANDBOX` |
| 4.1 | 质量管理概览 | `/qc/dashboard` | 质量与医保治理员 | 平台治理、临床治理角色 | `MENU_QC_DASHBOARD` |
...
| 5.1 | 审计与证据 | `/admin/audit` | 合规审计员 | 平台治理、人员访问、实施角色 | `MENU_ADMIN_AUDIT` |
| 5.2 | 安全与配置 | `/security/baseline` | 平台治理管理员 | 合规审计员、人员访问管理员 | `MENU_SECURITY_BASELINE` |
| 6.1 | 实施与验收 | `/onboarding/guide` | 实施运维员 | 平台治理管理员、机构管理员 | `MENU_IMPLEMENTATION_GUIDE` |
| 6.2 | 系统接入 | `/adapter/hub` | 集成运维员 | 实施运维员 | `MENU_ADAPTER_HUB` |
| 6.3 | 运行保障 | `/system/providers` | 集成运维员、实施运维员 | 平台治理管理员 | `MENU_SYSTEM_PROVIDERS` |
| 6.4 | 国产化核验 | `/advanced/domestic` | 实施运维员 | 集成运维员、平台治理管理员 | `MENU_DOMESTIC_CHECK` |
| 6.5 | 诊断工具 | `/advanced/dev-console` | 实施运维员、集成运维员 | 平台治理管理员 | `MENU_DEV_CONSOLE` |
...
| AI 工作流 | 跨域专业能力 | 智能工作流 | 知识治理 | 普通功能分类 | `/advanced/ai-workflows` | `MENU_AI_WORKFLOWS` |
...
- 前后端使用相同七业务域、入口名称、顺序、承载方式和权限目录；目录共 31 项：29 个普通主导航、1 个页头、1 个个人入口。
