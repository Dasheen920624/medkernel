...
| `DEFER-012` 规则跨域影响真实反向索引 | 本报告当轮未新增后端索引，因此未冒领关闭；后续 D2 域级验收收口已新增 `RelationalRuleImpactIndex` 并在 [待处理问题清单](deferred-issues.md) 中关闭。 | 已由 D2 域级验收关闭 |
| `DEFER-017` 路径图形编辑增强 | 2026-06-03当轮只交付结构化L2画布；2026-06-07统一接入React Flow并完成拖拽、连线、删除、布局持久化与窄屏验收。 | 已关闭 |
