...
**不做（历史切片边界）**：D3/D4 字段级加密落库当时作为独立缺口未纳入本切片；该缺口已在 2026-06-17 T6.4 通过 V145 字段级加密账本与字段分级元数据补齐。本切片三读模型仍均为 D2，不落患者字段。按科室/时间细分下钻仍未纳入。
...
- AC-1（导出部分）：规则/知识/临床信号真实异步导出、服务端分页、CSV 真实、SM3 真实；小样本抑制；三读模型 D2 不落患者字段，D3/D4 字段级加密由 2026-06-17 T6.4 独立补齐。
