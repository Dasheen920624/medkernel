...
- [x] Task 7: 写 AIK-STD-06 影子运行/指标/达标门禁测试。
- [x] Task 8: 实现影子运行记录、评测编排和不达标阻断。
- [x] Task 9: 与 LLM-07/OPT-06 机制对齐，真实基准集缺失时诚实 NOT_READY，本地提交。后端 B0 影子闸已落；真实事件流、人工反馈闭环和前端展示留后续。

### Chunk 4: 共存替换

- [x] Task 10: 写待审不执行、共存差异、替换提醒、回滚/紧急失效测试。已覆盖待审不执行、现行执行、无现行首版、非待审拒绝；回滚/紧急失效沿 SYS-08 既有测试，AI 端到端影响证据待后续。
- [x] Task 11: 实现 AIK-STD-11/09 接入 SYS-08/MED-C3 的服务和端点。后端 B0 已新增共存读模型端点，审核通过仍走 `reviewCandidate(APPROVE) → activate(...)` 原子替换。
- [ ] Task 12: 更新审核台展示与验证，本地提交。后端卡片已更新；前端左右高亮/审后任务化提醒留 Chunk 7。
