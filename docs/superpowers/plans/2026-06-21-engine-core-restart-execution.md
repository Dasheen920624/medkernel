...
> **唯一设计锚点**：[2026-06-21 集团医疗智能中枢从零重启总设计 v3](../specs/2026-06-21-engine-core-restart-design.md)。本计划只把 spec 落成可执行任务，不新增设计决策；冲突以 spec 为准。
...
- [ ] **Step 1:** 按 spec §13 改 [宪法](../../CONSTITUTION.md)：§0 **内补核心澄清（定位不改、仍中枢）** + §2/§6/§8/§15。
- [ ] **Step 2:** LANDING_PLAN/FOUNDATION/业务场景 spec 顶部加横幅「范围有效 · 定位(中枢)本就对 · 核心=规则引擎+赋能、非 SaaS · 以 v3 为准」。
...
- 定位/命名(§1)→X1 宪法 §0 内补(定位不改)；IA 4 入口(§3 件⑤/§13 spec)→S0-4 + X1 §2；不变量 R1–R11→分散在 S0–S5 验收 + §2 基准。
