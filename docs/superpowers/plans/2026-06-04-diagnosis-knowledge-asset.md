...
- [ ] **Step 2: 五方言同构补齐** — 复制为 h2/oracle/dm/kingbase 4 份，按现有迁移惯例做方言映射：Oracle/DM `TIMESTAMPTZ`→`TIMESTAMP`、`BIGSERIAL`→`NUMBER GENERATED BY DEFAULT AS IDENTITY`、`BOOLEAN`→`NUMBER(1)`、`TEXT`→`CLOB`、`VARCHAR`→`VARCHAR2`；H2（`MODE=PostgreSQL`）多数同 postgres。参照同目录任一近期五方言迁移（如 `V58__knowledge_invalidation_affected_tasks.sql`）的对应写法。
...
- [ ] **Step 3: 运行架构/owner 测试** — Run: `mvn -q -Dtest=DomainOwnershipCatalogTest,ErrorCodeTest test`（按实际存在的测试名）— Expected: PASS（每张 `mk_diagnosis_*` 表有唯一 owner）
 ...
 * value_constraint / temporal_constraint 的求值是 Plan B 挂点（接 RuleDslEvaluator），Plan A 不在此求值。
 */
@Component
...
- [ ] **Step 1: 写失败契约测试** — 仿 `RecommendationEngineControllerSecurityTest`：`POST /api/v1/engine/knowledge/diagnosis/versions/{versionId}/criteria` 需 `@PreAuthorize("@perm.has('knowledge.write')")`，无权限 403；带权限 201 返回 `ApiResult` 且 body 含 `findingTermCode`。`GET .../criteria` 需 `knowledge.read`。
...
```
（差异/鉴别/指针/测试病例的端点按同样式补齐；发布门禁在知识版本发布流里调 `service.publishGate(versionId)`，接线点留 Plan B 或知识发布服务扩展。）
...
- 测试病例发布门禁 → Task 8（`publishGate` + `ENG_DX_006`）。
