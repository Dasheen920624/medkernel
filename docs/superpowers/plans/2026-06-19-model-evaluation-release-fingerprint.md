...
**Goal:** 阻止旧部署制品的医学评测被签署或用于 Provider 放行，并修复已签署态误导与无权限包查询。
...
- [ ] 新增当前指纹、异指纹和空历史指纹用例，断言异指纹不可签署、不可通过 `isClearedForGoLive`，详情返回 `releaseCurrent=false`。
- [ ] 运行 `cd medkernel-backend && mvn -q -Dtest=ModelEvalServiceTest,MedicalRegressionRepositoryTest test`，确认测试因字段和门禁缺失失败。
...
- [ ] 增加 `releaseFingerprint` 配置和评测行字段，生产中心拒绝占位指纹。
- [ ] 生成评测时写指纹，签署与上线门禁比较当前指纹，详情返回 `releaseCurrent`。
- [ ] 新增五方言迁移、中文注释并更新最新迁移契约。
- [ ] 运行 Task 1 定向测试和迁移测试，确认全部通过。
...
- [ ] 先新增失败测试：`PASSED` 显示签署事实；历史签署显示警告；无 `package.read` 时 `usePackages` 不启用。
- [ ] 运行对应 Vitest，确认当前实现失败。
- [ ] 为详情类型增加 `releaseCurrent`，按状态渲染签署结果；为 `usePackages` 增加 `enabled`，评价指标页按权限调用。
- [ ] 重跑对应 Vitest，确认通过且不再产生包列表请求。
...
- [ ] 运行后端全量、前端 `npm run verify`、B0、中文注释、产品目录、迁移规约和 `git diff --check`。
- [ ] 更新接力：记录 2026-06-19 现场根因、旧签署仅保留历史审计、新制品须重跑评测。
- [ ] 本地提交，不 push、不创建 PR。

### Task 5: 134 前向部署与现场核验
