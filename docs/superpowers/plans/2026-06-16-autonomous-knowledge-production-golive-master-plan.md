...
> 代码无法伪造的真实外部前置——这一步 = "生产中心上线"本身。用户已明确按全新项目发布：发布前停服务并清空数据库、旧制品和旧运行数据，从最新迁移基线全新初始化，不迁就历史数据、不保留旧包袱，发布留痕只记录清库初始化和新版本证据。
  ...
  - [ ] T9.3 质量医保治理员复核真实医学基准集 + 专家签字；**实跑一次 PASSED 医学回归评测**（精确覆盖当前启用题）。134 已从 WHO 2024 慢性乙肝指南导入 1 条来源化高风险用例；本地模型与外部模型旧运行 `1`、`2` 虽分别得到 1/1 和 `PENDING_REVIEW`，但生成于 V151 逐例不可变证据上线前，现已由新门禁判定为证据不完整，不能签字或冒充上线证据。V151 发布后必须重跑两个模型，由真实独立医学专家在专用复核页逐例核对输入、期望、模型输出、来源引用和红线结果后留意见签字；自动化不得冒充，因此本项保持未完成。
  ...
  - [ ] T9.7 按全新项目发布流程部署最新版到 134（`mk-publish.sh --source` 当前 HEAD 全哈希并强制从干净工作树重建；不得从旧库回灌历史业务数据，不保留旧部署过渡路径）。当前功能代码 `e1f123a6615082aa167b848fd4736fa96cb7212d` 已发布，运行 JAR SHA-256=`985b06faaae3294d485a71116574ce9ee16de0df51474f18a3c89f7da2482cb8`，Flyway V150、readiness `UP`、`NRestarts=0`；本地证据与模型定义提交尚未形成并按最终 HEAD 重发，因此本项仍未勾选。
