...
■ 第三大块 · 在 134 上生产首发知识（运营，机器跑出来的）
  P10 自主公域获取 + 模型增强 → KNOWGEN-01~28 候选 → 基础包/临床包分装 → 专家审核上线
...
- **目标**：用 P4 公域引擎 + P5 模型增强**自主获取并生产** KNOWGEN 候选 → 专家审核上线，达“稳定基础发行版 + 全景分类全覆盖 + 全专科深度”。基础资产对选定官方发行版必须全量导入和清单对账，Top-N 只允许作为临床内容灰度批次，不得作为基础版本完成口径。**这是运营产物，发生在 P9 之后**。
  - [ ] T10.0 **基础发行版 F0**：锁定来源、许可、官方版本、原件 SHA-256、发行清单、覆盖矩阵、canonical ID/namespace 和语义版本策略；禁止模型生成官方编码、数据元、值集成员或单位换算。
  ...
  - [ ] T10.4 **确定性计算 F4**：生产 KNOWGEN-16 评分量表/计算器、07 评估指标、09 护理量表、10 参考区间与趋势等可复算资产。
  - [ ] T10.5 **高风险派生 F5**：生产 KNOWGEN-04 临床规则、18 检查检验适当性、19 特殊人群剂量+PGx、20 核心制度、22 急救、23 围术期/输血；每条带测试病例，高风险逐条双签。
  - [ ] T10.6 **流程组合 F6**：生产 KNOWGEN-05 路径、06 CDSS、08 随访、11 床旁、12 中医药、13 医保病案、14 公卫院感、17 鉴别诊断、21 罕见病、24 患教知情同意等组合资产。
  - [ ] T10.7 **总装 F7**：跨资产引用零断链，基础包与临床包分包并锁定依赖，完成 A1–A9、红线、无模型 B0、灰度、回滚和试点同步预验。
...
#### Phase 11 · 总验收 + GA 门禁 + 试点医院上线（第二次上线）
  - [ ] T11.1 **KNOWGEN-15 总验收**：27 类分装“基础知识包 v1.0 + 临床知识包 v1.0”，临床包锁定基础包版本；官方基础发行清单全量对账；A1–A9 通过；OPT-04 红线生效；PKG-01 可同步。
...
- ✅ wave2 全 71 卡映射：X-LLM(P5)、X-AIK(P3/P4/P6)、DATASVC(P6)、X-DOMAIN(P7)、X-KNOWGEN 代码(P7)/内容(P10)、GA(P11)。
