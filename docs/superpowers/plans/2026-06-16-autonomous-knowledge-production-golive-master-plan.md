...
> 代码无法伪造的真实外部前置——这一步 = "生产中心上线"本身。用户已明确按全新项目发布：发布前停服务并清空数据库、旧制品和旧运行数据，从最新迁移基线全新初始化，不迁就历史数据、不保留旧包袱，发布留痕只记录清库初始化和新版本证据。
  ...
  - [ ] T9.3 质量医保治理员复核真实医学基准集 + 专家签字；**实跑一次 PASSED 医学回归评测**（精确覆盖当前启用题与当前部署制品）。运行 `3`、`4` 已在旧制品 `b1eb6de3b264ade02459e512465ef947de2ab75f` 上由 `quality-governor` 账号签为 `PASSED`，但真实 WHO PDF 小样本随后暴露重复章节锚点冲突，修复将形成新发布指纹，因此旧签署只保留历史审计。修复部署后必须重新生成逐例完整、指纹匹配的新运行；Codex 可记录自动化技术审查，但不得冒用真人专家身份，本项保持未完成。
  ...
  - [ ] T9.8 验证 `/readiness` 9 闸全绿 → 跑一条真实自主获取→候选→审核→激活小样本闭环留证。旧制品曾真实达到 9/9，随后 WHO PDF 获取因同页重复编号章节生成重复 `anchor_path` 而失败；数据库事务完整回滚，事务外孤儿文件已按 SHA 精确删除。当前外部 Provider 停用、P6=false，readiness 恢复 5/9；锚点修复已按 TDD 完成，待合并部署后重跑获取、候选和新指纹评测。独立审核前可持续生产候选，不得绕过审核激活，本项保持未完成。
