...
> 代码无法伪造的真实外部前置——这一步 = "生产中心上线"本身。用户已明确按全新项目发布：发布前停服务并清空数据库、旧制品和旧运行数据，从最新迁移基线全新初始化，不迁就历史数据、不保留旧包袱，发布留痕只记录清库初始化和新版本证据。
  ...
  - [ ] T9.3 质量医保治理员复核真实医学基准集 + 专家签字；**实跑一次 PASSED 医学回归评测**（精确覆盖当前启用题与当前部署制品）。当前制品 `2c502f1e547a185dc5ab95a76d7a3329c4d1f724` 已生成运行 `7`：外部 MiMo 真实 1/1 通过，逐例证据完整、基准与 release fingerprint 均当前，状态 `PENDING_REVIEW`，执行人为 `platform-owner`、尚无 reviewer / signedAt。运行 `5`、`6` 只保留各自历史制品审计；Codex 可记录自动化技术审查和工程放行，但不得冒用真人专家身份，本项保持未完成。
  ...
  - [x] T9.6 **超管在配置中心翻 `medkernel.knowledge.production.p6-independent-acceptance = true`**（上线放行，高危二次确认 + 审计）。P6 当前为 `true`（SYSTEM 版本 6），但它只是九闸之一；Provider 仍停用，当前制品运行 `7` 尚待独立复核，因此模型调用继续失败关闭。
  - [x] T9.7 按全新项目发布流程部署最新版到 134。唯一正式清库部署后只做不清库前向发布；运行制品提交 `2c502f1e547a185dc5ab95a76d7a3329c4d1f724` 已于 `2026-06-20 18:14:18+08:00` 精确部署，manifest、运行环境制品指纹和提交一致，实际 JAR SHA-256 为 `97f7dc10bf38599f13a4628a932fc5a8048cbcd59a5be9db206be73479d04a0e`；备份 `/zoesoft/medkernel/backups/deploy-20260620-181417`，Flyway V159、服务 active/enabled、`MainPID=311379`、`NRestarts=0`，内部及 nginx HTTPS 健康检查均为 200。V159 实测旧权限、旧角色授权和 `credential_ref` 均为 0。上线后不再清库。
  - [ ] T9.8 验证 `/readiness` 9 闸全绿 → 跑一条真实自主获取→候选→审核→激活小样本闭环留证。当前制品在运行 `7` 完成后实测 readiness 为 7/9：文献根、部署形态、基准、出域治理、模型策略、版本三元组和 P6 均通过，`MODEL_PROVIDER` 因停用而阻断，`MODEL_EVALUATION` 因尚无独立签署而阻断；历史 `PASSED` 运行不再被复用。WHO PDF 来源版本 `11` 保持 8141 个片段且锚点全唯一；既有非模型候选已永久拒绝归档且 ACTIVE=0。真实专家完成运行 `7` 复核、精确启用 Provider 并确认九闸全绿前，不得启动正式知识生产。
