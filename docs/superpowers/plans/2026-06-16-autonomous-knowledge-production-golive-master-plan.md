...
> 代码无法伪造的真实外部前置——这一步 = "生产中心上线"本身。用户已明确按全新项目发布：发布前停服务并清空数据库、旧制品和旧运行数据，从最新迁移基线全新初始化，不迁就历史数据、不保留旧包袱，发布留痕只记录清库初始化和新版本证据。
  ...
  - [x] T9.7 按全新项目发布流程部署最新版到 134。唯一正式清库部署后只做不清库前向发布；当前 `origin/main=7573284bf1348fb8f437107e4c67ed626d2f3424` 已于 `2026-06-20 10:50:12+08:00` 精确部署，manifest、运行环境制品指纹和提交一致，实际 JAR SHA-256 为 `3cfe658cf36d0dab679801938b472aaf5685f5ac161f7ced9bb3517ef6eb7cc3`；Flyway V157、服务 active/enabled、`MainPID=3666533`、`NRestarts=0`，内部及 nginx HTTPS 健康检查均为 200。上线后不再清库。
  - [ ] T9.8 验证 `/readiness` 9 闸全绿 → 跑一条真实自主获取→候选→审核→激活小样本闭环留证。`7573284b` 部署后 WHO PDF 获取与解析已成功：来源版本 `11` 有 8141 个片段且锚点全唯一。随后严格 B0 非模型待编著骨架因影子服务错误要求 `knowledge.production.knowledge` 模型基准而被取消，未创建候选；当前 `codex/b0-shadow-gate-fix` 已用统一严格 B0 契约修复并通过全量门禁，待 PR、部署后重跑候选。外部 Provider 停用、P6=false、readiness 5/9；独立审核前可持续生产 B0 候选，不得绕过审核激活，本项保持未完成。
