...
> 代码无法伪造的真实外部前置——这一步 = "生产中心上线"本身。用户已明确按全新项目发布：发布前停服务并清空数据库、旧制品和旧运行数据，从最新迁移基线全新初始化，不迁就历史数据、不保留旧包袱，发布留痕只记录清库初始化和新版本证据。
  ...
  - [ ] T9.3 质量医保治理员复核真实医学基准集 + 专家签字；**实跑一次 PASSED 医学回归评测**（精确覆盖当前启用题与当前部署制品）。运行 `3`、`4` 已在旧制品 `b1eb6de3b264ade02459e512465ef947de2ab75f` 上由 `quality-governor` 账号签为 `PASSED`，只保留历史审计。锚点修复制品 `7573284bf1348fb8f437107e4c67ed626d2f3424` 已部署，B0 影子门禁还会形成下一发布指纹，因此最终修复部署后必须重新生成逐例完整、指纹匹配的新运行；Codex 可记录自动化技术审查和工程放行，但不得冒用真人专家身份，本项保持未完成。
  ...
  - [ ] T9.6 **超管在配置中心翻 `medkernel.knowledge.production.p6-independent-acceptance = true`**（上线放行，高危二次确认 + 审计）。旧制品完成签署后，P6 曾通过正式 API 置为 `true`（版本 4）并与外部 MIMO Provider 共同使 readiness 达到 9/9；PDF 小样本暴露缺陷后已立即恢复 `false`（版本 5），Provider 同步停用。修复部署与新指纹评测前保持失败关闭，本项继续未完成。
  - [x] T9.7 按全新项目发布流程部署最新版到 134。唯一正式清库部署后只做不清库前向发布；当前 `origin/main=1074f2bbf927654961953d73f67f35a89810e1bc` 已于 `2026-06-20 11:28:08+08:00` 精确部署，manifest、运行环境制品指纹和提交一致，实际 JAR SHA-256 为 `84f6786608fd28f3e096ec450f0534bd3a2962f208e43c350762f1dcc8136601`；备份 `/zoesoft/medkernel/backups/deploy-20260620-112807`，Flyway V157、服务 active/enabled、`MainPID=3740051`、`NRestarts=0`，内部及 nginx HTTPS 健康检查均为 200。上线后不再清库。
  - [ ] T9.8 验证 `/readiness` 9 闸全绿 → 跑一条真实自主获取→候选→审核→激活小样本闭环留证。WHO PDF 来源版本 `11` 保持 8141 个片段且锚点全唯一；`1074f2bb` 部署后，获取运行 `acq:e6ff8e9b-27eb-4eb3-bced-4d070d39265d` 复用该版本，生产 job `c1358725-501f-4df4-8036-a4a489542716` 已生成 1 个严格 B0 `KNOWLEDGE` 候选，0 跳过、0 阻断、9/9 安全门通过，影子为零用例 `PENDING_REVIEW` 且 `ready_for_review=true`。候选版本 `9` 仍为 `PENDING_REPLACEMENT_REVIEW`，审核席位归 `platform-knowledge-governor`，ACTIVE=0、已批准席位=0。Provider 保持停用、P6=false，旧评测指纹不得复用；真实专业人员完成编著、当前制品评测与签署前不得激活，因此本项仍只完成到候选审核边界。
