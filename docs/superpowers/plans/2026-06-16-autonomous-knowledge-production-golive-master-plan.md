...
> 代码无法伪造的真实外部前置——这一步 = "生产中心上线"本身。用户已明确按全新项目发布：发布前停服务并清空数据库、旧制品和旧运行数据，从最新迁移基线全新初始化，不迁就历史数据、不保留旧包袱，发布留痕只记录清库初始化和新版本证据。
  ...
  - [ ] T9.3 质量医保治理员复核真实医学基准集 + 专家签字；**实跑一次 PASSED 医学回归评测**（精确覆盖当前启用题与当前部署制品）。干净正式库已从 SHA 核验的 WHO 2024 慢性乙肝指南重建来源化高风险用例；运行 `1`（本地）与 `2`（外部）在 2026-06-19 22:34 已由 `quality-governor` 签为 `PASSED`，签署事实保留审计。但两次运行产生于 V156 制品指纹门禁部署前，`release_fingerprint` 均为空，只能视为历史制品签署，不能用于当前 Provider/P6 放行。剩余步骤是在当前 `65d164a7` 制品重跑评测并由真实独立医学专家逐例核对、签署；自动化不得冒充，因此本项保持未完成。
  ...
  - [ ] T9.6 **超管在配置中心翻 `medkernel.knowledge.production.p6-independent-acceptance = true`**（上线放行，高危二次确认 + 审计）。代码门禁已收紧为只有已验签的内置 `SYSTEM_SUPERADMIN` 可执行 `false → true`，租户覆盖、非超管放行及回滚绕过均被拒；关闭仍可快速失败关闭。现场值曾于 2026-06-20 00:18 被提前改为 `true`，但两条历史签署均缺当前 release fingerprint，不满足 T9.3；已于 00:32 经正式 API 恢复 `false` 并保留审计。本项须等待当前合并制品重新评测并由真实独立医学专家签署后再执行，不能因曾短暂打开或门禁代码完成而勾选。
  - [x] T9.7 按全新项目发布流程部署最新版到 134。唯一正式清库部署后只做不清库前向发布；当前冻结制品为 `65d164a7e3ebe8f0e56ee2be5be1aeb50fb0946c`，manifest、运行环境制品指纹和提交精确一致，JAR SHA-256=`bcf9f90d48e5f2b22733535bfe9ab6831c5804b5948153542addaae95cde488a`、前端归档 SHA-256=`81d20e6ae5e63fe2923217e11c7208896e5bc9c4c666ffb18aca75d8e3b7ed5a`；Flyway V156、服务 active/enabled、`MainPID=2370446`、`NRestarts=0`，内部及经服务器证书校验的 HTTPS readiness 均为 200。上线后不再清库。
  - [ ] T9.8 验证 `/readiness` 9 闸全绿 → 跑一条真实自主获取→候选→审核→激活小样本闭环留证。当前 134 仍按预期被专家边界阻断：两个 Provider 均 HEALTHY 但停用，P6=false；历史运行 `1`、`2` 虽已签 `PASSED`，但 V156 核验为 `release_fingerprint=NULL`，不能点亮当前制品评测与启用态版本联动门禁。自动化不得代签、提前启用、翻 P6 或创建正式候选，本项保持未完成。
