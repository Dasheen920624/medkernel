...
> 代码无法伪造的真实外部前置——这一步 = "生产中心上线"本身。用户已明确按全新项目发布：发布前停服务并清空数据库、旧制品和旧运行数据，从最新迁移基线全新初始化，不迁就历史数据、不保留旧包袱，发布留痕只记录清库初始化和新版本证据。
  ...
  - [x] T9.2 集成运维员配真 provider（Claude/OpenAI 协议型/Ollama）+ 租户加密凭据库 + 健康检查 HEALTHY。当前正式库仅有本地 `ollama-qwen25-15b/medkernel-qwen25:1.5b-v1` 与外部 `external-mimo-v25/mimo-v2.5` 两个 Provider，均停用；外部 MiMo Key 已从旧环境配置一次性迁入 SM4 加密凭据库，健康检查为 `VAULT / HEALTHY`，环境文件和当前进程中的旧 Key 已删除。V159 已删除 `credential_ref` 与环境变量回退，不保留兼容入口；启用仍须 MFA、二次确认、原因、当前乐观锁版本、部署形态和精确已签评测。
  - [ ] T9.3 质量医保治理员复核真实医学基准集 + 专家签字；**实跑一次 PASSED 医学回归评测**（精确覆盖当前启用题与当前部署制品）。当前制品 `b6e0c32ce8b30305123fd96b6bae7e52db31aa36` 已生成运行 `6`：外部 MiMo 真实 1/1 通过，逐例证据完整、基准与 release fingerprint 均当前，状态 `PENDING_REVIEW`，执行人为 `platform-owner`、尚无 reviewer。历史运行 `5` 仅对 `1074f2bb...` 有效；Codex 可记录自动化技术审查和工程放行，但不得冒用真人专家身份，本项保持未完成。
  ...
  - [x] T9.6 **超管在配置中心翻 `medkernel.knowledge.production.p6-independent-acceptance = true`**（上线放行，高危二次确认 + 审计）。P6 当前为 `true`（SYSTEM 版本 6），但它只是九闸之一；Provider 仍停用，当前制品运行 `6` 尚待独立复核，因此模型调用继续失败关闭。
  - [x] T9.7 按全新项目发布流程部署最新版到 134。唯一正式清库部署后只做不清库前向发布；当前 `origin/main=b6e0c32ce8b30305123fd96b6bae7e52db31aa36` 已于 `2026-06-20 17:43:56+08:00` 精确部署，manifest、运行环境制品指纹和提交一致，实际 JAR SHA-256 为 `17a9fd216981f172a6ca881d7b6d18e53c569e6acfcf91afffefe0f1913d317b`；备份 `/zoesoft/medkernel/backups/deploy-20260620-174355`，Flyway V159、服务 active/enabled、凭据迁移后 `MainPID=261966`、`NRestarts=0`，内部及 nginx HTTPS 健康检查均为 200。V159 实测旧权限、旧角色授权和 `credential_ref` 均为 0。上线后不再清库。
