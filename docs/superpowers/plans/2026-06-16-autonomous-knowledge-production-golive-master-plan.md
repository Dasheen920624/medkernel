...
■ 第三大块 · 在 134 上生产首发知识（运营，机器跑出来的）
  P10 自主公域获取 + 模型增强 → KNOWGEN-01~35 候选 → 基础包/临床包分装 → 专家审核上线
...
> 代码无法伪造的真实外部前置——这一步 = "生产中心上线"本身。用户已明确按全新项目发布：发布前停服务并清空数据库、旧制品和旧运行数据，从最新迁移基线全新初始化，不迁就历史数据、不保留旧包袱，发布留痕只记录清库初始化和新版本证据。
  - [x] T9.0 清库发布准备：最终候选 `95b53321c7baeb4e05e70b62834074fc59df323e` 已按“即时备份与隔离恢复→唯一一次停服清库→从零迁移→精确制品部署→独立验收”执行。正式库从空库迁移至 V152 / 207 张 public 基表，知识、候选、获取、评测、Provider 和版本包关键表均为 0，未回灌历史业务数据；历史备份、旧制品和旧运行数据已清理，当前仅保留本次清库前恢复锚点。
  - [x] T9.1 运维准备受管资料库后端（现场本地磁盘、COS/OSS/MinIO 或 HTTPS 网关）→ 配 `文献库根 URI`（受管、含 `/platform-knowledge/t-1/literature-materials/`、非 tmp/非明文 HTTP）。134 已按容量要求迁移到 `/medkernel-data` 的 COSFS 大容量挂载，`SYSTEM` 配置版本 3 为 `file:///medkernel-data/platform-knowledge/t-1/literature-materials/`；目录 `0750 medkernel:medkernel`、WHO PDF `0640 medkernel:medkernel`，源/目标 SHA-256 一致，服务用户读写探针和 readiness 均通过。旧系统盘目录暂保留作回滚副本，不再承接新写入。
  - [x] T9.2 集成运维员配真 provider（Claude/OpenAI 协议型/Ollama）+ 凭据（`credential_ref`）+ 健康检查 HEALTHY。当前干净正式库仅有本地 `ollama-qwen25-15b/medkernel-qwen25:1.5b-v1` 与外部 `external-mimo-v25/mimo-v2.5` 两个 Provider，均 HEALTHY 且停用；外部 HTTPS 和受控环境凭据映射已核验。V152 受控启停入口已部署 134，配置 PUT 强制停用且响应脱敏；启用仍须 MFA、二次确认、原因、当前乐观锁版本、部署形态和精确 `PASSED` 评测。
  - [ ] T9.3 质量医保治理员复核真实医学基准集 + 专家签字；**实跑一次 PASSED 医学回归评测**（精确覆盖当前启用题与当前部署制品）。干净正式库已从 SHA 核验的 WHO 2024 慢性乙肝指南重建来源化高风险用例；历史运行 `1`（本地）与 `2`（外部）在 2026-06-19 22:34 已由 `quality-governor` 签为 `PASSED`，但 `release_fingerprint` 均为空，只保留历史审计。当前 `origin/main=b1eb6de3b264ade02459e512465ef947de2ab75f` 已部署 134，并已重新生成运行 `3`、`4`：均真实 1/1、逐例证据完整、`release_fingerprint` 精确等于当前部署提交，状态为 `PENDING_REVIEW`。`quality-governor` 已完成改密和 MFA 绑定，仍须由真实独立医学专家逐例核对并签署；自动化不得冒充，因此本项保持未完成。
  - [x] T9.4 平台治理管理员配 部署形态=PRODUCTION_CENTER + 出域白名单 + 能力策略 + prompt/tool/model 版本三元组。当前 `SYSTEM` 部署形态配置版本 2；出域仅允许 `prompt`，MEDIUM、`MASK_ALL`、HIGH 审批阈值且护栏锁定；`rule.draft` 使用 `EXTERNAL_MODEL → LOCAL_MODEL → BASELINE`、60 秒超时和每分钟 10 次限流；ACTIVE 版本包 `1` 精确绑定 `mimo-v2.5` 及三项 64 位 hash。
  - [x] T9.5 公域源 allowlist 起步集审批生效（§7 确认后）。干净正式库的 `WHO-CHB-GUIDELINE-2024` 由 `knowledge-source-steward` 登记、`platform-owner` 独立审批；许可 `CC BY-NC-SA 3.0 IGO`、robots=`ALLOW_FETCH`、调度关闭。当前 PDF 长度 3,515,427 bytes、SHA-256=`e44231194db4a3c7378b9949752c2b1cf1fdb7629793a543a92792cdda0e785c`，并已对第 36 页来源原文做提取与渲染目视核验。
  - [ ] T9.6 **超管在配置中心翻 `medkernel.knowledge.production.p6-independent-acceptance = true`**（上线放行，高危二次确认 + 审计）。代码门禁已收紧为只有已验签的内置 `SYSTEM_SUPERADMIN` 可执行 `false → true`，租户覆盖、非超管放行及回滚绕过均被拒；关闭仍可快速失败关闭。现场值曾于 2026-06-20 00:18 被提前改为 `true`，但两条历史签署均缺当前 release fingerprint，不满足 T9.3；已于 00:32 经正式 API 恢复 `false`（SYSTEM 版本 3）并保留审计。当前发布运行 `3`、`4` 已生成但仍待真人专家签署，两个 Provider 仍停用，因此本项继续保持未完成。
  - [x] T9.7 按全新项目发布流程部署最新版到 134。唯一正式清库部署后只做不清库前向发布；当前 `origin/main=b1eb6de3b264ade02459e512465ef947de2ab75f` 已于 `2026-06-20T01:45:32+08:00` 精确部署，manifest、运行环境制品指纹和提交一致，manifest 与实际 JAR SHA-256 均为 `f8f24daa7595cd3af177db6446be97e1c99fd1a3e247929fd045c51495a29aee`；Flyway V157、服务 active/enabled、`MainPID=2631313`、`NRestarts=0`，内部及 nginx HTTPS 健康检查均为 200。上线后不再清库。
  - [ ] T9.8 验证 `/readiness` 9 闸全绿 → 跑一条真实自主获取→候选→审核→激活小样本闭环留证。当前 134 精确运行 `b1eb6de3b264ade02459e512465ef947de2ab75f` / Flyway V157，文献根已迁至 `/medkernel-data`；当前发布运行 `3`、`4` 均为真实 1/1 且指纹匹配，但仍为 `PENDING_REVIEW`。两个 Provider 均 HEALTHY 但停用、P6=false，readiness 为 5/9；基础初始化批次幂等 `REUSED`，8 个 MEDIUM 条目均 `PENDING_REVIEW`。自动化不得代签、提前启用、翻 P6 或替代真实治理审核，本项保持未完成。
- **验收**：readiness 全绿 + 一条真实知识端到端上线留证 + 清库初始化记录 + 134 运行 manifest 哈希一致。

### 〈第三大块 · 在 134 上生产首发知识〉

#### Phase 10 · 首发知识资产真产出（运营，在 live 的 134 上跑出来）
- **目标**：用 P4 公域引擎 + P5 模型增强**自主获取并生产** KNOWGEN 候选 → 专家审核上线，达“稳定基础发行版 + 全景分类全覆盖 + 全专科深度”。基础资产对选定官方发行版必须全量导入和清单对账，Top-N 只允许作为临床内容灰度批次，不得作为基础版本完成口径。**这是运营产物，发生在 P9 之后**。
  - [ ] T10.0 **基础发行版 F0**：生产 KNOWGEN-29 权威来源与许可目录，锁定来源、行政/司法适用范围、许可、官方版本、原件 SHA-256、发行清单、canonical ID/namespace 和语义版本策略；禁止模型生成官方编码、数据元、值集成员或单位换算。
  - [ ] T10.1 **语义地基 F1**：先生产 KNOWGEN-26 医疗数据元与字段目录、KNOWGEN-27 编码系统/值集/单位/系统字典、KNOWGEN-28 医疗主数据与互操作基线，再生产 KNOWGEN-01 全量标准术语；全部通过条目数/hash 对账、引用完整性、层级和值域校验。
  - [ ] T10.2 **证据地基 F2**：生产 KNOWGEN-25 GRADE/推荐强度/来源 A–E 映射和冲突仲裁。
  - [ ] T10.3 **原始事实 F3**：生产 KNOWGEN-02 药品说明书、03 指南条款，以及法规、制度、政策等可引用原始事实。
  - [ ] T10.4 **确定性构件 F4**：生产 KNOWGEN-16 评分量表/计算器、07 评估指标、09 护理量表、10 参考区间与趋势、30 条件片段/安全红线/风险矩阵/动作卡/医嘱套餐/子路径等可复用资产。
  - [ ] T10.5 **高风险派生 F5**：生产 KNOWGEN-04 临床规则、18 检查检验适当性、19 特殊人群剂量+PGx、20 核心制度、22 急救、23 围术期/输血；每条带测试病例，高风险逐条双签。
  - [ ] T10.6 **流程组合 F6**：生产 KNOWGEN-05 路径、06 CDSS、08 随访、11 床旁、12 中医药、13 医保病案、14 公卫院感、17 鉴别诊断、21 罕见病、24 患教知情同意、33 全生命周期预防、34 联合照护、35 器械耗材与设备安全等组合资产。
  - [ ] T10.7 **机构落地 F7**：生产 KNOWGEN-31 机构临床参数与本地化模板，参考区间、危急值、流程和制度由机构审核发布，不把地区/设备/人群相关值伪装成平台统一常量。
  - [ ] T10.8 **总装验证 F8**：生产 KNOWGEN-32 金标回归与发行兼容资产；六维覆盖矩阵无责任空洞，跨资产引用零断链，基础包与临床包分包并锁定依赖，完成发行 BOM、A1–A9、红线、无模型 B0、灰度、回滚和试点同步预验。
...
#### Phase 11 · 总验收 + GA 门禁 + 试点医院上线（第二次上线）
  - [ ] T11.1 **KNOWGEN-15 总验收**：34 类分装“基础知识包 v1.0 + 临床知识包 v1.0”，临床包锁定基础包版本；官方基础发行清单全量对账；六维覆盖矩阵无必需空洞；A1–A9 通过；OPT-04 红线生效；PKG-01 可同步。
...
- ✅ wave2 全 78 卡映射：X-LLM(P5)、X-AIK(P3/P4/P6)、DATASVC(P6)、X-DOMAIN(P7)、X-KNOWGEN 代码(P7)/内容(P10)、GA(P11)。
