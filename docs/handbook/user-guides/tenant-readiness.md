...
规则详情页的“可读预览”仍会暴露 `observations[].valueNumeric`、`K001` 和技术 ID；呼吸科医生不能进入规则库，只能进入规则校验页。因此 `OPT-VIS-01` 继续成立：规则需要给非配置者看的自然语言摘要和流程图只读视图。
...
| 医生读不懂或进不去规则库 | 医生角色没有配置页权限，现有可读预览仍偏技术 | 用 `/rule/validate` 查看运行解释；后续通过 `OPT-VIS-01` 增加受控只读入口、自然语言摘要和流程图 |
