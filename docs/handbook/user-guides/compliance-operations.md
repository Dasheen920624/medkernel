...
| 页面 / API             | 入口                                                                                                   | 用途                                                                                    |
| ---------------------- | ------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------- |
| 审计日志               | `/admin/audit`                                                                                         | 按 Trace ID、操作人、操作编码、对象类型查审计事件，打开详情看 traceId、签名和诊断链入口 |
...
| 数据权限策略与试算 API | `/medkernel/api/v1/compliance/data-permissions`、`/medkernel/api/v1/compliance/data-permissions:check` | 佐证资源、动作、医院、科室边界和字段级允许 / 拒绝结果                                   |
| 脱敏预览 API           | `/medkernel/api/v1/compliance/masking-rules:preview`                                                   | 佐证敏感字段遮罩结果；同能力已在 `/security/baseline` 脱敏预览面板开放                  |
...
| 现场问题                           | 页面                 | 讲解截图                                                                                                                                                                                                                                                                                                                                                                                                |
| ---------------------------------- | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 审计员能不能查到医生操作并看 trace | `/admin/audit`       | [审计事件筛选](../../release/evidence/v1.0-drill-20260611/幕10-合规审计与降级/ui-replay/01-ui-audit-events-doctor-filter.png)、[Trace ID 直搜](../../release/evidence/v1.0-drill-20260611/P1-体验重构/审计traceId诊断链-134复验/01-desktop-audit-trace-search.png)、[诊断链状态](../../release/evidence/v1.0-drill-20260611/P1-体验重构/审计traceId诊断链-134复验/03-desktop-trace-diagnosis-state.png) |
...
| 数据权限和脱敏规则在哪里看         | `/security/baseline` | [数据权限](../../release/evidence/v1.0-drill-20260611/幕10-合规审计与降级/ui-replay/10-ui-security-data-permissions.png)、[权限试算结果](../../release/evidence/v1.0-drill-20260611/P1-体验重构/安全基线试算预览-134复验/02-desktop-data-permission-result.png)、[脱敏预览结果](../../release/evidence/v1.0-drill-20260611/P1-体验重构/安全基线试算预览-134复验/04-desktop-masking-result.png)          |
...
| 步骤 | 操作                         | 通过信号                                                                                       |
| ---- | ---------------------------- | ---------------------------------------------------------------------------------------------- |
| 1    | 审计员登录并打开审计链       | 能按 Trace ID 直搜幕6相关事件，并从详情跳到诊断链                                              |
| 2    | 信息科管理员查看数据权限策略 | `/security/baseline` 能看到资源、动作、最小范围和允许字段                                      |
| 3    | 在安全基线页做数据权限试算   | 医院管理员可提交资源、动作、组织范围和字段清单，页面显示行级结果、命中策略、允许字段和拒绝字段 |
| 4    | 查看脱敏规则并执行预览       | 医院管理员可输入 JSON 对象，页面返回 patientName 与 idNo 的遮罩结果且 `rawAllowed=false`       |
...
| 现象               | 常见原因                                            | 处理                                                                                                      |
| ------------------ | --------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| 查不到审计链       | traceId 不对、筛选条件叠加过窄或账号缺 `audit.read` | 先用 Trace ID 直搜；仍无结果时放宽为操作人 / 对象类型筛选，再查 `/security/me` 权限                       |
| 本科室数据也被拒   | JWT 组织域与角色分配不一致                          | 查登录 claims、角色分配和 `org_unit.id`，不要改客户端请求绕过                                             |
| 脱敏预览仍显示原文 | 规则未启用、字段名不一致，或操作者有原文权限        | 在 `/security/baseline` 脱敏预览面板核对 resourceType、scenarioCode、fieldName、输入 JSON 和 `rawAllowed` |
...
- 新页面入口缺失或读不懂：产品体验重构线登记，进入能力可见性矩阵；traceId 直搜、权限试算和脱敏预览面板已由 P1 复验证据确认，不再作为幕10 L2 后续点。
