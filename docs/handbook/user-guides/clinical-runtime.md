...
> 状态：已由全流程演练幕5-7激活 · 幕8.5 已补临床路径、运行态推荐、医生闭环和智能随访前台复演配图 · P1 已补提醒与推荐中枢、医生只读路径图和复制新版本主入口
> 适用：临床医生 / 护士 / 科主任 / 专科专家 / 医务处质控员
> 证据：幕5真实演练归档在 [CAP 临床路径证据](../../release/evidence/v1.0-drill-20260611/幕5-CAP临床路径/README.md)；幕6真实演练归档在 [推荐引擎全链证据](../../release/evidence/v1.0-drill-20260611/幕6-推荐引擎全链/README.md)；幕7真实演练归档在 [随访与质控评估证据](../../release/evidence/v1.0-drill-20260611/幕7-随访与质控评估/README.md)；P1 复验证据见 [推荐中枢](../../release/evidence/v1.0-drill-20260611/P1-体验重构/推荐中枢-134复验/README.md) 和 [路径可读化](../../release/evidence/v1.0-drill-20260611/P1-体验重构/路径可读化-134复验/README.md)
...
P1 复验确认：配置者能在 L2 节点画布查看 CAP 全图；已全量生效的模板拓扑仍写保护，修改需通过详情页“复制为新版本”主入口生成下一版草稿，不能直接改已发布版本。
...
| 医生运行态     | 展示当前患者、当前节点、里程碑、关键时钟和医生只读路径图 | 医生在 `/pathway/patients` 可看当前节点、完整 CAP 路径和当前患者位置 |
...
| 7    | 图形阅读评审   | 质控员 / 专科专家在 L2 画布可按图口述完整路径；医生在运行态只读路径图可说出已完成、当前和待执行节点 |
...
医生没有路径配置页不是缺陷。配置入口属于专科专家、科主任和医务处；医生运行态入口在患者路径页。P1 路径可读化已在患者路径详情补齐“医生只读路径图”，医生能看到整条 CAP 路径、当前节点高亮、责任角色、时间窗和只读安全提示。

![医生患者路径详情](../../release/evidence/v1.0-drill-20260611/幕5-CAP临床路径/ui-replay/07-doctor-pathway-runtime-detail.png)

![医生只读路径图](../../release/evidence/v1.0-drill-20260611/P1-体验重构/路径可读化-134复验/02-desktop-patient-readonly-graph.png)
...
| 已发布模板不能直接修改 | 全量生效模板拓扑写保护，修改需创建新版本 | 在模板详情页使用“复制为新版本”，确认复制出的草稿节点、边、里程碑和指标绑定后再编辑 |
...
| 提醒与推荐中枢 | `/cdss/fatigue`                            | 查看推荐命中、反馈统计、疲劳信号，并按患者、traceId 或来源对象检索推荐卡    |
...
P1 复验已把 `/cdss/fatigue` 升级为提醒与推荐中枢。现场讲解时先用患者、traceId 或来源对象定位推荐卡，再打开“这条推荐是怎么来的”抽屉，按触发事件、命中规则、知识来源、路径上下文、待办/通知、医生反馈和药师复核七段链路解释。
...
| 为什么这么推荐     | `/cdss/fatigue`                     | [推荐链路抽屉](../../release/evidence/v1.0-drill-20260611/P1-体验重构/推荐中枢-134复验/02-desktop-recommendation-drawer.png)                                                                                                                |
...
复演结论：前台能完成推荐接收、通知已读、医生采纳 / 覆盖、待办闭环和药师复核；P1 推荐中枢已销项“推荐引擎在哪里”和“推荐为何而来”的第一批问题。仍需继续跟踪 `OPT-WORKFLOW-01`，因为待办和通知的状态同步还未统一成一个客户可口述的工作台。
...
| 待办有、通知没有 | 推荐卡与通知触达没有统一联动                                           | 先用提醒与推荐中枢按患者、traceId 或来源对象定位推荐卡；待办/通知状态同步仍归 `OPT-WORKFLOW-01` 跟踪 |
...
- 医生推荐卡处理：对应科室医生。
- DDI 覆盖复核：临床药师。
- 待办、通知和推荐状态同步不清：登记到 `OPT-WORKFLOW-01`，由体验重构批次继续处理。
