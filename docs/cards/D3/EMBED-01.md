...
- 代码范围：`EmbedFeedbackActionType` 只接受 `ADOPT` / `REJECT`；其他动作返回 `ENG_EMBED_005` 并写失败审计，不写成功反馈审计。`EmbedFeedbackResponse` 显式返回 `callbackStatus=NOT_CONNECTED`、`callbackDelivered=false`、`degradationReason=HOST_CALLBACK_NOT_CONFIGURED`，说明当前未配置宿主回调，不伪造已送达成功。
