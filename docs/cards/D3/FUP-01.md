...
- [ ] FR-3 异常回院：异常作答触发回院任务可见（汇 [SVC-CLINICAL-03](SVC-CLINICAL-03.md) 待办；异常上报响应证据已在页内只读展示，不前端造任务）。
- [ ] FR-4 进度看板：随访完成率/异常率真实统计，按作用域。（当前页面看板已收窄为当前页统计；全局聚合仍待后续后端统计口径）
...
- [ ] AC-2（FR-3/4）：异常回院可见汇待办；完成率真实。（当前页异常响应证据已展示；全局聚合统计待后续）
