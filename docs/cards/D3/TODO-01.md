...
- [ ] FR-3 闭环处理：待办流转到完成/转交，状态机闭环并写统一审计。
- [ ] FR-4 跳转来源：每条待办可跳来源页（提醒/患者/路径；仅安全站内 deepLink 显示为可点击来源，异常链接与缺失链接均诚实提示；页面展示来源对象与追踪链路，缺失 traceId 时显示诚实缺失状态）。
- [ ] FR-5 六态 + 五维 RBAC：齐全；按责任人/科室作用域；数据按 `OrgContext`。（当前用户 + 组织闭包范围默认后端保护已覆盖；页面显式组织范围筛选已接真实组织 API）
...
- [ ] AC-2（FR-3/4）：闭环处理留审计；跳转来源正确，异常 deepLink 不暴露为可点击外部 / 危险链接，缺失 deepLink 显示“来源未提供跳转”；来源对象与追踪链路来自真实 API 字段，缺失 traceId 不伪造追踪号。
