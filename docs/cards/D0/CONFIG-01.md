...
- [x] **FR-2 热生效**：配置变更即时生效，无需重启（功能开关 / JWT TTL / Cookie 策略 / 备份 RPO-RTO / 日志级别 等）。已覆盖功能开关、备份 RPO/RTO、JWT TTL、Cookie 策略、日志级别；同时清理审计降级路径与临床事件 worker 轮询间隔的旧 yml 直接读取残留。
- [x] **FR-3 变更审计 + 回滚**：每次配置变更留审计（[BASE-04](BASE-04.md)，who/before/after）+ 回滚点可恢复。
- [x] **FR-4 启动引导边界**：启动只从 yml/env 读 **DB 连接 / 端口 / profile / 迁移位置 / 启动密钥**；其余 `medkernel.*` 从配置存储读（核心 #19）；新增配置边界门禁阻断新增生产代码直接读取非启动必需 `medkernel.*`。
...
## 验收 + 验证
- [x] **AC-1（FR-1/2）**：从配置中心改功能开关/JWT TTL → 热生效（无需重启）。功能开关、备份 RPO/RTO、JWT TTL、Cookie 策略、日志级别均已覆盖。
- [x] **AC-2（FR-3）**：配置变更留审计（who/before/after）+ 回滚到前值成功。
- [x] **AC-3（FR-4）**：启动只读 yml 的 DB/端口/profile/迁移/密钥；写死业务配置 yml 被门禁/校验提示应迁配置中心；配置边界 inventory 已清零。
...
- PR3 本地证据：`SystemConfigSeeder` 种子补齐 JWT TTL、登录 Cookie 策略、日志级别、审计降级路径、临床事件 worker 轮询间隔；`JwtIssuer`、`AuthController`、`CookieBearerTokenResolver`、`RuntimeLogLevelManager`、`AuditFallbackStore`、`ClinicalEventOutboxScheduler` 均运行时读取配置中心并保留安全默认；新增 `scripts/config-boundary-guard.mjs`，CI 阻断新增生产 Java 直接读取非启动必需 `medkernel.*`；配置边界 inventory 清零。
...
- PR3：JWT TTL / Cookie 策略 / 日志级别热读取 + 审计降级路径 / 临床事件 worker 旧直接读取清理 + 启动引导边界门禁 → 收口 AC-1 剩余项与 AC-3。
