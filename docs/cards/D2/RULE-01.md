...
## 数据与迁移
- 表族：`rule_definition`/`rule_version`/`rule_applicability`/`rule_test_case`/`rule_execution_log`/`rule_override_log`；规则定义持久化优先级、抑制关系与去重窗口，适用域表只镜像 DSL 便于检索，执行日志持久化患者/就诊/语义键及 `NOT_APPLICABLE`/抑制/去重证据，人工继续独立留痕。
