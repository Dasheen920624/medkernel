...
- **明确缺口**（建卡"现状"段照实写、勿夸大）：D4 6 个前端页面已全部真实化；后续缺口转入 `D4-验收`，按域级 B0 主链路、角色矩阵、六态 / 五维 RBAC、T-GATE 和证据包统一验收。
...
| 医务处 medical-affairs | 质控规则/指标审核、整改督办、院级质控总览 |
