...
- **前端 6 页面真实化进度**（`frontend/src/pages/quality/`，`app/router.tsx` 真实路由）：`QcDashboard`→`/qc/dashboard` 已由 [QCDASH-01](QCDASH-01.md) 接入真实驾驶舱聚合/下钻；`QcAlerts`→`/qc/alerts` 已由 [QCALERT-01](QCALERT-01.md) 接入真实预警 read-model、确认端点与整改派发；`InsuranceAudit`→`/qc/insurance` 已由 [INSAUDIT-01](INSAUDIT-01.md) 接入真实医保问题列表、病案内涵审核、DRG/DIP 入组核对与医保审核整改联动；`QcEvalSets`→`/qc/eval/sets` 已由 [EVALSET-01](EVALSET-01.md) 接入真实指标台账、条件树创建、7 步流和快照仿真；`QcEvalResults`→`/qc/eval/results` 已由 [EVALRES-01](EVALRES-01.md) 接入真实评估结果、质控问题、病历证据追溯和整改派发；`AiReview`→`/aik/review` 仍待 [AIREVIEW-01](AIREVIEW-01.md) 去占位/mock + 接真实审核 / 发布 API + 六态/五维 RBAC/可下钻齐全。
...
- **明确缺口**（建卡"现状"段照实写、勿夸大）：除 [QCDASH-01](QCDASH-01.md) / [QCALERT-01](QCALERT-01.md) / [INSAUDIT-01](INSAUDIT-01.md) / [EVALSET-01](EVALSET-01.md) / [EVALRES-01](EVALRES-01.md) 外，剩余 1 个 D4 页面仍待真实化：`AiReview.tsx` 接 [AIREVIEW-01](AIREVIEW-01.md)。
