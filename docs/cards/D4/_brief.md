...
- **前端 6 页面真实化进度**（`frontend/src/pages/quality/`，`app/router.tsx` 真实路由）：`QcDashboard`→`/qc/dashboard` 已由 [QCDASH-01](QCDASH-01.md) 接入真实驾驶舱聚合/下钻；`QcAlerts`→`/qc/alerts` 已由 [QCALERT-01](QCALERT-01.md) 接入真实预警 read-model、确认端点与整改派发；`InsuranceAudit`→`/qc/insurance` · `QcEvalSets`→`/qc/eval/sets` · `QcEvalResults`→`/qc/eval/results` · `AiReview`→`/aik/review` 仍待对应页面卡去占位/mock + 接真实评估/整改 API + 六态/五维 RBAC/可下钻齐全。
...
- **明确缺口**（建卡"现状"段照实写、勿夸大）：除 [QCDASH-01](QCDASH-01.md) / [QCALERT-01](QCALERT-01.md) 外，剩余 4 个 D4 页面仍待真实化：`InsuranceAudit.tsx` 接 [INSAUDIT-01](INSAUDIT-01.md) 真实医保审核 API，`QcEvalSets.tsx` 接 [EVALSET-01](EVALSET-01.md)，`QcEvalResults.tsx` 接 [EVALRES-01](EVALRES-01.md)，`AiReview.tsx` 接 [AIREVIEW-01](AIREVIEW-01.md)。
