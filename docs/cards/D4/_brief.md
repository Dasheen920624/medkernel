...
- **明确缺口**（建卡"现状"段照实写、勿夸大）：电子病历评级目标映射与证据包（[EMR-LEVEL-01](EMR-LEVEL-01.md)/[EMR-LEVEL-02](EMR-LEVEL-02.md) 新建，无对应后端）；`InsuranceAudit.tsx` 仍由页卡 [INSAUDIT-01](INSAUDIT-01.md) 接真实医保审核 API；6 个 D4 页面仍待真实化。
