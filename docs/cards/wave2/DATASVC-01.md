...
- [ ] AC-2（FR-3/4/5）：CLI 命令域走后端鉴权可用、不直连库；MCP 动态工具目录返回含 traceId/级别/脱敏/来源/权限/降级的治理信封，D4 缺 launch token 诚实拒绝/降级；CLI/MCP 越权/绕治理被拒。
...
## 完工证据
- 代码 permalink：数据服务层四组 API + CLI 命令域 + MCP 动态工具目录 + 数据分级/字段级加密 + 审计 + 降级 + 5 方言迁移。
...
## 实现进度（2026-06-17，按工序逐 PR；卡 FR/AC 因大卡分期未全勾，诚实标切片范围）
...
- **T6.4（本切片，已实现）= D3/D4 字段级加密落库 + 数据分级元数据表**：新增 V145 五方言 `mk_engine_data_encrypted_field` / `mk_engine_data_field_policy`，字段级加密账本只存 `sm4:v1:` 密文、`sm3:` 不可逆检索 hash、`key_ref`、算法、trace 与创建人，不存患者明文；字段分级元数据记录 `field_path`、`data_level`、`encryption_required_flag`、允许通道和状态，D3/D4 由约束固定要求加密。`FieldLevelEncryptionService` 使用独立 `MEDKERNEL_FIELD_ENCRYPTION_KEY` / `medkernel.datasvc.field-encryption.secret-key` 主密钥边界，dev/test 才可用本地替代；回执与审计摘要不返回明文或密文本体。`PrivacyPolicyService.validatePrivacyPolicy` 改为 D3/D4 `allowed=true` 且 `requiresFieldEncryption=true`，D5 仍禁入。Agent 写工具 `submitProductionCandidate` / `fetchPublicMaterial` 继续在进入候选/抓取链前拒 D3/D4/D5，CLI/MCP 只能经后端受控工具，不因字段加密能力而获得患者数据写入通道。测试：`PrivacyPolicyServiceTest` 红转绿、`FieldLevelEncryptionServiceTest`、`FieldLevelEncryptionRepositoryIntegrationTest`、`ControlledToolServiceTest` D4 越权、`MigrationBaselineContractTest`、`H2BaselineMigrationTest`。
- **PR2-a（本切片，已实现）= 受控工具执行入口（CLI/MCP 共用，FR-4/5/6）**：`ControlledToolService` 把已建受控读模型以「受控工具」形式统一暴露——**仅派发到既有受控服务执行，不直连库、不绕权限脱敏审计降级**（FR-5）；每次执行包裹 `ToolExecutionEnvelope` 治理信封（traceId/数据级别/脱敏策略/来源版本/权限结果/降级状态/**真实 SHA-256 输出 hash**，FR-4）+ 工具调用审计含用途/级别/输出 hash（FR-6）；上游降级诚实透传不伪装（FR-7/铁律 #1）；未知工具结构化 404 不泄漏内部。首批注册两个 D2 工具 `queryRuleUsage`（派发规则使用读模型）、`summarizeEngineSignals`（汇总规则/知识/临床信号分组数，不虚构未建上游路径/质控）；后续继续登记 D1/D0/D4 读工具与 AIK-STD-14 写工具，但仍共用同一受控入口。`EngineDataController` 加 `GET /api/v1/engine-data/tools`（目录）+ `POST /api/v1/engine-data/tools/{toolName}:execute`（执行），复用 `engine-data.read`；契约 `engine-data` 补声明 `engine_data_tool` 审计点；产品功能目录重生成。测试：`ControlledToolServiceTest`(6：目录/信封/汇总/降级透传/未知工具结构化/审计含用途级别) + `EngineDataControllerSecurityTest` 工具用例(+2)。**B0/真实性**：纯调既有 B0 读模型，关模型仍可跑；输出 hash 真实非伪造（铁律 #1/#10）。
...
- **PR2-c（本切片，已实现，与 PR2-b 批入同一 PR #612，T6.4 已更新策略口径）= MCP 受控工具 `searchKnowledge`（D1）+ `validatePrivacyPolicy`（D0）**：① `KnowledgeSearchService` 按关键词只读检索 `knowledge_identity`（engine-knowledge 所属，仅读，复用现成 `pageByFilter/countByFilter` keyword 归一＝trim+lower+`%`，强租户隔离）映射 D1 命中列表，服务端分页；真实无匹配诚实空结果非降级、上游不可用诚实降级不伪装（铁律 #1）。② `PrivacyPolicyService` 纯数据分级 D0–D5 准入策略判定（无上游表，结果为 D0 策略元数据）：D0/D1/D2 准入；**D3/D4 在字段级加密账本与字段分级元数据落地后准入，但必须 `requiresFieldEncryption=true` 且只能经后端最小化/脱敏治理**；D5 重要个人信息禁入（FR-2）；非法级别结构化 400。`ControlledToolService` 注册两工具 + 派发；契约/产品目录无新增（searchKnowledge 走既有 `knowledge_identity` 审计点，validatePrivacyPolicy 不碰表，无新端点）。测试：`KnowledgeSearchServiceTest`(3) + `PrivacyPolicyServiceTest`(4) + `ControlledToolServiceTest` 新增(3)。**B0/真实性**：纯读现有真实知识身份事实 + 确定性策略判定，关模型仍可跑。
- **PR2-d（本切片，已实现，与 PR2-b/c 批入同一 PR #612，T6.4 已补字段加密底座）= 受控工具 `getClinicalContextExplanation`（D4）**：`ClinicalContextService` 只读校验真实临床 launch 令牌 `embed_launch_token`（engine-embed 所属，仅读不消费不写、不违域归属，强租户隔离 + 过期 + 状态 UNUSED/USED 校验），授权时返回**最小授权上下文**（触发点/角色/接入模式/会话有效期）+ **患者/就诊引用经不可逆 SHA-256 截断脱敏**（`ref:<12hex>`，**不输出原始患者字段**；若后续 D4 明文字段进入数据服务落库，须走 T6.4 `FieldLevelEncryptionService` 字段级加密账本；MCP 默认不返回可拼提示词患者上下文，核心视角 11 / FR-2）；令牌无效/过期/越租户＝诚实拒绝（`authorized=false`、不返回临床数据、不泄漏跨租户存在性），上游不可用诚实降级不以「未授权」伪装（铁律 #1）。`ControlledToolService` 注册（D4）+ `policyFor(D4)='D4_MASKED_MINIMAL_CONTEXT'`；契约 `engine-data` 补声明 `embed_launch_token` 审计点；**无新控制器/新端点**（走既有 `POST /tools/{toolName}:execute`，临床授权由 launch 令牌层叠加于 engine-data.read）。测试：`ClinicalContextServiceTest`(5：有效授权脱敏 / 无效拒绝 / 过期拒绝 / 越租户不泄漏 / 上游降级) + `ControlledToolServiceTest` 新增(3)。**B0/真实性**：纯读现有真实 launch 令牌事实，关模型仍可跑；患者引用脱敏真实不可逆。
- **PR2-e（已实现，已合并 [PR #613](https://github.com/Dasheen920624/medkernel/pull/613) `496ee09e`）= 产品级 CLI 6 命令域骨架（FR-3）**：新建 `cli/` 模块（Node ESM，零运行时依赖，面向交付/运维）。`config.mjs` 只读 `MEDKERNEL_API_BASE/TOKEN` 走后端 API 鉴权、**绝不读本地库连接串**（JDBC/DATASOURCE 一律不读，专测锁定）；`apiClient.mjs` 薄 HTTP 客户端携 Bearer 调后端受控合同、解包 `ApiResult`、错误走 RFC7807 转结构化 `CliApiError` 不泄漏内部、后端不可达诚实报错不伪造数据（铁律 #1）；`commands.mjs` 命令域 `knowledge`/`rules`/`clinical-signals`/`agent`/`privacy`/`exports`/`diagnostics`（规范 §8.5）只经受控工具/只读端点/受控导出端点；`cli.mjs` 薄入口（退出码 0/1/2）。CI `guard-rules` 加 `node --test cli/test/*.test.mjs`。后续 PR2-g 已补异步导出后端，当前命令以 `cli/README.md` 和测试为准。
- **PR2-f（已实现，已合并 [PR #614](https://github.com/Dasheen920624/medkernel/pull/614) `d5f096ba`）= MCP 服务传输层骨架（FR-4 / AIK-STD-14 底座）**：新建 `mcp-server/` 模块（Node ESM，零运行时依赖），把后端真实受控工具目录经 **MCP 协议（stdio JSON-RPC）**暴露给外部 Agent。`protocol.mjs` 映射 `initialize`/`tools/list`（经后端 `/tools` 取真实目录映射含 `inputSchema`、`purpose` 必填）/`tools/call`（派发后端 `/tools/{name}:execute`，不绕治理；失败以 MCP `isError` 返回结构化原因；未知方法 JSON-RPC -32601）；`server.mjs` stdio 换行分隔 JSON-RPC 循环（解析错误 -32700 / 通知不响应 / 空行忽略）；`config.mjs`+`apiClient.mjs` 自包含薄客户端（与 `cli/` 同形，rule-of-three 暂不抽共享包）。CI 加 `node --test mcp-server/test/*.test.mjs`。测试 16/16（config 3 + apiClient 3 + protocol 6 + server 4）。**初版**：仅 stdio + initialize/tools/list/tools/call，SSE/HTTP/resources/prompts/sampling 未实现。
- **AC-2 机制达成（FR-3/4/5）**：CLI 命令域 + MCP 动态工具目录 + REST 入口共用同一后端受控合同，全程不直连库/不读连接串/不绕权限脱敏审计降级；越权由后端权限拒、工具失败结构化。
- **PR2-g（本切片，已实现）= 异步导出后端 + CLI `exports` 接线（FR-1 异步导出 / AC-1 导出部分，补全 CLI 诚实缺口）**：新建 `com.medkernel.engine.datasvc.export`，把三组 D2 去标识聚合读模型（规则/知识/临床信号）经 **SYS-06 导出审批闸控制的异步 CSV 导出**对外开放，复用 `KnowledgeExportJob` 执行骨架。
  - `EngineDataExportJob`（表 `mk_engine_data_export_job`，V128 五方言 + 中文 COMMENT + 索引）+ `ExportJobStatus`（PENDING→RUNNING→SUCCEEDED/FAILED/CANCELLED/EXPIRED）+ `EngineDataExportType`（RULE_USAGE/KNOWLEDGE_USAGE/CLINICAL_SIGNALS，各携审批资源类型标识）；审批锚 `approval_id`/`idempotency_key`/`request_snapshot`。
  - `EngineDataExportService`：`submit` **不绕审批**（须先有 APPROVED 的 SYS-06 导出审批，资源类型 + 范围 JSON 一致否则结构化 403/409；幂等键去重）→ 写 PENDING + 事务提交后投递线程池 worker；worker 分页（批 500）拉既有读模型 + **小样本抑制（主计数 <10 → 计数列 `suppressed`、保留分组键、标 `suppressed=true`，规范 line 258）** + 写 UTF-8 BOM CSV（中文表头）→ SUCCEEDED；上游不可用诚实 FAILED 不出半真文件（铁律 #1）；TTL 7 天。
- **2026-06-17 AIK-STD-14 补充（已实现）= 受控工具 `fetchPublicMaterial` + CLI/MCP 暴露**：DATASVC 工具目录新增 D1 / `knowledge.write` 写工具 `fetchPublicMaterial`，载荷 `AgentPublicMaterialFetchPayload` 只声明公域来源、HTTPS URL、版本、格式、数据级别和可选候选生成计划；服务层拒 D3/D4/D5 后派发既有 `AcquisitionOrchestrationService.run`，因此仍受生产中心形态、allowlist、license、robots、资料库、解析和候选审核链裁决。CLI 接 `agent fetch-public-material`，MCP 继续动态暴露后端目录。测试：`ControlledToolServiceTest`、`cli`、`mcp-server`。
  - **SYS-06 审批产物来源泛化**：抽中性 `ExportArtifact` record（原 `LargeListExportArtifact` 改名）+ `ExportArtifactProvider` 接口；`LargeListEngineService` 与 `EngineDataExportService` 各实现一个，`ExportApprovalService` 注入 `List<ExportArtifactProvider>` 按审批 `resourceType` 解析唯一来源（零/多匹配结构化错）；审批流端点/状态机/证据/权限 `audit.export` 全不变。`completedExportArtifact` 按真实 CSV 文件字节算 `sm3:` 摘要。**为破 `ExportApprovalService`↔`EngineDataExportService` 循环依赖**，submit 的审批闸校验注入 `ExportApprovalRepository`（只读查审批），不依赖 `ExportApprovalService`。
  - 新权限 **`engine-data.export`**（MEDIUM，授质量/医保治理员，同 `engine-data.read` 归属，`allNonEmergencyPermissions` 自动并入超管/平台治理/机构管理员；临床决策用户无，`DefaultPermissionPolicyTest.engineDataExportRestsWithManagementAndQualityRoles` 锁定）；`EngineDataController` 加 5 端点（提交/状态/列表/取消/下载 CSV，均 `engine-data.export`）；契约 `engine-data` 加 `engine-data.export` 权限 + `mk_engine_data_export_job` 审计点；新域 `engine-data-service` 入 `DomainOwnershipCatalog`（prefix `mk_engine_data_`）；产品功能目录重生成（EngineDataController KEEP→MERGE 含导出端点 + 导出服务/配置入异步承载类 13→15）。
  - CLI `exports` 域**替换诚实缺口桩为真实动作** `submit`/`status`/`list`/`cancel`/`complete`（complete 走合规导出审批登记端点）；`apiClient` 加 `post`；CLI 仍不直连库、不绕审批。
  - 测试：`EngineDataExportServiceTest`(12) + `EngineDataExportJobRepositoryIntegrationTest`(3 真实 H2) + `EngineDataControllerSecurityTest`(+2) + `ExportApprovalServiceTest`(+2 provider 解析) + `MigrationBaselineContractTest`(V128 全套) + CLI node(+6)。**B0/真实性**：纯读既有 D2 读模型（关模型可跑）；无 APPROVED 审批不出文件；空上游诚实空 CSV；SM3 真实算文件字节。
- **PR2/PR3 待续切片（未实现，诚实标）**：MCP SSE/HTTP 传输 + resources/prompts；PR3 临床端只读解释合同（接 D3）+ 管理下钻/导出（接 D4）。**注**：T6.4 已补 D3/D4 字段级加密落库与数据分级元数据表；`getClinicalContextExplanation` 当前仍只输出不可逆脱敏引用，三组导出读模型仍为 D2 去标识聚合，均不输出患者明文。
