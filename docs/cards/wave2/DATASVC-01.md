...
- **PR2-d（本切片，已实现，与 PR2-b/c 批入同一 PR #612）= MCP 第 7 工具 `getClinicalContextExplanation`（D4），MCP 工具达 7/7（AC-2 机制补齐）**：`ClinicalContextService` 只读校验真实临床 launch 令牌 `embed_launch_token`（engine-embed 所属，仅读不消费不写、不违域归属，强租户隔离 + 过期 + 状态 UNUSED/USED 校验），授权时返回**最小授权上下文**（触发点/角色/接入模式/会话有效期）+ **患者/就诊引用经不可逆 SHA-256 截断脱敏**（`ref:<12hex>`，**不输出原始患者字段**——D4 落库须字段级加密〔当前未实现〕、且 MCP 默认不返回可拼提示词患者上下文，核心视角 11 / FR-2）；令牌无效/过期/越租户＝诚实拒绝（`authorized=false`、不返回临床数据、不泄漏跨租户存在性），上游不可用诚实降级不以「未授权」伪装（铁律 #1）。`ControlledToolService` 注册（D4）+ `policyFor(D4)='D4_MASKED_MINIMAL_CONTEXT'`；契约 `engine-data` 补声明 `embed_launch_token` 审计点；**无新控制器/新端点**（走既有 `POST /tools/{toolName}:execute`，临床授权由 launch 令牌层叠加于 engine-data.read）。测试：`ClinicalContextServiceTest`(5：有效授权脱敏 / 无效拒绝 / 过期拒绝 / 越租户不泄漏 / 上游降级) + `ControlledToolServiceTest` 新增(3)。**B0/真实性**：纯读现有真实 launch 令牌事实，关模型仍可跑；患者引用脱敏真实不可逆。
- **PR2-e（已实现，已合并 [PR #613](https://github.com/Dasheen920624/medkernel/pull/613) `496ee09e`）= 产品级 CLI 6 命令域骨架（FR-3）**：新建 `cli/` 模块（Node ESM，零运行时依赖，面向交付/运维）。`config.mjs` 只读 `MEDKERNEL_API_BASE/TOKEN` 走后端 API 鉴权、**绝不读本地库连接串**（JDBC/DATASOURCE 一律不读，专测锁定）；`apiClient.mjs` 薄 HTTP 客户端携 Bearer 调后端受控合同、解包 `ApiResult`、错误走 RFC7807 转结构化 `CliApiError` 不泄漏内部、后端不可达诚实报错不伪造数据（铁律 #1）；`commands.mjs` 6 命令域 `knowledge`/`rules`/`clinical-signals`/`privacy`/`exports`/`diagnostics`（规范 §8.5）只经受控工具/只读端点；**`exports` 后端异步导出未实现——诚实标缺口、不伪造任务、不绕导出审批**；`cli.mjs` 薄入口（退出码 0/1/2）。CI `guard-rules` 加 `node --test cli/test/*.test.mjs`。测试 21/21（config 4 + apiClient 4 + commands 7 + cli 6）。
- **PR2-f（已实现，已合并 [PR #614](https://github.com/Dasheen920624/medkernel/pull/614) `d5f096ba`）= MCP 服务传输层骨架（FR-4 / AIK-STD-14 底座）**：新建 `mcp-server/` 模块（Node ESM，零运行时依赖），把后端 7 受控工具经 **MCP 协议（stdio JSON-RPC）**暴露给外部 Agent。`protocol.mjs` 映射 `initialize`/`tools/list`（经后端 `/tools` 取真实目录映射含 `inputSchema`、`purpose` 必填）/`tools/call`（派发后端 `/tools/{name}:execute`，不绕治理；失败以 MCP `isError` 返回结构化原因；未知方法 JSON-RPC -32601）；`server.mjs` stdio 换行分隔 JSON-RPC 循环（解析错误 -32700 / 通知不响应 / 空行忽略）；`config.mjs`+`apiClient.mjs` 自包含薄客户端（与 `cli/` 同形，rule-of-three 暂不抽共享包）。CI 加 `node --test mcp-server/test/*.test.mjs`。测试 16/16（config 3 + apiClient 3 + protocol 6 + server 4）。**初版**：仅 stdio + initialize/tools/list/tools/call，SSE/HTTP/resources/prompts/sampling 未实现。
- **AC-2 机制达成（FR-3/4/5）**：CLI 6 命令域 + MCP 7 工具 + REST 三入口共用同一后端受控合同，全程不直连库/不读连接串/不绕权限脱敏审计降级；越权由后端 `engine-data.read` 拒、工具失败结构化。
