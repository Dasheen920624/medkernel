...
- **PR2 待续切片（未实现，诚实标）**：CLI 6 命令域骨架（FR-3，走后端 API 鉴权不直连库）；MCP 协议层适配（stdio/SSE 等传输层，当前为 REST 受控合同）；**D3/D4 字段级加密落库** + 数据分级元数据表 + 异步导出（FR-1 导出 / AC-1 字段级加密部分）。**注**：`getClinicalContextExplanation` 当前对患者引用做不可逆脱敏，未落库 D4 患者明文字段故未触发字段级加密；待真实 D4 患者上下文落地切片再实现字段级加密。
