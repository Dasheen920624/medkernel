...
- 新配置中心项 `medkernel.knowledge.production.p6-independent-acceptance` 默认 `false`、高风险受保护，仅作为 readiness 阻断事实源，不能替代文献库、provider、评测、出域与审核证据。放行 `false → true` 只接受已验签的内置超级管理员 authority，继续强制 MFA、二次确认、原因、乐观锁和审计；租户覆盖、租户种子与非超管放行均拒绝，回滚到 `true` 同样受控，关闭为 `false` 保留快速失败关闭能力。授权拒绝以独立子事务记录失败审计，不能随业务事务回滚丢失。
